	...
	AND CK_SRNO IN(SELECT MAX(CK_SRNO) FROM Client_CKYC WHERE CK_Panno = CXXX.CK_Panno))
		...
		PersonalDetails = (
				...
				SELECT Top 3 [BankAccNo] = ltrim(rtrim(ba_actno)), 
					...
					[IsBankDeleted] = '', BankAttachment = '', NameAsPerBank = ''
 ...
 --- Variable Declaration Start
  ...
  BEGIN
    ...
    BEGIN CATCH
      ...
      RETURN 1 		
    END CATCH 
	IF EXISTS(SELECT 1 FROM @tbl_jsonoutput WHERE ColumnName = 'message' and ColumnValue in('Account exists and is active.'))
    BEGIN 
