...
--@@VAIBHAV/18-DEC-2023/
  ...
  @strThirdPartyURL VARCHAR(MAX)='', @strRequestFrom VARCHAR(50)=''
  
  SELECT TOP 1 @strRequestFrom = rm_RequestType from Client_ReKycMain(nolock) 
  ...
  and rm_rekyc = 'N') AND @i_vcTemplateCode = 'Template1' AND @strRequestFrom <> 'RekycOffLine'
  BEGIN
    SET @o_vcErrorFlag = 'E'
 ...
 ------------ ******  Reject Logic Start ***********
  ...
  BEGIN
     DECLARE @vc_NRefNo INT = 0
	 SET @vc_NRefNo  = ISNULL((select MAX(ISNULL(ca_Nfiller3, 0)) + 1 FROM Client_ModifyAPI(NOLOCK)),1)
	  
	 IF ISNULL(@vc_NRefNo,0) = 0
	 BEGIN
	   SET @vc_NRefNo = 1
	 END
	 
	 INSERT INTO Client_ReKycMain(rm_cmcd, rm_cdate, rm_ctime, rm_rekyc, rm_status,
	 rm_desc  , mkrdt, mkrtm, rm_step, rm_refno, rm_RequestType)
	 SELECT rm_cmcd, rm_cdate, rm_ctime, rm_rekyc ='N', rm_status = 'Pending',
	 rm_desc = @i_vcApprovalReason, mkrdt = convert(varchar,getdate(),112), mkrtm = CONVERT(VARCHAR,GETDATE(),108), 
	 rm_step = 1, rm_refno = @vc_NRefNo, rm_RequestType
	 FROM Client_ReKycMain A 
	 WHERE rm_cmcd = @i_vcClient_Code  
	 AND rm_refno = @i_vcRefNo
	 
	 INSERT INTO Client_ModifyAttach(ma_cmcd, ma_date, ma_filename, ma_field, mkrdt, mkrtm, ma_proof,
	 ma_refno,ma_Nfiller1,ma_status,Requestid)
	 SELECT ma_cmcd, ma_date, ma_filename, ma_field, mkrdt, mkrtm, ma_proof, ma_refno = @vc_NRefNo, 
	 ma_Nfiller1, ma_status, Requestid 
	 FROM Client_ModifyAttach WHERE ma_cmcd = @i_vcClient_Code  AND  ma_refno = @i_vcRefNo
	 AND ma_filename NOT IN ('EsignRequest','EsignRequestKRA','UnSignedKRApdf','UnSignedPdf','SignedKRAPdf','SignedPdf')
     ...
     SELECT ca_cmcd, ca_field, ca_desc, ca_oldValue, ca_newValue, ca_date, ca_time, ca_computername, ca_Tplus ='N',
	 ca_Cross ='N', ca_Estro ='N', ca_Dematacno, ca_filler1, ca_filler2, ca_filler3, ca_Nfiller1, ca_Nfiller2, ca_Nfiller3 = @vc_NRefNo
     FROM Client_ModifyAPI 
	 where ca_cmcd = @i_vcClient_Code  AND  ca_Nfiller3 = @i_vcRefNo 
	
	 UPDATE A SET A.ma_status = 'R'
	 FROM Client_ModifyAttach A 
	 where ma_cmcd = @i_vcClient_Code  
	 AND  ma_refno = @i_vcRefNo
	
	 UPDATE A SET A.rm_status = 'Reject',
	 A.rm_Desc = @i_vcApprovalReason , A.rm_rekyc = 'R',
	 A.mkrdt = convert(varchar,getdate(),112), A.mkrtm = CONVERT(VARCHAR,GETDATE(),108)
	 FROM Client_ReKycMain A 
	 WHERE rm_cmcd = @i_vcClient_Code  AND  rm_refno = @i_vcRefNo
	  
	 UPDATE A SET A.ca_Tplus = 'R'
  ...
  BEGIN
    BEGIN TRY
	 ...
	 @strContractsColumnString VARCHAR(MAX)='',@strContractsvalueString  VARCHAR(MAX)= '',@strContractsUpdateString  VARCHAR(MAX) ='',
	 @StrUniqueContractKeyX VARCHAR(MAX)= '', @StrUniqueContractKey1 VARCHAR(MAX)='', @StrUniqueContractKey VARCHAR(MAx)='',
	 @strMaxSrNoDIN INT=0
	...
	@strMTFvalueString VARCHAR(MAX)='', @strcc_Cpanno VARCHAR(50)=''
	...
	DECLARE Cur2Main
     ...
     BEGIN 
	   SET @strcc_Cpanno = ''
	   IF @MainTradePlusTableName = 'Tbl_ClientRelatedPerson'
	   BEGIN
	     SELECT @strcc_Cpanno = ca_OLDValue FROM client_ModifyAPI(NOLOCK) WHERE ca_cmcd = @i_vcClient_Code 
		 AND ca_Nfiller3 = @i_vcRefNo
		 AND ca_field = 'cc_Cpanno' AND ca_Nfiller1 = @ca_Nfiller1
	   END
	   ...
	   set @StrUniqueContractKey = ''
	   set @strDematactUpdateString = ''
	   set @strBankactUpdateString = ''
	   set @StrUniqueBankKey1 = ''
	   set @StrUniqueContractKey1 = ''
	   set @StrUniqueDematKey1 = ''
	   set @StrUniqueBankKeyX = ''
	   set @StrUniqueContractKeyX = ''
	   set @StrUniqueDPKeyX = ''
	   SET @strContractsColumnString = ''
	   SET @strContractsvalueString = ''
	   set @strContractsUpdateString =''
       DECLARE Cur2 CURSOR FOR 
		...
		when TradePlusTableName = 'MrgTdgFin_Clients' then 'MTFC_CMCD'
       when TradePlusTableName = 'Tbl_ClientRelatedPerson' then 'cc_cmcd' else 'cm_cd' end)  from (
        ...
        BEGIN 
		  ...
		  IF @TradePlusTableName = 'Tbl_ClientRelatedPerson' 
		  AND EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = @TradePlusTableName and COLUMN_NAME = @ca_field) 
		  BEGIN
		    IF ISNULL(@strcc_Cpanno,'') = ''
			BEGIN
		      SET @strContractsColumnString = @strContractsColumnString +', '+@ca_field
		      SET @strContractsvalueString = @strContractsvalueString +' , '''+@ca_newValue+'''' 
			END
			ELSE
		    IF ISNULL(@strcc_Cpanno,'') <> ''
		    BEGIN
		      SET @strContractsUpdateString = @strContractsUpdateString +' , '+@ca_field+' = '''+@ca_newValue+'''' 
            END
			IF @ca_field IN('cc_Cpanno')
			BEGIN
			  SET @StrUniqueContractKey = @StrUniqueContractKey + @ca_oldValue
			  SET @StrUniqueContractKey1 = @StrUniqueContractKey1+'LTRIM(RTRIM('+@ca_field+'))'
			  SET @StrUniqueContractKeyX = @StrUniqueContractKeyX + @ca_NewValue
			END 
          END
		
		  IF @ca_newValue <> @ca_oldValue AND @TradePlusTableName <> 'Tbl_ClientRelatedPerson'
		  BEGIN
       ...
       END	   
	    ...
	    IF @strContractsvalueString <> ''
	   BEGIN
         DECLARE @sColumn_Name1 VARCHAR(100)='', @sdata_type1 VARCHAR(20)=''
		 DECLARE Cur31Main1
         CURSOR FOR SELECT Column_Name, data_type FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Tbl_ClientRelatedPerson'
         ...
         and column_name not in('cc_cmcd', 'cc_srno')
         OPEN Cur31Main1 
         FETCH NEXT FROM Cur31Main1 INTO @sColumn_Name1, @sdata_type1  
         WHILE @@FETCH_STATUS = 0
         BEGIN 
           IF EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Tbl_ClientRelatedPerson' and COLUMN_NAME = @sColumn_Name1) 
           BEGIN
             SET @strContractsColumnString = @strContractsColumnString +', '+@sColumn_Name1
             SET @strContractsvalueString = @strContractsvalueString +' , '''+CAST((CASE WHEN @sdata_type1 IN('int','money','numeric') THEN '0' ELSE '' END) AS VARCHAR)+'''' 
           END
           FETCH NEXT FROM Cur31Main1 INTO @sColumn_Name1, @sdata_type1  
         END
         CLOSE Cur31Main1
         DEALLOCATE Cur31Main1	
		 
		 
		 IF @strMaxSrNoDIN = 0
		 BEGIN
		   SELECT @strMaxSrNoDIN = MAX(cc_srno)+1 FROM Tbl_ClientRelatedPerson(NOLOCK) WHERE cc_cmcd = @strClientCode
		   IF ISNULL(@strMaxSrNoDIN,0) = 0
           BEGIN
		     SET @strMaxSrNoDIN = 1
           END 	
		 END
         ELSE
         BEGIN
		   SET @strMaxSrNoDIN = @strMaxSrNoDIN + 1
         END
      	 
		 
         SET @strContractsvalueString = SUBSTRING(@strContractsvalueString,3,LEN(@strContractsvalueString))
	     SET @strContractsColumnString = SUBSTRING(@strContractsColumnString,3,LEN(@strContractsColumnString))
	     SET @strContractsColumnString = @strContractsColumnString+', '+'cc_cmcd, cc_srno '
		 set @strContractsvalueString = @strContractsvalueString+', '''+@strClientCode+''','''+CAST(@strMaxSrNoDIN AS VARCHAR)+''' '
		 
	     SET @strContractsvalueString = 'INSERT INTO '+@MainTradePlusTableName+'  ('+ @strContractsColumnString +' ) VALUES (' +@strContractsvalueString+' )'	 
		 
	     BEGIN TRY
	       INSERT INTO @tbl_ExecuteCommand(TableName, QueryExecute, PrimaryKey)
		   VALUES('Tbl_ClientRelatedPerson',@strContractsvalueString, @StrUniqueContractKey)
         END TRY
	     BEGIN CATCH
	        SET @o_vcErrorFlag = 'E'
	        SET @o_vcErrorMessage = 'Tbl_ClientRelatedPerson '+ERROR_MESSAGE()
		    RETURN 1
	     END CATCH
       END
	   
	   IF @strContractsUpdateString <> ''
	   BEGIN
	     SET @strContractsUpdateString = SUBSTRING(@strContractsUpdateString,3,LEN(@strContractsUpdateString))
		 --SET @StrUniqueContractKey1 = SUBSTRING(@StrUniqueContractKey1,6,LEN(@StrUniqueContractKey1))
		 SET @strContractsUpdateString = 'UPDATE '+@MainTradePlusTableName+' SET '+@strContractsUpdateString+' WHERE cc_cmcd = '''+@strClientCode+''''
           +'and '+@StrUniquecONTRACTKey1+' = '''+@StrUniqueContractKey+''''
		 
	     BEGIN TRY
	      INSERT INTO @tbl_ExecuteCommand(TableName, QueryExecute, PrimaryKey)
		  VALUES('Tbl_ClientRelatedPerson',@strContractsUpdateString, @StrUniqueContractKey)
         END TRY
	     BEGIN CATCH
	      SET @o_vcErrorFlag = 'E'
	      SET @o_vcErrorMessage = 'Tbl_ClientRelatedPerson Update '+ERROR_MESSAGE()
		  RETURN 1
	     END CATCH
	   END
    ...
    BEGIN  
      ...
      */
	  ...
	  IF  @dpType = 'CDSL' AND @i_vcTemplateCode <>'ONLYCLOSURE' AND ISNULL(@CrossDB,'') <> ''
	  BEGIN
	...
	--BEGIN
	  ...
	  DECLARE @IserialNo INT = 0, @strCK_Reference VARCHAR(50)='', @strCKYC_NO VARCHAR(50)='', @strAccountTyp VARCHAR(2)=''
	  
	  SELECT @IserialNo = isnull(MAX(CK_SRNO),0)+ 1 
	  from(
	  select max(CK_SRNO) as CK_SRNO
	  FROM Client_CKYC(NOLOCK)
	  union all
	  select max(CI_SRNO) as CK_SRNO
	  FROM Client_CKYCImages(NOLOCK)) x123
	  ...
	  where ca_cmcd = @i_vcClient_Code  AND  ca_Nfiller3 = @i_vcRefNo AND ca_desc = 'CKYCFlag'
	  AND ca_newValue = 'Modification')
	  BEGIN
	    SELECT TOP 1 @strCKYC_NO = ca_newValue FROM Client_ModifyAPI(NOLOCK)
	    where ca_cmcd = @i_vcClient_Code  AND  ca_Nfiller3 = @i_vcRefNo AND ca_desc = 'CkycRefNo'
		SET @strAccountTyp = '03'
      END
	  ELSE IF EXISTS(SELECT 1 FROM Client_ModifyAPI(NOLOCK)
	  where ca_cmcd = @i_vcClient_Code  AND  ca_Nfiller3 = @i_vcRefNo AND ca_desc = 'CKYCFlag'
	  AND ca_newValue = 'Addition')
	  BEGIN
	    SET @strAccountTyp = '01'
	  END
	  ELSE 
	  BEGIN
	    SELECT TOP 1 @strCKYC_NO = Ck_NFiller1 FROM Client_CKYC(NOLOCK), 
	    CLIENT_MASTER(NOLOCK) CM  WHERE CM.CM_CD = @i_vcClient_Code AND CK_Panno = CM.cm_panno AND Ck_NFiller1 <> ''
	    AND CK_SRNO IN(SELECT MAX(CK_SRNO) FROM Client_CKYC(NOLOCK) WHERE CK_Panno = CM.cm_panno)
	    SET @strAccountTyp = '03'
	  END
	  ...
	  mkrDt = CONVERT(VARCHAR,GETDATE(),112), CK_ActType = '01', 
	  Ck_RespType = '', Ck_AppType = @strAccountTyp,
      Ck_Filler1 = '', Ck_Filler2 = '', Ck_Filler3 = '', Ck_NFiller1 = @strCKYC_NO, Ck_Nfiller2 = 0, Ck_NFiller3  = 0
	...
	AND CK_SRNO IN(SELECT MAX(CK_SRNO) FROM Client_CKYC WHERE CK_Panno = CXXX.CK_Panno))
		...
		PersonalDetails = (
				SELECT ClientCode = ltrim(rtrim(Cm_cd)), 
					...
					FirstName = LTRIM(RTRIM(CASE WHEN CHARINDEX(' ', LTRIM(RTRIM(cm_name))) > 0 THEN LEFT(LTRIM(RTRIM(cm_name)), CHARINDEX(' ', LTRIM(RTRIM(cm_name))) - 1) 
					ELSE LTRIM(RTRIM(cm_name)) END)), 
					MiddleName = LTRIM(RTRIM(CASE WHEN LEN(LTRIM(RTRIM(cm_name))) - LEN(REPLACE(LTRIM(RTRIM(cm_name)), ' ', '')) = 2 
                    THEN SUBSTRING(LTRIM(RTRIM(cm_name)), CHARINDEX(' ', LTRIM(RTRIM(cm_name))) + 1, CHARINDEX(' ', LTRIM(RTRIM(cm_name)), CHARINDEX(' ', LTRIM(RTRIM(cm_name))) + 1) - 
                    CHARINDEX(' ', LTRIM(RTRIM(cm_name))) - 1) ELSE '' END)), 
					LastName = LTRIM(RTRIM(CASE WHEN CHARINDEX(' ', LTRIM(RTRIM(cm_name))) > 0 
                    THEN REVERSE(LEFT(REVERSE(LTRIM(RTRIM(cm_name))), CHARINDEX(' ', REVERSE(LTRIM(RTRIM(cm_name)))) - 1)) ELSE '' END)), 
					...
					CASE WHEN ISNULL((
					  ...
					  Gender = RTRIM(LTRIM(cm_sex)), [DateOfBirth] = cm_dob, [MaritalStatus] = RTRIM(LTRIM(cm_maritalstatus)), 
				...
				Constitution = (SELECT cc_descrip FROM Client_category(NOLOCK) WHERE CC_CD = cm_constitution),
				[KRAStatus] = CASE WHEN ISNULL((SELECT ExceptionMessage FROM tbl_GenericAPIDebugLog(NOLOCK) WHERE RequestSource = 'CVLKRA'
                AND RequestSource = @vcClientCode),'') = ''
				THEN ISNULL((select cn_filler5 from Client_Nominee where cn_cd = @vcClientCode),'')
				ELSE ISNULL((SELECT ExceptionMessage FROM tbl_GenericAPIDebugLog(NOLOCK) WHERE RequestSource = 'CVLKRA'
                AND RequestSource = @vcClientCode),'') END,
				[CKYCNo] = CASE WHEN ISNULL((SELECT ExceptionMessage FROM tbl_GenericAPIDebugLog(NOLOCK) WHERE RequestSource = 'CKYCACCNO'
                AND RequestSource = @vcClientCode),'') = ''
				THEN ISNULL((SELECT CK_Reference FROM CLIENT_CKYC X WHERE CK_PANNO = CM_PANNO AND ISNULL(CM_PANNO,'') <> ''
                AND CK_SRNO IN(SELECT MAX(CK_SRNO) FROM CLIENT_CKYC WHERE CK_PANNO = X.CK_PANNO AND ISNULL(CK_Reference,'') <> '')),'')
				ELSE ISNULL((SELECT ExceptionMessage FROM tbl_GenericAPIDebugLog(NOLOCK) WHERE RequestSource = 'CKYCACCNO'
                AND RequestSource = @vcClientCode),'') END,
				CkycRefNo = ISNULL((SELECT Ck_Nfiller1 FROM CLIENT_CKYC X WHERE CK_PANNO = CM_PANNO AND ISNULL(CM_PANNO,'') <> ''
                AND CK_SRNO IN(SELECT MAX(CK_SRNO) FROM CLIENT_CKYC WHERE CK_PANNO = X.CK_PANNO AND ISNULL(Ck_Nfiller1,'') <> '')),'')
				...
				SELECT NomSerial = cn_Srno, NomFirstName = (
					...
					NomPercentage = cn_NomPershare, NomineeDOB = RTRIM(LTRIM(cn_DOB)),
					...
					[GuardianDetails] =  (
						SELECT NomGuardianSerial = cn_Srno, NomGuardianFirstName = (
		                    ...
		                    WHEN ISNULL(cn_Relation, '') ='Not Provided' THEN '12'  
		                    WHEN ISNULL(cn_Relation, '') ='Others' THEN '13' ELSE  ISNULL(cn_Relation, '') END), 
							NomGuardianDOB = LTRIM(RTRIM(cn_DOB)), NomGuardianMobile = ltrim(rtrim(cn_Tel)), NomGuardianEmail = ltrim(rtrim(cn_Email)),
				...
				---- For Bank details
				[BankDetails] = (
				SELECT [BankAccNo] = ltrim(rtrim(ba_actno)), 
				...
				SELECT [Relationship] = ltrim(rtrim(cc_Cdesignation)), 
					[ContractName] = ltrim(rtrim(cc_Cname)), 
					[FatherSpouseName] = cc_FatherSpouseName,
					[ContractDateofBirth] = cc_DateofBirth,
					[ContractGender] = cc_Gender,
					[ContractMaritialStatus] = cc_MaritialStatus,
					[ContractDifferentlyAbledstatusFlag]  = cc_DifferentlyAbledstatusFlag,
					[ContractTypeofimpairment]  = cc_Typeofimpairment,
					[ContractPercentageofimpairment] = cc_Percentageofimpairment,
					[ContractAddress1] = RTRIM(LTRIM(cc_Caddress1)),
					[ContractAddress2] = cc_Address2,
					[ContractAddress3] = cc_Address3, [ContractAddressCity] = cc_AddressCity,
					[ContractAddressDistrict] = cc_AddressDistrict,
					[ContractPincode] = cc_AddressPINCode,
					[ContractAddressState] = cc_AddressState,
					[ContractCountry] = cc_AddressCountry,
					[ContractMobileNo] = LTRIM(RTRIM(cc_MobileNo)),
					[ContractEmailid] = LTRIM(RTRIM(cc_Cemail)),
					[ContractPanNo] = LTRIM(RTRIM(cc_Cpanno)),
					[ContractUIDNo] = ltrim(rtrim(cc_UID)),
					[ContractDINNo] = ltrim(rtrim(cc_DIN)),
					[ContractSerialNo] = cc_srno
				FROM Tbl_ClientRelatedPerson(NOLOCK)
				WHERE cc_cmcd = @vcClientCode 
				Order By cc_srno 
				FOR JSON PATH
				),
				...
				---- For Demat details
				[DematDetails] = (
				SELECT  [DematId] = (ROW_NUMBER() OVER(ORDER BY da_clientcd ASC) + 1000), [DPAcNo] = ltrim(rtrim(
				...
				[Attachments] = (SELECT  TOP 1 AddressAttachment = ISNULL((SELECT CASE WHEN CAST(ma_proof  AS VARCHAR(MAX)) <> ''
				...
				AND ma_field ='AddressAttachment'),''), 
				AddressProofType = ISNULL((SELECT  TOP 1 ma_filename FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode 
				AND ma_refno = @i_vcRefNo AND ma_field ='AddressAttachment'),''),
				IDAttachment = ISNULL((SELECT  TOP 1 CASE WHEN CAST(ma_proof  AS VARCHAR(MAX)) <> ''
                then CAST(ma_proof  AS VARCHAR(MAX)) else '' end FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode AND ma_refno = @i_vcRefNo 
				AND ma_field ='IDAttachment'),''), 
				IDProofType = ISNULL((SELECT  TOP 1 ma_filename FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode 
				AND ma_refno = @i_vcRefNo AND ma_field ='IDAttachment'),''),
				SegmentProofType ='',SegmentAttachment = '',
				IncomeAttachment = ISNULL((SELECT TOP 1 CASE WHEN CAST(ma_proof  AS VARCHAR(MAX)) <> ''
                then CAST(ma_proof  AS VARCHAR(MAX)) else '' end FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode AND ma_refno = @i_vcRefNo 
				AND ma_field ='IncomeAttachment' AND ma_proof IS NOT NULL),''), 
				IncomeProofType = ISNULL((SELECT  TOP 1  ma_filename FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode 
				AND ma_refno = @i_vcRefNo AND ma_field ='IncomeAttachment' AND ma_proof IS NOT NULL),''), 
				
				Nominee1ProofType = ISNULL((SELECT  TOP 1 ma_filename FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode AND ma_refno = @i_vcRefNo AND ma_field ='Nominee1Attachment'),''), 	
				
				Nominee1Attachment = ISNULL((SELECT TOP 1  CASE WHEN CAST(ma_proof  AS VARCHAR(MAX)) <> '' then CAST(ma_proof  AS VARCHAR(MAX)) else '' end
	            FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode AND ma_refno = @i_vcRefNo AND ma_field ='Nominee1Attachment'),''),
				
				
				Nominee2Attachment = ISNULL((SELECT  TOP 1 CASE WHEN CAST(ma_proof  AS VARCHAR(MAX)) <> ''
				...
				Nominee2ProofType = ISNULL((SELECT  TOP 1 ma_filename FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode AND ma_refno = @i_vcRefNo AND ma_field ='Nominee2Attachment'),''), 	
				
				Nominee3Attachment = ISNULL((SELECT  TOP 1 CASE WHEN CAST(ma_proof  AS VARCHAR(MAX)) <> ''
                then CAST(ma_proof  AS VARCHAR(MAX)) else '' end
	            FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode AND ma_refno = @i_vcRefNo AND ma_field ='Nominee3Attachment'),''),
				Nominee3ProofType = ISNULL((SELECT  TOP 1 ma_filename FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode AND ma_refno = @i_vcRefNo AND ma_field ='Nominee3Attachment'),''), 	
				
				SignatureAttachment= ISNULL((SELECT  TOP 1 CASE WHEN CAST(ma_proof  AS VARCHAR(MAX)) <> ''
                then CAST(ma_proof  AS VARCHAR(MAX)) else '' end
	            FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode AND ma_refno = @i_vcRefNo AND ma_field ='SignAttachment'),''),
				PhotoAttachment= ISNULL((SELECT  TOP 1 CASE WHEN CAST(ma_proof  AS VARCHAR(MAX)) <> ''
                then CAST(ma_proof  AS VARCHAR(MAX)) else '' end
	            FROM  Client_ModifyAttach(NOLOCK) WHERE ma_cmcd =  @vcClientCode AND ma_refno = @i_vcRefNo AND ma_field ='PhotoAttachment'),'')
        FOR JSON PATH),
...
@o_vcOutputJson NVARCHAR(MAX) OUTPUT, @strHeaderVal VARCHAR(MAX)='', @strTokenVal VARCHAR(MAX)='', @strLOGINUrl VARCHAR(MAX)='' 
WITH ENCRYPTION AS
BEGIN
  set @o_vcOutputJson = ''
  ...
  IF ISNULL(@strLOGINUrl,'') <> ''
  BEGIN
    SET @strAPIURL = @strLOGINUrl
  END
...
BEGIN
  ...
  DECLARE @strCMRAttachmentAttach NVARCHAR(MAX)
  ...
  */
  ...
  WHERE ma_filename='CMRAttachment' and ma_cmcd = @i_vcUserCode and ma_status in('N','Y') )
  BEGIN
    /*
    SELECT TOP 1 @strCMRAttachmentAttach = CAST(ma_proof AS NVARCHAR(MAX))
    ...
    */
	...
	/*SET @strLedgerJson = (SELECT [Attachment] = @strCMRAttachmentAttach FOR JSON PATH, ROOT('CMRAttachment'))
	TOP 1 [Attachment] = replace(CAST(CAST('' AS XML).value('xs:base64Binary(sql:variable("@strCMRAttachmentAttach"))', 'VARBINARY(MAX)'
      ) AS VARCHAR(MAX)),'data:application/pdf;base64,','')
	  FROM  Client_ModifyAttach(NOLOCK) 
  ...
  SET @strLedgerJson = ''
  IF EXISTS(SELECT 1 FROM Client_ModifyAttach(NOLOCK) 
  WHERE ma_filename='SignedPdf' and ma_cmcd = @i_vcUserCode AND  
  EXISTS(SELECT 1 FROM Client_ReKycMain(NOLOCK) WHERE RM_CMCD = @i_vcUserCode AND rm_RequestType <> 'REKYC') 
  and ma_status in('N','Y')  )
  BEGIN
    SET @strLedgerJson = (SELECT TOP 1 [Attachment] =
	case when isnull(dbo.fnBinaryToBase64(DBO.FN_base64toBinary(CAST(CONVERT(varchar(MAX), REPLACE(ma_proof,'data:image/jpeg;base64,','')) AS NVARCHAR(MAX)))),'') = ''
    then dbo.fnBinaryToBase64(ma_proof) else dbo.fnBinaryToBase64(DBO.FN_base64toBinary(CAST(CONVERT(varchar(MAX), REPLACE(ma_proof,'data:image/jpeg;base64,','')) AS NVARCHAR(MAX)))) end
	FROM  Client_ModifyAttach(NOLOCK) 
    WHERE ma_filename='SignedPdf' and ma_cmcd = @i_vcUserCode and ma_status in('N','Y')  
	and EXISTS(SELECT 1 FROM Client_ReKycMain(NOLOCK) WHERE RM_CMCD = @i_vcUserCode AND rm_RequestType <> 'REKYC') 
	ORDER BY ma_srno	DESC
...
BEGIN
  ...
  SELECT X.ca_cmcd,  X.ca_Nfiller3, ca_filler1 as JsonTag, X1.JsonKey, 
  CASE WHEN x1.FieldName like '%dob' AND ISDATE(ca_oldValue) > 0 THEN Replace(CONVERT(VARCHAR,CAST(ca_oldValue AS DATE),103),'01/01/1900','')
  ELSE ca_oldValue END, CASE WHEN x1.FieldName like '%dob' AND ISDATE(ca_newValue) > 0 THEN Replace(CONVERT(VARCHAR,CAST(ca_newValue AS DATE),103),'01/01/1900','')
	...
	BEGIN
	  ...
	  BEGIN
        ...
        BEGIN
		  ...
		  EXEC stpr_CVLKRAFileGen @vcXML , @o_vcErrorFlag OUTPUT, @o_vcErrorMessage OUTPUT
...
BEGIN
  ...
  INSERT INTO @tbl_OutputResponse (Json_Table) EXEC sp_OAMethod @Object, 'responseText'
  SELECT @VCOUTPUT = Json_Table FROM @tbl_OutputResponse
 --SELECT @VCOUTPUT, @CallingAPIURLMain, @i_vcInputJson1
  SET @o_vcOutputJson = @VCOUTPUT  
  
  INSERT INTO tbl_GenericAPIDebugLog(ParentSerialNo, RequestSource, RequestUniqueID, RequestString, ResponseString, ExceptionMessage, UpdateBy, UpdateTimeStamp)
  VALUES(0, 'CKYC'+' '+@i_vcType, 0, ISNULL(@i_vcInputJson1,''), ISNULL(@o_vcOutputJson,''), '','SA', getdate())
  EXEC sp_OADestroy @Object  
  RETURN 
END
GO
...
CREATE FUNCTION fn_GetFileExtension (@strFileBase64 NVARCHAR(MAX)) returns VARCHAR(50)
as
BEGIN
  DECLARE @vStrBinaryData VARBINARY(MAX);
  declare @o_vcFileExtension varchar(50)=''
  SET @vStrBinaryData = CAST(@strFileBase64 AS XML).value('.','VARBINARY(MAX)');

  DECLARE @StrFileExtension NVARCHAR(10);
  IF SUBSTRING(@vStrBinaryData, 1, 2) = 0xFFD8
  BEGIN
    SET @StrFileExtension = 'jpg';
  END
  ELSE IF SUBSTRING(@vStrBinaryData, 1, 2) = 0x8950
  BEGIN
    SET @StrFileExtension = 'png';
  END
  ELSE IF SUBSTRING(@vStrBinaryData, 1, 4) = 0x47494638
  BEGIN
    SET @StrFileExtension = 'gif';
  END
  ELSE IF SUBSTRING(@vStrBinaryData, 1, 4) = 0x25504446
  BEGIN
    SET @StrFileExtension = 'pdf';
  END
  ELSE
  BEGIN
    SET @StrFileExtension = 'png';
  END
  SET @o_vcFileExtension = @StrFileExtension
  RETURN @o_vcFileExtension
END
GO
...
BEGIN
  ...
  @o_vcOutputJsonapi VARCHAR(MAX)='', @strKRAAPIPassword NVARCHAR(MAX)='', @strurl VARCHAR(MAX)='', @StrHeaderString VARCHAR(MAX)='',
  @strPANNO VARCHAR(20)='', @strFormNo VARCHAR(10), @strRequestType VARCHAR(50)=''
  BEGIN TRY
    SELECT @strClient =  ISNULL(x.value('(ClientCode)[1]', 'VARCHAR(MAX)'),''),
	@strFormNo = ISNULL(x.value('(FormNo)[1]', 'INT'),0)
    FROM @XMLData.nodes('/root') AS XTbl(x) 
  END TRY
  BEGIN CATCH
    SET @o_vcErrorFlag = 'E'
 ...
 SET @o_vcErrorMessage = 'Process Completed'
  ...
  IF ISNULL(@strFormNo,0) = 0
  begin
    SELECT @strFormNo = MAX(ma_refno) FROM Client_ModifyAttach(NOLOCK) WHERE ma_cmcd = @strClient
  END
  ...
  SELECT @strRequestType = rm_RequestType FROM Client_ReKycMain(NOLOCK) 
  WHERE rm_cmcd = @strClient AND rm_refno = @strFormNo
    
  SELECT @strThirdPartyURL = sp_sysvalue 
  FROM Sysparameter(NOLOCK) WHERE sp_parmcd = 'ThirdPRekyc'
  ...
  SET @strFolderName = @strCKYCFINO+'_'+@strCKYCREGIONCD+'_'+REPLACE(CONVERT(VARCHAR,GETDATE(),105),'-','')+'_'+'V1.3'+'_'+@strCKYCUSERID+'_U'+RIGHT(REPLICATE('0', 5) + cast(@strBatchNO AS VARCHAR), 5)
  SET @StrTXTFileName = @strCKYCFINO+'_'+@strCKYCREGIONCD+'_'+REPLACE(CONVERT(VARCHAR,GETDATE(),105),'-','')+'_'+'V1.3'+'_'+@strCKYCUSERID+'_U'+RIGHT(REPLICATE('0', 5) + cast(@strBatchNO AS VARCHAR), 5)+'.txt'
  ...
  DECLARE @cx_Status VARCHAR(1)='', @cx_Reason VARCHAR(200)='', @cx_srno INT = 0,
	...
	@str30String VARCHAR(MAX)='', @str70String VARCHAR(MAX)='', @strcm_constitution VARCHAR(10)='',
	@strconstitution VARCHAR(10)='', @strEntityName VARCHAR(135)='', @cm_residentialstatus VARCHAR(50)='',
	@cm_regplace VARCHAR(100), @cm_regdate VARCHAR(20)='', @str40String VARCHAR(MAX)='', @cm_regno VARCHAR(50)='',
	@XXcx_dob VARCHAR(20)='', @ckycrfeno VARCHAR(50)=''
  ...
  DECLARE @strCount111 INT = 0, @strConstitutionType VARCHAR(2)=''
  
  SELECT @strCount111 = COUNT(DISTINCT cm_constitution) FROM CLIENT_INFO(NOLOCK)
  WHERE CM2_CD IN(SELECT ClientCode FROM @tbl_Client)
  
  IF @strCount111 > 1
  BEGIN
    SET @o_vcErrorFlag = 'E'
    SET @o_vcErrorMessage = 'Please Select Only Single Constitution Type Client'
	RETURN 1
  END
  
  SELECT DISTINCT @strConstitutionType =  cm_constitution FROM CLIENT_INFO(NOLOCK)
  WHERE CM2_CD IN(SELECT ClientCode FROM @tbl_Client)
  
  DECLARE @strIDAttCount INT = 0, @strIDAttCount1 INT=0, @COUNTNO40 INT
  ...
  SET @strHeaderString = '10|'+RIGHT(REPLICATE('0', 5) + cast(@strBatchNO AS VARCHAR), 5)+'|'+@strCKYCFINO+'|'+@strCKYCREGIONCD+'|'+CAST(@strtotalClientCount AS VARCHAR)+'|'+CONVERT(VARCHAR,GETDATE(),105)
  IF @strConstitutionType = 1
  BEGIN
    SET @strHeaderString = @strHeaderString +'|V1.3|01||||'
  END	
  ELSE
  BEGIN
    SET @strHeaderString = @strHeaderString +'|V1.3|02||||'
  END	
        
  DECLARE @StrClientCode VARCHAR(50)='', @strCounter INT = 0
  
  DECLARE @tbl_IDDocument TABLE(Client VARCHAR(50), FileName VARCHAR(50), IdProof VARCHAR(20), 
  AddressProof VARCHAR(20), DocumentProof VARCHAR(20), DocumentNo VARCHAR(50),
  Document VARCHAR(MAX), FileType VARCHAR(50), DocumentType VARCHAR(50))
  ...
  BEGIN  
	...
	IF EXISTS(sELECT 1 FROM Client_ModifyAPI(NOLOCK) WHERE ca_cmcd = @StrClientCode AND ca_Nfiller3 = @strFormNo AND ca_desc = 'IPV NAME' AND ca_newValue <> '')
    BEGIN
	  SET @um_user_name = ''
	  SET @um_designation = ''
      SELECT @um_user_name = ca_newValue FROM Client_ModifyAPI(NOLOCK) WHERE ca_cmcd = @StrClientCode AND ca_Nfiller3 = @strFormNo 
	  AND ca_desc = 'IPV NAME' AND ca_newValue <> ''
	  SELECT @um_designation = ca_newValue FROM Client_ModifyAPI(NOLOCK) WHERE ca_cmcd = @StrClientCode AND ca_Nfiller3 = @strFormNo 
	  AND ca_desc = 'IPV_DESG' AND ca_newValue <> ''
    END
	
    SELECT @cx_Status = 'Y' , @cx_Reason = '', @cx_srno = ck_srno, @cx_mrkdt = CONVERT(VARCHAR,CAST(a.mkrdt AS DATE),105), 
    @cx_Panno = A.CK_Panno , @cx_sex = cm_sex , @cm_name = RTRIM(LTRIM(cm_name)), 
	@cx_name = RTRIM(LTRIM(cm_name)), @cx_FatherFname = CK_FatherFname , @cx_FatherMname = CK_FatherMname , 
	...
	@cx_dob = CONVERT(VARCHAR,CAST(cm_dob AS DATE),105), 
	@XXcx_dob = cm_dob,
	@cx_nationalcode = cm_nationalcode , 
    ...
    @Cx_AppType = Ck_AppType, @CK_ActType = ISNULL(CK_ActType,''), @cx_Reference = isNull(CK_Reference, ''),
	...
	@strcm_constitution = cm_constitution, @cm_residentialstatus = CASE WHEN cm_residentialstatus = 'I' THEN '01'
	WHEN cm_residentialstatus = 'N' THEN '02'
	WHEN cm_residentialstatus = 'F' THEN '03' ELSE '' END,
	@cm_regplace = cm_regplace,
	@cm_regdate = cm_regdate, @cm_regno = cm_regno, @ckycrfeno = ISNULL(Ck_Nfiller1,0)
	...
	BEGIN
	  ...
	  BEGIN
		...
		SET @str40String = @str40String+'\r\n' 
	    --SET @str70String = @str70String+'\r\n' 
	  END
	  SET @strconstitution = ''
      
	  SELECT @strconstitution = CS_DataTypeCd FROM CKYC_StaticCodes(NOLOCK) WHERE CS_DataType = 'CKYCConstitutionType'
      AND cs_code = @strcm_constitution
  
	  if ISNULL(@strconstitution,'') = ''
	  BEGIN
	    SET @o_vcErrorFlag = 'E'
        SET @o_vcErrorMessage = 'Constitution Type is Not Mapped For Constitution - '+@strcm_constitution
		CLOSE Cur0
        DEALLOCATE Cur0
	    RETURN 1
	  END
      
      IF CAST(DATEDIFF(YEAR,CAST(@XXcx_dob AS DATE),GETDATE()) AS INT) < 18 AND ISNULL(@XXcx_dob,'') <> ''
      BEGIN		
		SET @CK_ActType = '05'
	  END	
	  
	  SET @cx_Reference = @ckycrfeno
	  
	  /*IF ISNULL(@ckycrfeno,'') = ''
	  BEGIN
	    SELECT TOP 1 @ckycrfeno = Ck_Nfiller1 FROM Client_CKYC(NOLOCK), 
	    CLIENT_MASTER(NOLOCK) CM  WHERE CM.CM_CD = @StrClientCode AND CK_Panno = CM.cm_panno AND Ck_Nfiller1 <> ''
	    AND CK_SRNO IN(SELECT MAX(CK_SRNO) FROM Client_CKYC(NOLOCK) WHERE CK_Panno = CM.cm_panno)
	  END
	  ...
	  */
	  ...
	  DECLARE @PersonalEntityUpdateFlag VARCHAR(2)='02', @AddressDetailsUpdateFlag VARCHAR(2)='', 
	  @ContactDetailsUpdateFlag VARCHAR(2)='', @IdentityDetailsUpdateFlag VARCHAR(2)='',
	  @RelatedPersonUpdateFlag VARCHAR(2)='', @ImageUpdateFlag VARCHAR(2)='01'
	  
	  IF EXISTS(SELECT 1 FROM Client_ModifyAPI WHERE ca_cmcd = @StrClientCode and
	  ca_oldValue <> ca_newValue and ca_filler1 = 'RelatedPersonDetails' and ca_Nfiller3 = @strFormNo)
	  BEGIN
	    SET @RelatedPersonUpdateFlag = '01'
	  END
	  ELSE
	  BEGIN
	    SET @RelatedPersonUpdateFlag = '02'
	  END
	  ...
	  IF EXISTS(SELECT 1 FROM Client_ModifyAPI WHERE ca_cmcd = @StrClientCode and ca_Nfiller3 = @strFormNo 
	  and ca_oldValue <> ca_newValue and ca_filler1 = 'PersonalDetails'
      AND ca_desc IN('Mobile','EMAIL'))
	  BEGIN
	    SET @ContactDetailsUpdateFlag = '01'
	  END
	  ELSE
	  BEGIN
	    SET @ContactDetailsUpdateFlag = '02'
	  END
	  
	  IF EXISTS(SELECT 1 FROM Client_ModifyAPI WHERE ca_cmcd = @StrClientCode and ca_Nfiller3 = @strFormNo 
	  and ca_oldValue <> ca_newValue and ca_filler1 = 'PersonalDetails'
      AND ca_desc like  'Address%')
	  BEGIN
	    SET @AddressDetailsUpdateFlag = '01'
	  END
	  ELSE
	  BEGIN
	    SET @AddressDetailsUpdateFlag = '02'
	  END
	  
	  IF ISNULL(@cx_Reference,'') = ''
	  BEGIN
	     SET @cx_Reference = @StrClientCode
	  END
	  
	  IF @Cx_AppType = '03'			   
	  BEGIN
	    SET @str20String = @str20String+'02'+'|'+@PersonalEntityUpdateFlag+'|'+@AddressDetailsUpdateFlag+'|'+@ContactDetailsUpdateFlag
		    +'|'+'02'+'|'+'01'+'|'+'01'+'|'+@RelatedPersonUpdateFlag+'|'+'02'+'|'+'01'+'|'+LTRIM(RTRIM(@strconstitution))+'|||'+LTRIM(RTRIM(@CK_ActType))+'|'+RTRIM(LTRIM(ISNULL(@cx_Reference,'')))+'|'
 	  END
	  ELSE				   
	  BEGIN
	    SET @cx_Reference = @StrClientCode
	    SET @str20String = @str20String+'||||||||||'+LTRIM(RTRIM(@strconstitution))+'|||'+LTRIM(RTRIM(@CK_ActType))+'|'+RTRIM(LTRIM(CAST(ISNULL(@cx_Reference,'') AS VARCHAR)))+'|'
	  END
	  
	  IF @strcm_constitution <> '1'
	  BEGIN
	    SET @strEntityName = @cx_name
	  END
	  
	  SET @str20String = @str20String+LTRIM(RTRIM(REPLACE(@cx_prefix,'\\','\\')))+'|'+LTRIM(RTRIM(@CK_Fname))+'|'+LTRIM(RTRIM(@CK_Middlename))+'|'+LTRIM(RTRIM(@CK_Lname))+'|'+@strEntityName+'||||||'
      ...
      SET @str20String = @str20String+'|'+@cm_regplace+'|'+CONVERT(VARCHAR,CAST(@cm_regdate AS DATE),105)+'|'+@cx_pcountry+'|||||'+LTRIM(RTRIM(@cx_Panno))+'|'+@cm_residentialstatus+'|'+'0'+'||||||'+@cx_padd1+'|'+@cx_padd2+'|'+@cx_padd3+'|'+@cx_padd4+'|'
	  ...
	  DELETE FROM @tbl_IDDocument

	  IF @strRequestType <> 'REKYC'
	  BEGIN
	    INSERT INTO @tbl_IDDocument(Client, FileName, IdProof, AddressProof, DocumentProof, DocumentNo, Document, FileType, DocumentType)
	    SELECT LTRIM(RTRIM(ma_cmcd)), 
		LTRIM(RTRIM(@cx_Reference))+'_'+REPLACE(replace(substring(CASE WHEN isnull(DocumentName,'') = '' THEN ma_filename ELSE DocumentName END  ,1,15),' ',''),'/','')+'_'+
		CAST(ma_srno AS VARCHAR),
	    ISNULL(CKycIDCode,'Z') as IDProof, ISNULL(CKycAddressCode,'99') as AddressProof, ISNULL(CKYCDocCode,'98') As DocumentCode,  DocNo = (CASE WHEN DocumentName LIKE '%PAN%' THEN 
        ISNULL((SELECT CM_PANNO FROM CLIENT_MASTER(NOLOCK) WHERE CM_cD = XX.ma_cmcd), XX.DocumentNo)
        WHEN DocumentName LIKE '%Aadhaar%' 
		THEN CASE WHEN ISNULL((SELECT CM_uid FROM CLIENT_info(NOLOCK) WHERE CM2_cD = XX.ma_cmcd),XX.DocumentNo) ='' THEN  XX.DocumentNo ELSE 
		ISNULL((SELECT CM_uid FROM CLIENT_info(NOLOCK) WHERE CM2_cD = XX.ma_cmcd),XX.DocumentNo) END
		else XX.DocumentNo end),
	    REPLACE(CAST(ma_proof AS VARCHAR(MAX)),'data:image/jpeg;base64,','') AS DO1,
	    dbo.fn_GetFileExtension( REPLACE(CAST(ma_proof AS VARCHAR(MAX)),'data:image/jpeg;base64,','')) as filetype, ProofType
		FROM Client_ModifyAttach(NOLOCK) xx LEFT OUTER JOIN tbl_DocumentKRACKYCMapping(NOLOCK) 
		on(CASE WHEN ma_field  IN('AadhaarAttachment','PANAttachment') THEN 'DocumentAttachment' 
	    ELSE ma_field END = ProofType and CASE WHEN ma_filename IN('Sign') then  '50'
		WHEN ma_filename IN('Photo')  then  '51' WHEN ma_filename LIKE '%Aadhaar%' THEN '28' else ma_filename end = DocumentCode)
		WHERE  ma_cmcd = @StrClientCode
        AND ma_refno = @strFormNo
	    AND ma_proof IS NOT NULL
	  END
	  ELSE 
	  BEGIN
	    INSERT INTO @tbl_IDDocument(Client, FileName, IdProof, AddressProof, DocumentProof, DocumentNo, Document, FileType)
		 SELECT LTRIM(RTRIM(ma_cmcd)), LTRIM(RTRIM(@cx_Reference))+'_'+
		 REPLACE(replace(substring(CASE WHEN isnull(CS_Description,CS_DataType) = '' THEN CS_DataType ELSE CS_Description END  ,1,15),' ',''),'/','')+'_'+
		 CAST(ma_srno AS VARCHAR),
	    CS_DocTypeCode as IDProof, CS_KRACode as AddressProof, CS_DataTypeCd As DocumentCode,  
		DocNo = (CASE WHEN CS_Description LIKE '%PAN%' THEN 
        ISNULL((SELECT CM_PANNO FROM CLIENT_MASTER(NOLOCK) WHERE CM_cD = XX.ma_cmcd),XX.DocumentNo)
        WHEN CS_Description LIKE '%Aadhaar%' THEN ISNULL((SELECT CM_uid FROM CLIENT_info(NOLOCK) WHERE CM2_cD = XX.ma_cmcd),XX.DocumentNo) 
		else XX.DocumentNo end),
	    REPLACE(CAST(ma_proof AS VARCHAR(MAX)),'data:image/jpeg;base64,','') AS DO1,
		dbo.fn_GetFileExtension( REPLACE(CAST(ma_proof AS VARCHAR(MAX)),'data:image/jpeg;base64,','')) as filetype
        FROM Client_ModifyAttach(NOLOCK) XX LEFT OUTER JOIN CKYC_StaticCodes ON
		((ma_field = CS_DataType AND (( CASE WHEN ma_filename = 'Sign' THEN '09'
	    WHEN ma_filename = 'Photo' THEN '02' else '' end = CS_DataTypeCd) and ma_field <> 'AadhaarAttachment') 
		or (CS_Description LIKE '%Proof of Possession of Aadhaar%' and  ma_field = 'AadhaarAttachment')))
        WHERE ma_cmcd = @StrClientCode
        AND ma_refno IN(SELECT MAX(ma_refno) FROM Client_ModifyAttach(NOLOCK) WHERE ma_cmcd = @StrClientCode)
	    AND ma_proof IS NOT NULL
		and ma_field in('PhotoAttachment','AadhaarAttachment','SignAttachment' )
	  END
	  
	  IF @strRequestType <> 'REKYC'
	  BEGIN
	    SELECT TOP 1 @cx_AddrProof = AddressProof FROM @tbl_IDDocument WHERE ISNULL(AddressProof,'') <> '' AND DocumentType = 'AddressAttachment'
		IF ISNULL(@cx_AddrProof,'') = ''
		BEGIN
		  SELECT TOP 1 @cx_AddrProof = AddressProof FROM @tbl_IDDocument WHERE ISNULL(AddressProof,'') <> ''
		END
	  END
	  ELSE
	  BEGIN
	  ...
	  BEGIN
	    SET @str20String = @str20String+LTRIM(RTRIM(@cx_std))+'|'+RTRIM(LTRIM(@cx_tele2))+'|'+LTRIM(RTRIM(@cx_std))+'|'+RTRIM(LTRIM(@cx_tele2))+'|'
	  END
	  ELSE  
	  BEGIN
	    SET @str20String = @str20String+'||||'
	  ...
	  IF @strRequestType <> 'REKYC'
	  BEGIN
	    SELECT @strIDAttCount = COUNT(*) FROM @tbl_IDDocument WHERE ISNULL(DocumentNo,'') <> ''
	    AND ISNULL(IdProof,'') <> '' AND DocumentType = 'IDAttachment'
		IF ISNULL(@strIDAttCount,0) = 0
		BEGIN
		  SELECT @strIDAttCount = COUNT(*) FROM @tbl_IDDocument WHERE ISNULL(DocumentNo,'') <> ''
	      AND ISNULL(IdProof,'') <> ''
		END
	  ...
	  SELECT @strIDAttCount1 = COUNT(*) FROM @tbl_IDDocument  
      
	  select @COUNTNO40 = count(*) from Tbl_ClientRelatedPerson(NOLOCK) WHERE cc_cmcd = @StrClientCode
	   
	  IF ISNULL(@strIDAttCount1,0) = 0
	  BEGIN
	    SET @strIDAttCount1 = 0
      END	  

	  SET @str20String = @str20String+'|'+CONVERT(VARCHAR,GETDATE(),105)+'|HeadOffice|'+CONVERT(VARCHAR,GETDATE(),105)+'|01|'+LTRIM(RTRIM(ISNULL(@um_user_name,'')))+'|'
	  SET @str20String = @str20String+ISNULL(@um_designation,'')+'|'+LTRIM(RTRIM(ISNULL(@cx_VerifyBy,'')))+'|'+ISNULL(@um_empCode,'')+'|'+@strCKYCCOMPNAME+'|'+@strCKYCFINO+'|'+CAST(@strIDAttCount AS VARCHAR)+'|'
	  SET @str20String = @str20String+CAST(@COUNTNO40 AS VARCHAR)+'|01||'+CAST(@strIDAttCount1 AS VARCHAR)+'|||||'+RTRIM(LTRIM(CAST(ISNULL(@cx_Reference,'') AS VARCHAR)))+'|'
	  	  
	  
	  IF @strRequestType <> 'REKYC'
	  BEGIN
	    IF EXISTS(SELECT 1 FROM @tbl_IDDocument
	    WHERE ISNULL(DocumentNo,'') <> ''
	    AND ISNULL(IdProof,'') <> '' 
	    AND DocumentType = 'IDAttachment')
		BEGIN
	      SELECT @str30String = @str30String +'\r\n'+'30|'+CAST(@strCounter AS VARCHAR)+'|'+ISNULL(RTRIM(LTRIM(IdProof)),'')+'|'+ISNULL(RTRIM(LTRIM(DocumentNo)),'')+'||||02|||||'
	      FROM @tbl_IDDocument
	      WHERE ISNULL(DocumentNo,'') <> ''
	      AND ISNULL(IdProof,'') <> '' 
	      AND DocumentType = 'IDAttachment'
	  ...
	  --IF ISNULL(@strcm_constitution,'') <> 1
	  --BEGIN
	  
	    SELECT @str40String= @str40String +'\r\n'+'40|'+CAST(@strCounter AS VARCHAR)+'|'+TypeofRelationship+'|'+RelatedPerson+'||'+RelatedPersonNamePrefix+'|'+
		FirstName+'|'+MiddleName+'|'+LastName+'|||||'+'01'+'|'+
		FatherSpouseNamePrefix+'|'+FatherSpouseFirstName+'|'+FatherSpouseMiddleName+'|'+FatherSpouseLastName+'|||||'+
		cc_DateofBirth+'|'+cc_Gender+'|'+cc_MaritialStatus+'|'+NATIONALITY+'|'+ResidentialStatus+'|'+DifferentlyAbledstatusFlag+'|'+
		Typeofimpairment+'|'+CAST(Percentageofimpairment AS VARCHAR)+'|'+UDIDnumber+'|||'+PANNO+'|'+AadhaarNo+'|||||||||'+
		PhotoFileName+'|'+IDProofFileName+'|'+AddressProofFileName+'|'+cc_Caddress1+'|'+cc_Address2+'|'+cc_address3+'|'+
		cc_AddressCity+'|'+cc_AddressDistrict+'|'+cc_AddressState+'|'+cc_AddressCountry+'|'+cc_AddressPINCode+'|'+
		AddressProofType+'|'+AddressSameas+'||||||||||||||+91|'+cc_MobileNo+'|||'+cc_Cemail+'|'+CONVERT(VARCHAR,GETDATE(),105)+'|'+cc_AddressCity+'|'+
		CONVERT(VARCHAR,GETDATE(),105)+'|01|'+LTRIM(RTRIM(ISNULL(@um_user_name,'')))+'|'+ISNULL(@um_designation,'')+'|'+LTRIM(RTRIM(ISNULL(@cx_VerifyBy,'')))+'|'+
		ISNULL(@um_empCode,'')+'|'+@strCKYCCOMPNAME+'|'+@strCKYCFINO+'|'+cc_DIN+'||||||'
		FROM(
		SELECT TypeofRelationship = ISNULL((SELECT CAST(CS_DataTypeCd AS VARCHAR) FROM CKYC_StaticCodes 
	    WHERE CS_DataType='CKYCRelatedPerson' AND CS_Description = cc_Cdesignation),''),
        RelatedPerson = '01',KYCNumber = '',RelatedPersonNamePrefix ='Mr.',  
	    FirstName = ltrim(rtrim((CASE WHEN CHARINDEX(' ', cc_Cname) > 0 THEN SUBSTRING(cc_Cname, 1, CHARINDEX(' ', cc_Cname) - 1) 
	    ELSE cc_Cname END))), 
	    MiddleName = ltrim(rtrim((CASE WHEN CHARINDEX(' ', cc_Cname) = 0 THEN '' ELSE RTRIM(LTRIM(REPLACE(REPLACE(cc_Cname, 
	    SUBSTRING(cc_Cname,1, CHARINDEX(' ', cc_Cname) - 1), ''), REVERSE(LEFT(REVERSE(cc_Cname), 
	    CHARINDEX(' ', REVERSE(cc_Cname)) - 1)), ''))) END))), 
	    LastName = ltrim(rtrim((CASE WHEN CHARINDEX(' ', cc_Cname) > 0 THEN REVERSE(LEFT(REVERSE(cc_Cname), 
	    CHARINDEX(' ', REVERSE(cc_Cname)) - 1)) ELSE '' END))),
	    FatherSpouseNamePrefix ='Mr.', FatherSpouseFirstName = ltrim(rtrim((CASE WHEN CHARINDEX(' ', cc_FatherSpouseName) > 0 
	    THEN SUBSTRING(cc_FatherSpouseName, 1, CHARINDEX(' ', cc_FatherSpouseName) - 1) ELSE cc_FatherSpouseName END))), 
	    FatherSpouseMiddleName = ltrim(rtrim((CASE WHEN CHARINDEX(' ', cc_FatherSpouseName) = 0 THEN '' ELSE 
	    RTRIM(LTRIM(REPLACE(REPLACE(cc_FatherSpouseName, SUBSTRING(cc_FatherSpouseName, 1, CHARINDEX(' ', cc_FatherSpouseName) - 1), ''), 
	    REVERSE(LEFT(REVERSE(cc_FatherSpouseName), CHARINDEX(' ', REVERSE(cc_FatherSpouseName)) - 1)), ''))) END))), 
	    FatherSpouseLastName = ltrim(rtrim((CASE WHEN CHARINDEX(' ', cc_FatherSpouseName) > 0 THEN REVERSE(LEFT(REVERSE(cc_FatherSpouseName), 
	    CHARINDEX(' ', REVERSE(cc_FatherSpouseName)) - 1)) ELSE '' END))), 
	    cc_DateofBirth = convert(VARCHAR,cc_DateofBirth,105), cc_Gender,  cc_MaritialStatus, NATIONALITY ='IN',
        ResidentialStatus ='01',  DifferentlyAbledstatusFlag = CAST(case when cc_DifferentlyAbledstatusFlag = 'Y' THEN 1 ELSE 0 END AS VARCHAR),
        Typeofimpairment = cc_Typeofimpairment, Percentageofimpairment = cc_Percentageofimpairment,
        UDIDnumber = REPLICATE('X', LEN(cc_UID)-4) + RIGHT(cc_UID, 4), PANNO=cc_Cpanno, 
        AadhaarNo = REPLICATE('X', LEN(cc_UID)-4) + RIGHT(cc_UID, 4), 
        PhotoFileName = LTRIM(RTRIM(cc_cmcd))+'_'+cc_Cpanno+'_'+'Photo.'+dbo.fn_GetFileExtension(CAST(PhotoImage AS VARCHAR(MAX))),
        IDProofFileName = LTRIM(RTRIM(cc_cmcd))+'_'+cc_Cpanno+'_'+'Id.'+dbo.fn_GetFileExtension(CAST(IDDocumentImage AS VARCHAR(MAX))),
        AddressProofFileName = LTRIM(RTRIM(cc_cmcd))+'_'+cc_Cpanno+'_'+'Address.'+dbo.fn_GetFileExtension(CAST(AddressDocumentImage AS VARCHAR(MAX))),
        cc_Caddress1, cc_Address2, cc_Address3, cc_AddressCity, cc_AddressDistrict , cc_AddressPINCode,
        cc_AddressState = isnull((select CAST(cs_code AS VARCHAR) from CKYC_StaticCodes(NOLOCK) WHERE CS_DataType='State' and CS_Description = cc_AddressState),''), 
        cc_AddressCountry = isnull((select CAST(cs_code AS VARCHAR) from CKYC_StaticCodes(NOLOCK) WHERE CS_DataType='Country' and CS_Description = cc_AddressCountry),''),
        AddressProofType ='01', AddressSameas = 'Y', cc_MobileNo, cc_Cemail, cc_DIN  
        FROM Tbl_ClientRelatedPerson(NOLOCK) WHERE cc_cmcd = @StrClientCode) X123
     -- END

	  SELECT @str70String =@str70String+'\r\n'+'70|'+CAST(@strCounter AS VARCHAR)+'|'+FileName+'.'+filetype+'|'+
	  LTRIM(RTRIM(DocumentProof))+'||||||'
	  FROM @tbl_IDDocument
   END 	
   FETCH NEXT FROM Cur0 INTO @StrClientCode
  END 
  ...
  SET  @StrRequestJson  = REPLACE(@StrRequestJson,'##baseFileString##',@strHeaderString+'\r\n'+@str20String+@str30String+@str40String+@str70String)
    
  DECLARE @subFileDetails NVARCHAR(MAX)='' 
  SET @subFileDetails=(
  SELECT * FROM (
  SELECT [subFolderName] = CAST(@cx_Reference AS VARCHAR), [fileName] =  fileName+'.'+FileType,
  [fileBase64] =  Document
  FROM @tbl_IDDocument
  UNION ALL
  SELECT [subFolderName] = CAST(LTRIM(RTRIM(@cx_Reference)) AS VARCHAR), [fileName] =  LTRIM(RTRIM(@cx_Reference))+'_'+cc_Cpanno+'_'+'Photo.'+dbo.fn_GetFileExtension(CAST(PhotoImage AS VARCHAR(MAX))),
  [fileBase64] =  PhotoImage
  FROM Tbl_ClientRelatedPerson(NOLOCK) WHERE cc_cmcd = @StrClientCode AND ISNULL(PhotoImage,'') <> '' 
  UNION ALL
  SELECT [subFolderName] = CAST(LTRIM(RTRIM(@cx_Reference)) AS VARCHAR), [fileName] =  LTRIM(RTRIM(@cx_Reference))+'_'+cc_Cpanno+'_'+'Id.'+dbo.fn_GetFileExtension(CAST(IDDocumentImage AS VARCHAR(MAX))),
  [fileBase64] =  IDDocumentImage
  FROM Tbl_ClientRelatedPerson(NOLOCK) WHERE cc_cmcd = @StrClientCode AND ISNULL(IDDocumentImage,'') <> '' 
  UNION ALL
  SELECT [subFolderName] = CAST(LTRIM(RTRIM(@cx_Reference)) AS VARCHAR), [fileName] =  LTRIM(RTRIM(@cx_Reference))+'_'+cc_Cpanno+'_'+'Address.'+dbo.fn_GetFileExtension(CAST(AddressDocumentImage AS VARCHAR(MAX))),
  [fileBase64] =  AddressDocumentImage
  FROM Tbl_ClientRelatedPerson(NOLOCK) WHERE cc_cmcd = @StrClientCode AND ISNULL(AddressDocumentImage,'') <> '' ) X123
  FOR JSON PATH)
...
CREATE PROCEDURE stpr_CVLKRAFileGen @vcXML NVARCHAR(MAX), @o_vcErrorFlag VARCHAR(1) OUTPUT, @o_vcErrorMessage VARCHAR(MAX) OUTPUT WITH ENCRYPTION AS
BEGIN
  --SET @vcXML = '<root>'+@vcXML+'</root>' 
  DECLARE @XMLData XML = CAST(@vcXML AS XML), @strClient VARCHAR(20)='', @strThirdPartyURL VARCHAR(200)='',
  @o_vcOutputJsonapi VARCHAR(MAX)='', @strKRAAPIPassword NVARCHAR(MAX)='', @strurl VARCHAR(MAX)='', @StrHeaderString VARCHAR(MAX)='',
  @strRefNo VARCHAR(10)='', @strFormNo INT, @strLogSerialNo INT
  BEGIN TRY
    SELECT @strClient =  ISNULL(x.value('(ClientCode)[1]', 'VARCHAR(500)'),''),
	@strRefNo =  ISNULL(x.value('(RefNo)[1]', 'VARCHAR(500)'),''),
	@strFormNo = ISNULL(x.value('(FormNo)[1]', 'INT'),'')
    FROM @XMLData.nodes('/root') AS XTbl(x) 
  END TRY
  BEGIN CATCH
    SET @o_vcErrorFlag = 'E'
  ...
  BEGIN
    ...
    BEGIN
	 SET @o_vcErrorFlag = 'E'
	 SET @o_vcErrorMessage = 'Token Not Found '+@o_vcOutputJsonapi
    END
  END
  ...
  BEGIN
	...
	INSERT INTO tbl_GenericAPIDebugLog(ParentSerialNo, RequestSource, RequestUniqueID, RequestString,
	ResponseString, ExceptionMessage, UpdateBy, UpdateTimeStamp)
  ...
  @cx_maritalstatus VARCHAR(20)='', @cx_networth VARCHAR(50)='', @cx_networthdt VARCHAR(8)='', @cx_VerifyBy VARCHAR(50)='',
  @cx_IdentityProofcd VARCHAR(20)='', @cx_AddrProofCd VARCHAR(20)='', @cx_PermAddrProofCd VARCHAR(20)='', @cx_AddrProof VARCHAR(20)='',
  @cm_regno VARCHAR(50)='', @cm_regdate VARCHAR(20)='', @cm_constitution VARCHAR(10)='',
  @cm_regplace VARCHAR(100)
  ...
  @cx_ResiTaxPurpose = RIGHT(ISNULL(CM_UID,''), 4) , @cx_add1 = cm_add1 , 
  ...
  @cx_PermAddrProof = CK_PermAddrProof , @cx_PermAddrProofID = CK_PermAddrProofID , 
  @cx_PermAddrProofExpDt = CK_PermAddrProofExpDt , @cx_grossincome = cm_grossincome , @cx_brboffcode = cm_brboffcode, 
  @cx_maritalstatus = cm_maritalstatus , @cx_networth = cm_networth , @cx_networthdt = cm_networthdt, @cm_regno = cm_regno ,
  @cm_regdate = CONVERT(VARCHAR,CAST(cm_regdate AS DATE),103), @cm_constitution = 
  ISNULL((SELECT CS_KRACode FROM CKYC_StaticCodes WHERE CS_DataType='CKYCConstitutionType' AND  ISNULL(CS_KRACode,'') <> ''
  AND CS_DocTypeCode = cm_constitution),''), @cm_regplace = cm_regplace
  FROM Client_Master, Client_Info, Client_CKYC a
  WHERE cm_cd = cm2_cd AND cm_panno = CK_Panno 
  AND cm_freezeyn = 'N' 
  ...
  BEGIN
	SET @o_vcErrorFlag = 'E'
	SELECT @o_vcErrorMessage =  @o_vcErrorMessage+'<br>'+Descp
	FROM @tbl_ErrorLog
	
	UPDATE A SET A.ResponseString = @o_vcErrorMessage
    FROM tbl_GenericAPIDebugLog A
  ...
  SELECT @strKraStatus = cn_KRAStatus 
  FROM Client_Nominee(NOLOCK) WHERE cn_cd =  @strClient
	
  IF ISNULL(LTRIM(RTRIM(@StrKRAPANCode)),'') =''
  BEGIN
	SET @StrKRAPANCode = 'CVLKRA'
  END
	
  IF LTRIM(RTRIM(@StrKRAPANCode)) = 'CVLKRA'
  BEGIN
    IF ISNULL(@strKraStatus,'') = 'Y'
	BEGIN
	  SET @strUpdateInsertTag = '02'
	END  
	ELSE
	BEGIN
	  SET @strUpdateInsertTag = '01'
	END   
  END
  ELSE
  BEGIN
  ...
  IF EXISTS (SELECT 1 FROM Client_ReKycMain(NOLOCK) 
  WHERE rm_cmcd=@strClient AND rm_refno  = @strRefNo AND rm_RequestType = 'RekycOffLine')
  BEGIN
    IF EXISTS(sELECT 1 FROM Client_ModifyAPI(NOLOCK) WHERE ca_cmcd = @strClient AND ca_Nfiller3 = @strRefNo
    AND ca_desc ='KRAFlag' AND ca_newValue = 'Modification')
	BEGIN
	  SET @strUpdateInsertTag = '02'
	END
	ELSE IF EXISTS(sELECT 1 FROM Client_ModifyAPI(NOLOCK) WHERE ca_cmcd = @strClient AND ca_Nfiller3 = @strRefNo
    AND ca_desc ='KRAFlag' AND ca_newValue = 'Addition')
	BEGIN
	  SET @strUpdateInsertTag = '01'
	END
  END
  ...
  IF EXISTS(sELECT 1 FROM Client_ModifyAPI(NOLOCK) WHERE ca_cmcd = 	@strClient AND ca_Nfiller3 = @strFormNo AND ca_desc = 'IPV NAME' AND ca_newValue <> '')
  BEGIN
    SELECT @cx_Username = ca_newValue FROM Client_ModifyAPI(NOLOCK) WHERE ca_cmcd = @strClient AND ca_Nfiller3 = @strFormNo 
	AND ca_desc = 'IPV NAME' AND ca_newValue <> ''
	SELECT @cx_Designation = ca_newValue FROM Client_ModifyAPI(NOLOCK) WHERE ca_cmcd = 	@strClient AND ca_Nfiller3 = @strFormNo 
	AND ca_desc = 'IPV_DESG' AND ca_newValue <> ''
  END
  	
  SELECT @strKraCompanyName = sp_sysvalue FROM Sysparameter WHERE sp_parmcd='KRACOMPNAME'
  
  DECLARE @tbl_DocumentType TABLE(ClientCode VARCHAR(50), FileName1 VARCHAR(100), idProof VARCHAR(5), AddressProof VARCHAR(5),
  DocumentNo VARCHAR(5), DocNO VARCHAR(50), ProofType VARCHAR(50))
  
  
  IF NOT EXISTS(SELECT 1 FROM Client_ReKycMain WHERE rm_RequestType='rekyc' and rm_cmcd = @strClient 
  AND rm_status='Approve' AND rm_srno IN(SELECT MAX(rm_srno) FROM  Client_ReKycMain WHERE rm_cmcd = @strClient 
  AND rm_status='Approve'))
  BEGIN
    INSERT INTO @tbl_DocumentType(ClientCode, FileName1, idProof, AddressProof, DocumentNo,DocNO,  ProofType)
    SELECT ma_cmcd = LTRIM(RTRIM(ma_cmcd)), 
    [FILENAME] = REPLACE(replace(substring(CASE WHEN isnull(DocumentName,'') = '' THEN ma_filename ELSE DocumentName END  ,1,15),' ',''),'/','')+'_'+
    CAST(ma_srno AS VARCHAR),
    ISNULL(CKycIDCode,'Z') as IDProof, ISNULL(CKycAddressCode,'99') as AddressProof, ISNULL(CKYCDocCode,'98') As DocumentCode,  DocNo = (CASE WHEN DocumentName LIKE '%PAN%' THEN 
    ISNULL((SELECT CM_PANNO FROM CLIENT_MASTER(NOLOCK) WHERE CM_cD = XX.ma_cmcd), XX.DocumentNo)
    WHEN DocumentName LIKE '%Aadhaar%' THEN ISNULL((SELECT CM_uid FROM CLIENT_info(NOLOCK) WHERE CM2_cD = XX.ma_cmcd),XX.DocumentNo) else XX.DocumentNo end), 
    ProofType FROM Client_ModifyAttach(NOLOCK) xx LEFT OUTER JOIN tbl_DocumentKRACKYCMapping(NOLOCK) 
    on(CASE WHEN ma_field  IN('AadhaarAttachment','PANAttachment') THEN 'AddressAttachment' 
    ELSE ma_field END = ProofType and CASE WHEN ma_filename IN('Sign') then  '50'
    WHEN ma_filename IN('Photo')  then  '51' WHEN ma_filename LIKE '%Aadhaar%' THEN '28' else ma_filename end = DocumentCode)
    WHERE  ma_cmcd = @strClient
    AND ma_refno = @strFormNo  
    AND ma_proof IS NOT NULL
  END
  ELSE
  BEGIN
	INSERT INTO @tbl_DocumentType(ClientCode, FileName1, idProof, AddressProof, DocumentNo,DocNO,  ProofType)
	SELECT LTRIM(RTRIM(ma_cmcd)),'',
	CASE WHEN CS_DocTypeCode ='E' THEN '02' ELSE CS_DocTypeCode END as IDProof, CS_KRACode as AddressProof, CS_DataTypeCd As DocumentCode,  
	DocNo = (CASE WHEN CS_Description LIKE '%PAN%' THEN 
	ISNULL((SELECT CM_PANNO FROM CLIENT_MASTER(NOLOCK) WHERE CM_cD = XX.ma_cmcd),XX.DocumentNo)
	WHEN CS_Description LIKE '%Aadhaar%' THEN ISNULL((SELECT CM_uid FROM CLIENT_info(NOLOCK) WHERE CM2_cD = XX.ma_cmcd),XX.DocumentNo) 
	else XX.DocumentNo end),
	...
	or (CS_Description LIKE '%Proof of Possession of Aadhaar%' and  ma_field = 'AadhaarAttachment')))
	WHERE ma_cmcd = @strClient
	AND ma_refno IN(SELECT MAX(ma_refno) FROM Client_ModifyAttach(NOLOCK) WHERE ma_cmcd = @strClient)
	AND ma_proof IS NOT NULL
	and ma_field in('PhotoAttachment','AadhaarAttachment','SignAttachment' )
   END
   

  DECLARE @strIDProof1 VARCHAR(50)='', @strIDProofNo VARCHAR(50)='', 
  @strAddressProof1 VARCHAR(50)='', @strAddressProofNo1 VARCHAR(50)=''
	
	
  IF EXISTS(SELECT 1 FROM @tbl_DocumentType WHERE ISNULL(IdProof,'') <> '' and ProofType = 'IDAttachment')
  BEGIN
    SELECT TOP 1 @strIDProof1 = IDProof, @strIDProofNo =  DocNo 
	FROM @tbl_DocumentType WHERE ISNULL(IdProof,'') <> '' and ProofType = 'IDAttachment'
  END
  ELSE
  BEGIN
    SELECT TOP 1 @strIDProof1 = IDProof, @strIDProofNo =  DocNo 
	FROM @tbl_DocumentType WHERE ISNULL(IdProof,'') <> ''
  END
  
    
  IF EXISTS(SELECT 1 FROM @tbl_DocumentType WHERE ISNULL(AddressProof,'') <> '' and ProofType = 'AddressAttachment')
  BEGIN
   SELECT TOP 1 @strAddressProof1 = AddressProof, @strAddressProofNo1 = DocNo 
   FROM @tbl_DocumentType WHERE ISNULL(AddressProof,'') <> '' and ProofType = 'AddressAttachment'
  END
  ELSE
  BEGIN
    SELECT TOP 1 @strAddressProof1 = AddressProof, @strAddressProofNo1 = DocNo 
    FROM @tbl_DocumentType WHERE ISNULL(AddressProof,'') <> '' 
  END  
 	 
  SET @cx_IdentityProofcd = @strIDProof1
	
  SET @strRequestString = ''
  ...
  BEGIN
	...
	SET @strRequestString = @strRequestString+'"APP_POS_CODE": "##POSCODE##","APP_TYPE":"##APPTYPE##","APP_NO": "##APP_NO##","APP_DATE": "##APP_DATE##","APP_PAN_NO": "##PAN##",'
    ...
    SET @strRequestString = @strRequestString+'"APP_COMP_STATUS": "","APP_OTH_COMP_STATUS": "","APP_RES_STATUS": "R","APP_RES_STATUS_PROOF": "",'
	SET @strRequestString = @strRequestString+'"APP_UID_NO": "##APP_UID_NO##","APP_COR_ADD1": "##COR_ADD1##","APP_COR_ADD2": "##COR_ADD2##","APP_COR_ADD3": "##COR_ADD3##",'
	...
	SET @strRequestString = @strRequestString+'"APP_AUTH_MOBILE": "","APP_AUTH_FPICONSENT": "","APP_AUTH_UBOCONSENT": "", "APP_MOBILE_FLG":"","APP_EMAIL_FLG":"", '
	SET @strRequestString = @strRequestString+'"APP_OTP_REFNO":"","APP_PERM_DISTRICT":"##APP_PERM_DISTRICT##","APP_CORR_DISTRICT":"##APP_CORR_DISTRICT##",'
	SET @strRequestString = @strRequestString+'"APP_FATCA_APPLICABLE_FLAG":""'
	SET @strRequestString = @strRequestString+' ,"APP_FATCA_BIRTH_PLACE": "",'
  ...
  ELSE
  BEGIN
	SET @strRequestString = '<ROOT><HEADER><COMPANY_CODE>##POSCODE##</COMPANY_CODE><BATCH_DATE>##BATCH_DATE##</BATCH_DATE></HEADER><KYCDATA>'
    +'<APP_UPDTFLG>##APP_UPDTFLG##</APP_UPDTFLG><APP_KRA_CODE>##KRA_CODE##</APP_KRA_CODE><APP_POS_CODE>##POSCODE##</APP_POS_CODE><APP_TYPE>##APPTYPE##</APP_TYPE>'
    +'<APP_NO>##APP_NO##</APP_NO><APP_DATE>##APP_DATE##</APP_DATE><APP_PAN_NO>##PAN##</APP_PAN_NO><APP_PAN_COPY>##PAN_COPY##</APP_PAN_COPY><APP_EXMT>N</APP_EXMT>'
    +'<APP_EXMT_CAT></APP_EXMT_CAT><APP_EXMT_ID_PROOF>##EXMT_ID_PROOF##</APP_EXMT_ID_PROOF><APP_IPV_FLAG>##IPV_FLAG##</APP_IPV_FLAG><APP_IPV_DATE>##IPV_DATE##</APP_IPV_DATE>'
    +'<APP_GEN>##GEN##</APP_GEN><APP_NAME>##NAME##</APP_NAME><APP_F_NAME>##F_NAME##</APP_F_NAME><APP_REGNO>##APP_REGNO##</APP_REGNO><APP_DOB_INCORP>##DOB_INCORP##</APP_DOB_INCORP>'
    +'<APP_COMMENCE_DT>##APP_COMMENCE_DT##</APP_COMMENCE_DT><APP_NATIONALITY>##NATIONALITY##</APP_NATIONALITY><APP_OTH_NATIONALITY></APP_OTH_NATIONALITY>'
    +'<APP_COMP_STATUS>##APP_COMP_STATUS##</APP_COMP_STATUS><APP_OTH_COMP_STATUS></APP_OTH_COMP_STATUS><APP_RES_STATUS>R</APP_RES_STATUS>'
    +'<APP_RES_STATUS_PROOF></APP_RES_STATUS_PROOF><APP_UID_NO>##APP_UID_NO##</APP_UID_NO><APP_COR_ADD1>##COR_ADD1##</APP_COR_ADD1>'
    +'<APP_COR_ADD2>##COR_ADD2##</APP_COR_ADD2>'
    +'<APP_COR_ADD3>##COR_ADD3##</APP_COR_ADD3><APP_COR_CITY>##COR_CITY##</APP_COR_CITY><APP_COR_PINCD>##COR_PINCD##</APP_COR_PINCD>'
    +'<APP_COR_STATE>##COR_STATE##</APP_COR_STATE>'
    +'<APP_COR_CTRY>##COR_CTRY##</APP_COR_CTRY><APP_OFF_ISD></APP_OFF_ISD><APP_OFF_STD>##OFF_STD##</APP_OFF_STD><APP_OFF_NO>##OFF_NO##</APP_OFF_NO>'
    +'<APP_RES_ISD></APP_RES_ISD>'
    +'<APP_RES_STD>##RES_STD##</APP_RES_STD><APP_RES_NO>##RES_NO##</APP_RES_NO><APP_MOB_ISD>91</APP_MOB_ISD><APP_MOB_NO>##MOB_NO##</APP_MOB_NO>'
    +'<APP_FAX_ISD></APP_FAX_ISD>'
    +'<APP_FAX_STD></APP_FAX_STD><APP_FAX_NO></APP_FAX_NO><APP_EMAIL>##EMAIL##</APP_EMAIL><APP_COR_ADD_PROOF>##COR_ADD_PROOF##</APP_COR_ADD_PROOF>'
    +'<APP_COR_ADD_REF>##COR_ADD_REF##</APP_COR_ADD_REF><APP_COR_ADD_DT>##COR_ADD_DT##</APP_COR_ADD_DT><APP_PER_ADD_FLAG>Y</APP_PER_ADD_FLAG>'
    +'<APP_PER_ADD1>##PER_ADD1##</APP_PER_ADD1><APP_PER_ADD2>##PER_ADD2##</APP_PER_ADD2><APP_PER_ADD3>##PER_ADD3##</APP_PER_ADD3>'
    +'<APP_PER_CITY>##PER_CITY##</APP_PER_CITY>'
    +'<APP_PER_PINCD>##PER_PINCD##</APP_PER_PINCD><APP_PER_STATE>##PER_STATE##</APP_PER_STATE><APP_PER_CTRY>##PER_CTRY##</APP_PER_CTRY>'
    +'<APP_PER_ADD_PROOF>##PER_ADD_PROOF##</APP_PER_ADD_PROOF>'
    +'<APP_PER_ADD_REF>##PER_ADD_REF##</APP_PER_ADD_REF><APP_PER_ADD_DT>##PER_ADD_DT##</APP_PER_ADD_DT><APP_INCOME></APP_INCOME><APP_OCC></APP_OCC>'
    +'<APP_OTH_OCC></APP_OTH_OCC><APP_POL_CONN>NA</APP_POL_CONN><APP_DOC_PROOF>S</APP_DOC_PROOF><APP_INTERNAL_REF>##INTERNAL_REF##</APP_INTERNAL_REF>'
    +'<APP_BRANCH_CODE>##BRANCH_CODE##</APP_BRANCH_CODE><APP_MAR_STATUS>##MAR_STATUS##</APP_MAR_STATUS><APP_NETWRTH>##NETWRTH##</APP_NETWRTH>'
    +'<APP_NETWORTH_DT>##NETWORTH_DT##</APP_NETWORTH_DT><APP_INCORP_PLC>##APP_INCORP_PLC##</APP_INCORP_PLC><APP_OTHERINFO></APP_OTHERINFO>'
    +'<APP_FILLER1></APP_FILLER1><APP_FILLER2></APP_FILLER2><APP_FILLER3></APP_FILLER3><APP_IPV_NAME>##IPV_NAME##</APP_IPV_NAME>'
    +'<APP_IPV_DESG>##IPV_DESG##</APP_IPV_DESG><APP_IPV_ORGAN>##IPV_ORGAN##</APP_IPV_ORGAN>'
    +'<APP_KYC_MODE>0</APP_KYC_MODE><APP_VER_NO>V29</APP_VER_NO><APP_VID_NO></APP_VID_NO>'
	+'<APP_UID_TOKEN></APP_UID_TOKEN>'
	+'<APP_AUTH_NAME></APP_AUTH_NAME>'
    +'<APP_AUTH_EMAIL></APP_AUTH_EMAIL><APP_AUTH_EMAIL1></APP_AUTH_EMAIL1><APP_AUTH_EMAIL2></APP_AUTH_EMAIL2>'
    +'<APP_AUTH_MOBILE></APP_AUTH_MOBILE>'
    +'<APP_AUTH_FPICONSENT></APP_AUTH_FPICONSENT><APP_AUTH_UBOCONSENT></APP_AUTH_UBOCONSENT><APP_MOBILE_FLG></APP_MOBILE_FLG>'
    +'<APP_EMAIL_FLG></APP_EMAIL_FLG><APP_OTP_REFNO></APP_OTP_REFNO><APP_PERM_DISTRICT>##APP_PERM_DISTRICT##</APP_PERM_DISTRICT>'
    +'<APP_CORR_DISTRICT>##APP_CORR_DISTRICT##</APP_CORR_DISTRICT>'
    +'<APP_FATCA_APPLICABLE_FLAG>N</APP_FATCA_APPLICABLE_FLAG><APP_FATCA_COUNTRY_RES></APP_FATCA_COUNTRY_RES>'
    +'<APP_FATCA_BIRTH_PLACE></APP_FATCA_BIRTH_PLACE>'
    +'<APP_FATCA_BIRTH_COUNTRY></APP_FATCA_BIRTH_COUNTRY><APP_FATCA_OTHER_SERVICES></APP_FATCA_OTHER_SERVICES>'
    +'<APP_FATCA_COUNTRY_CITYZENSHIP></APP_FATCA_COUNTRY_CITYZENSHIP>'
    +'<APP_FATCA_DATE_DECLARATION>##APP_FATCA_DATE_DECLARATION##</APP_FATCA_DATE_DECLARATION></KYCDATA>'
  END
	
  SET @strRequestString = REPLACE(@strRequestString ,'##APPTYPE##',CASE WHEN ISNULL(LTRIM(RTRIM(@cm_constitution)),'') = '' THEN 'I' ELSE 'N' END)
  ...
  set @strRequestString = REPLACE(@strRequestString ,'##F_NAME##',LTRIM(RTRIM(LTRIM(RTRIM(@cx_FatherFname))+' '+LTRIM(RTRIM(@cx_FatherMname))+' '+LTRIM(RTRIM(@cx_FatherLname)))))
  set @strRequestString = REPLACE(@strRequestString ,'##APP_REGNO##',LTRIM(RTRIM(@cm_regno)))
  set @strRequestString = REPLACE(@strRequestString ,'##DOB_INCORP##',LTRIM(RTRIM(@cx_dob)))
  set @strRequestString = REPLACE(@strRequestString ,'##APP_COMMENCE_DT##',LTRIM(RTRIM(@cm_regdate)))
  ...
  set @strRequestString = REPLACE(@strRequestString ,'##APP_COMP_STATUS##',LTRIM(RTRIM(@cm_constitution)))
  set @strRequestString = REPLACE(@strRequestString ,'##APP_UID_NO##',LTRIM(RTRIM(@cx_ResiTaxPurpose)))
  ...
  Set @strRequestString = REPLACE(@strRequestString ,'##COR_ADD_PROOF##',LTRIM(RTRIM(@strAddressProof1)))
  set @strRequestString = REPLACE(@strRequestString ,'##COR_ADD_REF##',LTRIM(RTRIM(@strAddressProofNo1)))
  ...
  DECLARE @StrCORRDISTRICT VARCHAR(5)='', @StrPERMDISTRICT VARCHAR(5)=''
	  
  SELECT @StrCORRDISTRICT = CS_KRACode FROM CKYC_StaticCodes(NOLOCK) 
  WHERE cs_datatype ='DISTRICT' AND CS_CODE = LTRIM(RTRIM(@cx_pincode))
	  
  SELECT @StrPERMDISTRICT = CS_KRACode FROM CKYC_StaticCodes(NOLOCK) 
  WHERE cs_datatype ='DISTRICT' AND CS_CODE = LTRIM(RTRIM(@cx_ppincode))
	  
  SET @strRequestString = REPLACE(@strRequestString ,'##APP_INCORP_PLC##',LTRIM(RTRIM(@cm_regplace)))
  SET @strRequestString = REPLACE(@strRequestString ,'##APP_CORR_DISTRICT##',LTRIM(RTRIM(@StrCORRDISTRICT)))
  SET @strRequestString = REPLACE(@strRequestString ,'##APP_PERM_DISTRICT##',LTRIM(RTRIM(@StrPERMDISTRICT)))
  ...
  IF ISNULL(@strurl,'') = ''
  BEGIN
    SELECT @strRequestString = @strRequestString+'<ADDITIONALDATA><APP_ADDLDATA_UPDTFLG>01</APP_ADDLDATA_UPDTFLG><APP_ENTITY_PAN>'+LTRIM(RTRIM(@cx_Panno))+'</APP_ENTITY_PAN>'
	  +'<APP_ADDLDATA_PAN>'+cc_cpanno+'</APP_ADDLDATA_PAN><APP_ADDLDATA_NAME>'+cc_cname+'</APP_ADDLDATA_NAME>'
	  +'<APP_ADDLDATA_DIN_UID>'+cc_din+'</APP_ADDLDATA_DIN_UID><APP_ADDLDATA_RELATIONSHIP>'+ISNULL((SELECT CS_KRACode FROM CKYC_StaticCodes(NOLOCK) 
	WHERE cs_datatype like  'CKYCRelatedPerson%' AND CS_Description = cc_Cdesignation),'')+'</APP_ADDLDATA_RELATIONSHIP>'
      +'<APP_ADDLDATA_POLCONN>NA</APP_ADDLDATA_POLCONN><APP_ADDDATA_REGDADD1>'+cc_caddress1+'</APP_ADDDATA_REGDADD1>'
	  +'<APP_APPDATA_REGDADD2>'+cc_address2+'</APP_APPDATA_REGDADD2>'
      +'<APP_APPDATA_REGDADD3>'+cc_address3+'</APP_APPDATA_REGDADD3>'
	  +'<APP_ADDDATA_CITY>'+cc_addressCity+'</APP_ADDDATA_CITY><APP_ADDDATA_PINCD>'+cc_AddressPINCode+'</APP_ADDDATA_PINCD>'
      +'<APP_ADDDATA_STATE>'+ISNULL((SELECT CS_KRACode FROM CKYC_StaticCodes(NOLOCK) Where cs_datatype='State' and cs_description = cc_AddressState),'')+'</APP_ADDDATA_STATE>'
	  +'<APP_ADDDATA_COUNTRY>'+ISNULL((SELECT CS_KRACode FROM CKYC_StaticCodes(NOLOCK) Where cs_datatype='Country' and cs_description = cc_AddressCountry),'')+'</APP_ADDDATA_COUNTRY>'
	  +'<APP_ADDLDATA_FILLER1></APP_ADDLDATA_FILLER1>'
      +'<APP_ADDLDATA_FILLER2></APP_ADDLDATA_FILLER2><APP_ADDLDATA_FILLER3></APP_ADDLDATA_FILLER3></ADDITIONALDATA>' 
    FROM Tbl_ClientRelatedPerson(NOLOCK) WHERE cc_cmcd = @strClient
	 
	DECLARE @strRelatedPersonCount INT = 0
	SELECT @strRelatedPersonCount = COUNT(*)
	FROM Tbl_ClientRelatedPerson(NOLOCK) WHERE cc_cmcd = @strClient
     
    SET @strRequestString = @strRequestString+'<FATCA_ADDL_DTLS><APP_FATCA_ENTITY_PAN></APP_FATCA_ENTITY_PAN><APP_FATCA_COUNTRY_RESIDENCY></APP_FATCA_COUNTRY_RESIDENCY>'
      +'<APP_FATCA_TAX_IDENTIFICATION_TYPE></APP_FATCA_TAX_IDENTIFICATION_TYPE><APP_FATCA_TAX_IDENTIFICATION_NO></APP_FATCA_TAX_IDENTIFICATION_NO>'
      +'<APP_FATCA_TAX_EXEMPT_FLAG></APP_FATCA_TAX_EXEMPT_FLAG><APP_FATCA_TAX_EXEMPT_REASON></APP_FATCA_TAX_EXEMPT_REASON>'
      +'</FATCA_ADDL_DTLS><FOOTER><NO_OF_KYC_RECORDS>1</NO_OF_KYC_RECORDS>'
      +'<NO_OF_ADDLDATA_RECORDS>'+CAST(@strRelatedPersonCount AS VARCHAR)+'</NO_OF_ADDLDATA_RECORDS><NO_OF_FATCA_ADDL_DTLS_RECORDS>0</NO_OF_FATCA_ADDL_DTLS_RECORDS></FOOTER></ROOT>'
    
	DECLARE @StrRequestJson VARCHAR(MAX)='', @StrTXTFileName VARCHAR(50)=@cx_Panno+'.XML'
  
    SET @StrRequestJson = '{"baseFolderName":"##baseFolderName##","baseFileString": "##baseFileString##",
      "baseFileName":"##baseFileName##","subFileDetails":[]}'
    SET  @StrRequestJson  = REPLACE(@StrRequestJson,'##baseFolderName##','')
    SET  @StrRequestJson  = REPLACE(@StrRequestJson,'##baseFileName##',@StrTXTFileName)
    SET  @StrRequestJson  = REPLACE(@StrRequestJson,'##baseFileString##',@strRequestString)
