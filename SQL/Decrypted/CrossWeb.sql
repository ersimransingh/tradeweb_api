...
CREATE PROCEDURE stpr_HoldingStatement @vcXML NVARCHAR(MAX), @o_vcErrorFlag VARCHAR(1) OUTPUT, @o_vcErrorMessage VARCHAR(MAX) OUTPUT
WITH ENCRYPTION
AS
BEGIN
  /*
 ...
 // Created Date  : 
 // Description   : HOLDING Statement Report use in Cross Web Application
 // Reviewed By   : 
 // Review Date   : 
 //////////////////////////////////////////////////////////////////////////////////////////
*/
  --- Parameter Declaration
  DECLARE @dtFromDate VARCHAR(20), @dtToDt VARCHAR(20), @strUserId VARCHAR(500) = '', @strLevel VARCHAR(1), @strCode VARCHAR(500), 
    @strSelection VARCHAR(100), @XMLData XML, @strOutputType VARCHAR(1) = 'G', @strRequestFrom VARCHAR(1) = 'W', @XMLDATA1 XML = '', 
    @strShowValuation VARCHAR(20) = '', @strReportType VARCHAR(30) = '', @strReportDisplay VARCHAR(1) = 'P', @strISIN VARCHAR(50) = '', 
    @strBenefType VARCHAR(50) = '', @strSql1 NVARCHAR(MAX) = '', @strCallingFrom VARCHAR(1) = ''
  ...
  SELECT @dtFromDate = ISNULL(x.value('(FromDate)[1]', 'VARCHAR(8)'), ''), @dtToDt = ISNULL(x.value('(ToDate)[1]', 'VARCHAR(8)'), 
      ''), @strUserId = ISNULL(x.value('(UserId)[1]', 'VARCHAR(500)'), ''), @strLevel = ISNULL(x.value('(Level)[1]', 'VARCHAR(1)'
      ), ''), @strCode = ISNULL(x.value('(Code)[1]', 'VARCHAR(500)'), ''), @strSelection = ISNULL(x.value('(Selection)[1]', 
        'VARCHAR(50)'), ''), @strOutputType = ISNULL(x.value('(OutputType)[1]', 'VARCHAR(1)'), ''), @strRequestFrom = ISNULL(x.
      value('(RequestFrom)[1]', 'VARCHAR(1)'), ''), @strShowValuation = ISNULL(x.value('(ShowValuation)[1]', 'VARCHAR(10)'), ''
    ), @strReportType = ISNULL(x.value('(ReportType)[1]', 'VARCHAR(30)'), ''), @strReportDisplay = ISNULL(x.value(
        '(ReportDisplay)[1]', 'VARCHAR(30)'), ''), @strISIN = ISNULL(x.value('(ISIN)[1]', 'VARCHAR(30)'), ''), @strBenefType = 
    ISNULL(x.value('(BenefType)[1]', 'VARCHAR(30)'), ''), @strCallingFrom = ISNULL(x.value('(CallingFrom)[1]', 'VARCHAR(30)'), 
      '')
  FROM @XMLData.nodes('/root') AS XTbl(x)

  IF ISNULL(@strCallingFrom, '') = ''
  BEGIN
    SET @strCallingFrom = ''
  END
  ...
  BEGIN
    SELECT @dtFromDate = ISNULL(x.value('(FromDt)[1]', 'VARCHAR(8)'), ''), @dtToDt = ISNULL(x.value('(ToDt)[1]', 'VARCHAR(8)'), ''
      )
    FROM @XMLData.nodes('/root') AS XTbl(x)
  END
  ...
  DECLARE @strServer VARCHAR(50) = '', @strOwner VARCHAR(50) = ''

  IF CHARINDEX('|', @strUserId) > 0
  BEGIN
    INSERT INTO @tbl_UserList (ClientCode)
    SELECT *
    FROM (
      SELECT VALUE
      FROM DBO.RETURNTABLE(@strUserId, '|')
      ) X1
    WHERE ISNULL(X1.VALUE, '') <> ''
  END
  ELSE IF ISNULL(@strCallingFrom, '') = 'T'
  BEGIN
    DECLARE @strConn1 VARCHAR(MAX) = '', @tplusUser VARCHAR(500) = '',@FullServerPath VARCHAR(50)=''

    SELECT @strServer = sp_sysvalue
    FROM Sysparameter
    WHERE sp_parmcd IN ('TPSERVER')

    SELECT @tplusUser = sp_sysvalue
    FROM Sysparameter
    WHERE sp_parmcd IN ('TPDB')

    SELECT @strOwner = sp_sysvalue
    FROM Sysparameter
    WHERE sp_parmcd IN ('TPOWNER')

    IF ISNULL(@strServer, '') <> ''
	BEGIN
      SET @FullServerPath = @strServer;
    
      SET @strSql1 = N' SELECT ClientCode FROM OPENQUERY([' + @FullServerPath + 
      '], ''
	    SELECT DISTINCT d.da_actno AS ClientCode FROM ' + @tplusUser + '.' + @strOwner + '.fn_GetClients(''''' + 
      @strUserid + ''''', '''''''', '''''''') c
            INNER JOIN ' + @tplusUser + '.' + @strOwner + 
      '.DEMATACT d
                ON c.CLIENT_CODE = d.da_clientcd
            WHERE d.DA_STATUS = ''''A''''
                AND d.da_defaultyn = ''''Y''''
                AND d.da_actno IS NOT NULL
        '')';
	END
	ELSE
	BEGIN
	   SET @strSql1 = 'DECLARE @tbl_AccessRights TABLE(Client VARCHAR(50), clientcode VARCHAR(20)) 
	   INSERT INTO @tbl_AccessRights(Client)
	   SELECT CLIENT_CODE FROM ' + @tplusUser + '.' + @strOwner + '.fn_GetClients('''+@strUserid+''','''','''')
	   UPDATE A SET clientcode = b.da_actno
	   FROM @tbl_AccessRights A, ' + @tplusUser + '.' + @strOwner + '.DEMATACT B
	   WHERE A.Client = B.da_clientcd
	   AND DA_STATUS = ''A''
	   AND da_defaultyn = ''Y''
	   SELECT ClientCode from @tbl_AccessRights'
	END
  ...
  BEGIN
    ...
    WITH DistinctNodes
    AS (
      SELECT DISTINCT NodeName = X.n.value('local-name(.)', 'NVARCHAR(100)')
      FROM @xmlData2.nodes('/root/ClientDetail/*') AS X(n)
      )
    SELECT @DynamicColumns = @DynamicColumns + ',' + 'LTRIM(RTRIM(X1.value(''(' + XX.NodeName + ')[1]'', ''NVARCHAR(MAX)''))) AS [' 
      + XX.NodeName + ']'
    ...
    BEGIN
      ...
      WHERE ReportCode = 'CrossNet' AND ReportCategroy = 'Disp_Holding'
      ORDER BY Orderby
    END
  END
  ...
  CREATE TABLE #tbl_HoldingRepDP (
    ClientCode VARCHAR(500), ClientName VARCHAR(200), BranchCode VARCHAR(50), ScripCode VARCHAR(50), ScripName VARCHAR(200), ISIN 
    VARCHAR(20), Qty MONEY, ClosingPrice MONEY, MarketValue MONEY, AccountType VARCHAR(500), BackOfficeCode VARCHAR(509)
    )

  BEGIN TRY
    IF @strReportType = 'UPTODATE'
    BEGIN
      INSERT INTO #tbl_HoldingRepDP (ClientCode, ClientName, BranchCode, ScripName, ISIN, AccountType, Qty, ClosingPrice, MarketValue, BackOfficeCode
        )
      SELECT cm_cd AS ClientCode, cm_name AS ClientName, cm_brboffcode AS BranchCode, ScripName, td_isin_code AS ISIN, bt_description, 
        BalanceQty AS Qty, 0, 0, cm_blsavingcd
      FROM (
        SELECT td_ac_code, td_isin_code, ScripName, bt_description, td_ac_type, BalanceQty = sum(BalanceQty)
        FROM (
          SELECT td_ac_code = hld_ac_code, ScripName = sc_isinname, td_isin_code = hld_isin_code, td_ac_type = hld_ac_type, SUM(
              hld_ac_pos) AS BalanceQty
          FROM DBO.Holding(NOLOCK), DBO.Security(NOLOCK), @tbl_UserList x
          WHERE hld_ac_code = x.clientcode AND hld_isin_code = sc_isincode
          GROUP BY hld_ac_code, hld_isin_code, hld_ac_type, sc_isinname
          HAVING SUM(hld_ac_pos) <> 0
          ) x1
        LEFT OUTER JOIN DBO.Beneficiary_type BN ON (td_ac_type = BN.bt_code)
        GROUP BY td_ac_code, td_isin_code, bt_description, td_ac_type, ScripName
        ) A, [dbo].client_master(NOLOCK)
      WHERE cm_schedule = (
          SELECT sp_sysvalue
          FROM Sysparameter
          WHERE sp_parmcd = 'cmschedule'
          ) AND td_ac_code = cm_cd AND ((td_isin_code = @strISIN AND @strISIN <> '') OR @strISIN = '') AND ((td_ac_type = @strBenefType AND @strBenefType <> '') OR @strBenefType = ''
          )
    ...
    BEGIN
      ...
      FROM (
        SELECT td_ac_code, td_isin_code, ScripName, bt_description, td_ac_type, BalanceQty = sum(BalanceQty)
        FROM (
          SELECT td_ac_code = td_ac_code, ScripName = sc_isinname, td_isin_code = td_isin_code, td_ac_type = td_ac_type, SUM(CASE WHEN 
                  td_debit_credit = 'D' THEN td_qty * - 1 ELSE td_qty END) AS BalanceQty
          FROM Trxdetail(NOLOCK), DBO.Security(NOLOCK), @tbl_UserList x
          WHERE td_ac_code = x.clientcode AND td_isin_code = sc_isincode AND td_curdate <= @dtFromDate
          GROUP BY td_ac_code, td_isin_code, td_ac_type, sc_isinname
          HAVING SUM(CASE WHEN td_debit_credit = 'D' THEN td_qty ELSE td_qty * - 1 END) <> 0
          ) x1
        LEFT OUTER JOIN DBO.Beneficiary_type BN ON (td_ac_type = BN.bt_code)
        GROUP BY td_ac_code, td_isin_code, bt_description, td_ac_type, ScripName
        ) A, [dbo].client_master(NOLOCK)
  ...
  BEGIN
    ...
    SELECT @dtMax = max(rm_trx_date)
    FROM DBO.Rate_master
    WHERE rm_trx_date <= @dtFromDate

    INSERT INTO @TBL_CloseRate (ISIN, CloseRate)
    SELECT rm_isin_code, rm_rate
    FROM DBO.Rate_master(NOLOCK) X
    WHERE rm_trx_date = @dtMax AND EXISTS (
        SELECT 1
        FROM #tbl_HoldingRepDP
        WHERE ISIN = X.rm_isin_code
        )

    UPDATE A
    SET A.ClosingPrice = B.CloseRate
    ...
    FROM DBO.Rate_master(NOLOCK) X, (
        ...
        FROM #tbl_HoldingRepDP
        WHERE isnull(ClosingPrice, 0) = 0
        ) B
    WHERE x.rm_isin_code = B.ISIN AND rm_trx_date = (
        SELECT max(rm_trx_date)
        FROM DBO.Rate_master
        WHERE rm_trx_date <= CONVERT(VARCHAR, GETDATE(), 112) AND rm_isin_code = X.rm_isin_code
        )

    UPDATE A
    SET A.ClosingPrice = B.CloseRate
    FROM #tbl_HoldingRepDP A, @TBL_CloseRate b
    WHERE A.ISIN = B.ISIN AND isnull(ClosingPrice, 0) = 0

    IF ISNULL(@strOutputType, 'G') = 'G'
    BEGIN
      SELECT ClientCode, ClientName, BackOfficeCode, [ISINName] = ScripName, ISIN, [BalanceType] = AccountType, Qty, Rate = 
        ClosingPrice, [Value] = ROUND(Qty * ClosingPrice, 2)
      FROM #tbl_HoldingRepdp
    ...
    BEGIN
      ...
      BEGIN
        SET @XMLDATA1 = (
            SELECT ClientCode = '', ClientName = '', BackOfficeCode = '', ISINName = '', ISIN = '', BalanceType = '', Qty = '', Rate = '', 
              [Value] = ''
            FOR XML PATH('Detail')
            )
      END
  ...
  BEGIN
    IF ISNULL(@strOutputType, 'G') = 'G'
    BEGIN
      SELECT ClientCode, ClientName, BackOfficeCode, [ISIN Name] = ScripName, ISIN, [Balance Type] = AccountType, Qty
      FROM #tbl_HoldingRepdp
      ORDER BY ClientCode, AccountType, ScripName
    ...
    BEGIN
      ...
      BEGIN
        SET @XMLDATA1 = (
            SELECT ClientCode = '', BackOfficeCode = '', ClientName = '', ISINName = '', ISIN = '', BalanceType = '', Qty = ''
            FOR XML PATH('Detail')
            )
      END
      ELSE
      BEGIN
        SET @XMLDATA1 = (
            SELECT ClientCode, ClientName, BackOfficeCode, ISINName = ScripName, ISIN, BalanceType = AccountType, Qty
            FROM #tbl_HoldingRepdp
            ORDER BY ClientCode, AccountType, ScripName
            FOR XML PATH('Detail')
            )
...
BEGIN
 ...
 // Create By     : VAIBHAV GARG
 // Created Date  : 
 // Description   : Transaction Statement Report use in Cross Web Application
 // Reviewed By   : 
 // Review Date   : 
 //////////////////////////////////////////////////////////////////////////////////////////
*/
  ...
  --SELECT @vcXML
  DECLARE @tbl_UserList dbo.UserAccessList;
  DECLARE @strJsonText VARCHAR(MAX)=''
  SET @XMLData = CAST('<root>' + @vcXML + '</root>' AS XML)
  ...
  @strJsonFormat = ISNULL(x.value('(JsonFormat)[1]', 'VARCHAR(1)'), '')
  FROM @XMLData.nodes('/root') AS XTbl(x)
  ...
  Runingbalace = (SUM(DebitQty+CreditQty) OVER (PARTITION  BY td_ac_code, sc_isinname, bt_description,  td_ac_type 
  ORDER BY td_ac_code, sc_isinname, bt_description, td_curdate, td_trxno, SerialNo))*-1 
  FROM( 
  ...
  IF @strJsonFormat = 'Y' 
  BEGIN
    SET @strJsonText = (SELECT DISTINCT ClientCode , ClientName, [Scrip Detail] =  
	(SELECT DISTINCT ISIN, ScripName, Description, 
	[Transaction] = (select TrxnDate, TrxnNo, Narration, DebitQty, CreditQty, Runingbalace   
    FROM @tbl_Transaction xdtl where xdtl.ClientCode = xsc.ClientCode  and ISIN = xsc.ISIN and Description = xsc.Description
    FOR JSON PATH )
     FROM @tbl_Transaction xsc where xmain.ClientCode = xsc.ClientCode  FOR JSON PATH )
    FROM @tbl_Transaction xmain FOR JSON PATH )
	SELECT @strJsonText
	RETURN 1
  END	
