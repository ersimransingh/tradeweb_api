...
BEGIN
  ...
  BEGIN
	...
	AND CK_SRNO IN(SELECT MAX(CK_SRNO) FROM Client_CKYC WHERE CK_Panno = CXXX.CK_Panno))
		...
		[OldEmailMob] = (Select sp_sysvalue From WebParameter(NOLOCK) where sp_parmcd='OldEmailMobOTP') FOR JSON PATH),
	    ---- FOR ReKyc HEADER
...
BEGIN
  ...
  BEGIN
	...
	Set @CompanyAddress = (Select (em_add1 + ' ' +	em_add2 + ' ' +	em_add3 + ' ' +	em_add4 + ' ' +	em_tele + ' ' +	em_jurisdiction)
								 From Entity_master with (Nolock) WHERE em_cd = (SELECT MIN(em_cd) from Entity_master)) 
    Select @DPID = Left(da_dpid,8), @ClientId = RIGHT(da_actno,8)   From Dematact where da_clientcd = @strClientCode and da_status = 'A'
