...
BEGIN
  ...
  SUM(Qty) TotalQty, ClosingPrice, MarketValue = SUM(ROUND(Qty* ClosingPrice,2)), 
	   ...
	   WHERE Rq_Clientcd = ClientCode and Rq_Scripcd = ScripCode and Rq_Status3 = 'P'),0),
	   ReqValue = isNull((select sum(Rq_Qty) From PledgeRequest(NOLOCK) 
	   WHERE Rq_Clientcd = ClientCode and Rq_Scripcd = ScripCode and Rq_Status3 = 'P'),0)*ClosingPrice
  FROM #tbl_HoldingRep
...
BEGIN
  ...
  BEGIN
    DECLARE @i_vxXML VARCHAR(MAX), @strClientCode VARCHAR(50)='', @strCompanyCode VARCHAR(1)
	...
	SET @i_vxXML = '<UserId>'+@strClientCode+'</UserId><CompanyCode>'+@strCompanyCode+'</CompanyCode><DPCode>'+@strDPAccountNo+'</DPCode>'
