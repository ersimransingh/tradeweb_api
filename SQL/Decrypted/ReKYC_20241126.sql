...
BEGIN
  ...
  RequestType VARCHAR(50), RequestReason VARCHAR(500), rm_refno numeric, BOID VARCHAR(16))
  ...
  BEGIN
    ...
    AND rm_status = 'Pending' And rm_RequestType = 'Account Closure' And rm_step = 5
  ...
  BEGIN
    ...
    WHEN RequestType = 'B' THEN 'Trading+Demat' else '' end, RequestReason, rm_refno, BOID FROM #TBL_ClosureData 
  ...
  BEGIN
	...
	RequestType = '', RequestReason = '', rm_refno = '', BOID = '' FOR JSON PATH)
  End
...
BEGIN
  ...
  */
  ...
  BEGIN
    SET @strLedgerJson = (SELECT [Attachment] =
    ...
    WHERE ma_filename='CMRAttachment' and ma_cmcd = @i_vcUserCode and ma_status ='N'  FOR JSON PATH, ROOT('CMRAttachment'))
  END
  ELSE
  BEGIN
  ...
  BEGIN
    ...
    then dbo.fnBinaryToBase64(ma_proof) else dbo.fnBinaryToBase64(DBO.FN_base64toBinary(CAST(CONVERT(varchar(MAX), REPLACE(ma_proof,'data:image/jpeg;base64,','')) AS NVARCHAR(MAX)))) end
	FROM  Client_ModifyAttach(NOLOCK) 
    WHERE ma_filename='SignedPdf' and ma_cmcd = @i_vcUserCode and ma_status ='N'  FOR JSON PATH, ROOT('SignedPdf'))
  END
  ELSE
  BEGIN
...
BEGIN
  ...
  WHEN ma_field = 'SegmentAttachment' THEN 'Segment Attachment' ELSE ma_field END),  ma_filename, 
  ...
  */
  ...
  DELETE FROM @tbl_fn_json_merge
 -- SELECT @mainjson, @o_JsonOutput
  --SET @strString = 'EXEC '+@strtradeplustempdb+'.DBO.' + 'SP_json_merge' + ' ''' + @o_JsonOutput + ''', '''+@mainjson+''', @o_jsonoutput OUTPUT';
  --EXEC sp_executesql @strString, N'@o_jsonoutput VARCHAR(MAX) OUTPUT', @o_JsonOutput OUTPUT;
...
CREATE OR ALTER PROCEDURE [dbo].[SP_AccountClosurePDF]  @dsXml AS XML  WITH ENCRYPTION
AS
BEGIN
  ...
  -- SELECT @strAcType = Isnull(ParameterValue,0)  From @tb_ParamListDetail    WHERE ParameterName = 'AccountType' AND HeaderName = 'X_Data' 
   ...
   SET @strString  =  ' SELECT cm_name FirstHolderName, '''' SecondHolderName, '''' ThirdHolderName, Left(cm_cd,8) DPID, Right(cm_cd,8) ClientId, 
		...
		(Select sp_sysvalue From CrossNariman.dbo.Sysparameter where sp_parmcd = ''NAME'') as CompanyName, rm_RefNo  RefNo , 
		STUFF((Select '' '' + sp_sysvalue From CrossNariman.dbo.Sysparameter Where sp_parmcd IN (''ADD1'',''ADD2'',''ADD3'') FOR XML PATH ('''')), 1, 1, '''')  as CompanyFullAddress ,
