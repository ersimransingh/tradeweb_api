...
@strReportCode VARCHAR(50), @strReportCategroy VARCHAR(50), @strRequestFrom VARCHAR(1), @StrUserType VARCHAR(10) ='Client' WITH ENCRYPTION  AS
BEGIN
...
*/
 --- Parameter Declaration
  ...
  END
   IF @strOutputType = 'G'
   BEGIN
    SELECT SERIALNO, ClientCode, ClientName, Date = convert(varchar,cast(Date as date),112), ExchSeg, Voucher, Particular, Debitflag, 
	Chequeno,
	DebitAmount, CreditAmount, Balance, BalanceTag, Documenttype, Common, Ldate, CESCD, LookUp  
...
BEGIN
  ...
  @StrPlScripCode VARCHAR(20)='', @StrClientCode VARCHAR(50)='', @StrUserType VARCHAR(50)='', @StroptTurnover VARCHAR(50)='Premium', @StrASEXTurnover VARCHAR(50)='Difference'
  ...
  BEGIN
   ...
   BEGIN
	 ...
	 SELECT @StrClientCode = LTRIM(RTRIM(ParameterValue)) FROM @tbl_InputJSONTable WHERE ParameterName = 'ClientCode'
	 ...
	 BEGIN
	   ...
	   SET @CommanXMLString = REPLACE(@CommanXMLString,'##Product##',ISNULL(@strBillSegment,''))  
   ...
   'InvestorReport','CaptialGain','CapitalGain','Transaction','BILL','Margin','COLLPOS','Download','Margin_NEW',
   'Margin1','Margin2','BillSummary','DeliveryStatement','DeliveryPending')
   BEGIN
   */
...
*/
 --- Parameter Declaration
  ...
  DECLARE @strString NVARCHAR(MAX)='',
    ...
    @outputmessage VARCHAR(MAX)='', @vcProcedureName VARCHAR(50)='', @strGridType VARCHAR(20)=''
 ...
 --SELECT @i_vcProjectCode, @i_vcReportCode, @i_vcReportCategory,@vcProcedureName
  ...
  BEGIN TRY
	...
	@SettUserType VARCHAR(50)='', @SettoptTurnover VARCHAR(50), @SettASEXTurnover VARCHAR(50)
	SET @XMLData = CAST('<root>'+@vcXML+'</root>' AS XML)
	...
	--'CapitalGain','Transaction','Bill','Margin','COLLPOS','Download','Margin_NEW','Margin1','Margin2','BillSummary','DeliveryStatement')
	--BEGIN
	  ...
	  EXEC stpr_GenerateMobileGrid @o_vcErrorMessage, @strtag, @i_vcReportCode, @i_vcReportCategory, @RequestFrom, @SettUserType
	--END
	...
	BEGIN
	 ...
	 and ColumnVisible = 0
	 AND ISNULL(ProductCode,'') = ''
	 and ((@settUserType = 'Client' and ColumnHeading not in('Client Code','Client Name'))
	  ...
	  */
	  ...
	  SET @StrShowFilter2 = REPLACE(@StrShowFilter2,'<<optTurnover>>',@SettoptTurnover)
      ...
      SET @StrShowFilter3 = REPLACE(@StrShowFilter3,'<<ASEXTurnover>>',@SettASEXTurnover)
...
BEGIN 
  ...
  BEGIN
	...
	BEGIN
	  SET @strEmailJson = (select ParameterName, ParameterValue from @tbl_InputJSONTable where HeaderName = 'X_Filter' and ParameterName <> 'X_Filter' FOR JSON PATH);
      SET @strParam = (SELECT '{' + STRING_AGG('"' + ParameterName + '":"' + ParameterValue + '"', ',') + '}' AS FinalJson
...
BEGIN 
  ...
  UPDATE X SET X.Jsonvalue = REPLACE(REPLACE(X.Jsonvalue,'"##filters##"',CASE WHEN ISNULL(XX.filters,'') = '' THEN '[]' ELSE '['+ISNULL(XX.filters,'')+']'  END),
  '##UserId##',@strUserid)
  ...
  BEGIN
	...
	*/
	...
	BEGIN
	  IF @strOutputType = 'X'
	  BEGIN
	    SET @XMLDATA1 = (SELECT CompanyCode, Exchange, Segment, ClientCode, ClientName, BillNo, BillDate, Scrip = ISNULL(Scrip,''), Symbol = isnull(Symbol,''), 
...
CREATE PROCEDURE [dbo].[stpr_InsertUpdateXMLData] @dsXml XML, @o_iReturnMainSlNo INT OUTPUT, @o_vcFlag VARCHAR
...
BEGIN
	...
	DECLARE @strType VARCHAR(50) = @strUserType
	...
	BEGIN
		...
		WHERE cm_schedule = 49843750 --AND cm_freezeyn <> 'Y'
		AND EXISTS(SELECT 1 FROM @tbl_UserList WHERE ClientCode = XMAIN.CM_CD)
	END
	...
	BEGIN
		...
		WHERE EXISTS(SELECT 1 FROM @tbl_UserList WHERE ClientCode = XMAIN.CM_CD))
