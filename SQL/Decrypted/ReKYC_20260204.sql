 ...
 ------------ ******  Reject Logic Start ***********
  ...
  BEGIN
	...
	--BEGIN
	  ...
	  Ck_RespType = '', Ck_AppType = (CASE WHEN ISNULL(@strCK_Reference,'') <> '' THEN '03' ELSE '01' END),
	...
	*/
	...
	AND CK_SRNO IN(SELECT MAX(CK_SRNO) FROM Client_CKYC WHERE CK_Panno = CXXX.CK_Panno))
		...
		PersonalDetails = (
                ...
                AND CK_SRNO IN(SELECT MAX(CK_SRNO) FROM CLIENT_CKYC WHERE CK_PANNO = X.CK_PANNO)),'')
...
BEGIN
 ...
 SET @o_vcErrorMessage = 'Process Completed'
  ...
  BEGIN  
	...
	@cm_regdate = cm_regdate, @cm_regno = cm_regno
	...
	BEGIN
	  ...
	  BEGIN
	    ...
	    dbo.fn_GetFileExtension(CAST(ma_proof AS VARCHAR(MAX))) as filetype, ProofType
	  ...
	  BEGIN
	    ...
	    REPLACE(CAST(ma_proof AS VARCHAR(MAX)),'data:image/jpeg;base64,','') AS DO1,
		dbo.fn_GetFileExtension(CAST(ma_proof AS VARCHAR(MAX))) as filetype
