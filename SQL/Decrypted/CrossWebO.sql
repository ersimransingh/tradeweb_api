...
BEGIN
	...
	Declare @DPID Varchar(500) =	(Select distinct STUFF((SELECT Distinct ',' + cm_companycode
               FROM Client_master  (nolock) Where isNull(cm_companycode,'') <> '' and Left(cm_companycode,2) not in ('IN') 
               FOR XML PATH('')), 1, 1, '')) 
...
CREATE   PROCEDURE [dbo].[stpr_CrossWebMenu] @dsXml AS XML = NULL 
WITH ENCRYPTION
AS
BEGIN
   ...
   from tbl_CrossWebMenu where MenuType = 'C'
   return
   END
   ELSE IF @strOption = 'Routes'
   BEGIN
   Select [name] = MenuName, [path] = [Path], element = MenuTag from tbl_CrossWebMenu Order By MenuCode
   return
   END
   ELSE IF @strOption = 'List'
   BEGIN
   select ModuleCode as 'id', MenuName as 'title', Path as 'path', '' as 'icon',
   submenu=Isnull((Select REPLACE((select ModuleCode as 'id', MenuName as 'title', [Path] as 'path', '' as 'icon' from tbl_CrossWebMenu b where b.ParentMenu=a.MenuName  order by MenuCode FOR JSON PATH),'\\','')),'')
   from tbl_CrossWebMenu a where a.MenuType='P'
   order by MenuCode
   END
END
GO