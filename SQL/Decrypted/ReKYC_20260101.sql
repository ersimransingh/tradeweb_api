...
BEGIN
 ...
 SET @o_vcErrorMessage = 'Process Completed'
  ...
  Document VARCHAR(MAX), FileType VARCHAR(50))
  ...
  BEGIN  
	...
	BEGIN
	  ...
	  BEGIN
		...
		FROM Client_ModifyAttach(NOLOCK) xx LEFT OUTER JOIN tbl_DocumentKRACKYCMapping(NOLOCK) on(CASE WHEN ma_field  IN('AadhaarAttachment','PANAttachment') THEN 'DocumentAttachment' 
	  ...
	  BEGIN
	    INSERT INTO @tbl_IDDocument(Client, FileName, IdProof, AddressProof, DocumentProof, DocumentNo, Document, FileType)
		 SELECT LTRIM(RTRIM(ma_cmcd)), LTRIM(RTRIM(@cx_Reference))+'_'+REPLACE(replace(substring(CASE WHEN isnull(CS_Description,CS_DataType) = '' THEN CS_DataType ELSE CS_Description END  ,1,15),' ',''),'/','')+'_'+CAST(ma_srno AS VARCHAR),
	    CS_DocTypeCode as IDProof, CS_KRACode as AddressProof, CS_DataTypeCd As DocumentCode,  DocNo = (CASE WHEN CS_Description LIKE '%PAN%' THEN 
        ISNULL((SELECT CM_PANNO FROM CLIENT_MASTER(NOLOCK) WHERE CM_cD = XX.ma_cmcd),XX.DocumentNo)
        WHEN CS_Description LIKE '%Aadhaar%' THEN ISNULL((SELECT CM_uid FROM CLIENT_info(NOLOCK) WHERE CM2_cD = XX.ma_cmcd),XX.DocumentNo) else XX.DocumentNo end),
	    CAST(ma_proof AS VARCHAR(MAX)) AS DO1,
		dbo.fn_GetFileExtension(CAST(ma_proof AS VARCHAR(MAX))) as filetype
...
BEGIN
  ...
  SELECT TOP 1 @strIDProof1 = IDProof, @strIDProofNo =  DocNo  FROM(
  SELECT  ma_cmcd = LTRIM(RTRIM(ma_cmcd)), 
  [FILENAME] = LTRIM(RTRIM(ma_cmcd))+'_'+REPLACE(replace(substring(CASE WHEN isnull(DocumentName,'') = '' THEN ma_filename ELSE DocumentName END  ,1,15),' ',''),'/','')+'_'+
  CAST(ma_srno AS VARCHAR),
  ISNULL(KRAIDCode,'16') as IDProof, ISNULL(KRAAddressCode,'32') as AddressProof,  DocNo = (CASE WHEN DocumentName LIKE '%PAN%' THEN 
  ISNULL((SELECT CM_PANNO FROM CLIENT_MASTER(NOLOCK) WHERE CM_cD = XX.ma_cmcd), XX.DocumentNo)
  WHEN DocumentName LIKE '%Aadhaar%' THEN ISNULL((SELECT RIGHT(ISNULL(CM_uid,''), 4) FROM CLIENT_info(NOLOCK) WHERE CM2_cD = XX.ma_cmcd),XX.DocumentNo) else XX.DocumentNo end)
  ...
  AND ma_refno = @strFormNo    
  AND ma_proof IS NOT NULL) X12
  WHERE ISNULL(IDProof,'') <> ''
  ORDER BY DOCNO DESC
	
  SELECT TOP 1 @strAddressProof1 = AddressProof, @strAddressProofNo1 = DocNo FROM(
  SELECT  ma_cmcd = LTRIM(RTRIM(ma_cmcd)), 
  [FILENAME] = LTRIM(RTRIM(ma_cmcd))+'_'+REPLACE(replace(substring(CASE WHEN isnull(DocumentName,'') = '' THEN ma_filename ELSE DocumentName END  ,1,15),' ',''),'/','')+'_'+
  CAST(ma_srno AS VARCHAR),
  ISNULL(KRAIDCode,'16') as IDProof, ISNULL(KRAAddressCode,'32') as AddressProof,  DocNo = (CASE WHEN DocumentName LIKE '%PAN%' THEN 
  ...
  BEGIN
    ...
    +'<APP_KYC_MODE>5</APP_KYC_MODE><APP_VER_NO>V29</APP_VER_NO><APP_VID_NO></APP_VID_NO>'
