...
*/
 ...
 -- INSERT INTO @tbl_Exchange
  ...
  BEGIN 
	...
	BEGIN
	  ...
	  BEGIN
		...
		[LTP] = CMP, [Net Value] = NET_VALUE, Booked = RelPL, [Notional] = UnRelPL, MTM
		FROM (
	    ...
	    MTM = (bc_amount)*-1, --AccountType = '', 
		RelPL = (ISNULL(bc_amount,0))*-1, UnRelPL = 0, Lookup = Exchange+Segment
	    FROM @tbl_billCharges) X1 ORDER BY ClientCode, TAG, SeriesDetails
  ...
  BEGIN
    ...
    +'<WebColumns>ClientCode,Client Name,Scrip,Series Name,Exchange,B/F Qty,B/F Rate,B/F Value,Buy Qty,Buy Value,Sell Qty,Sell Value,Excr Qty,'
    +'Excr Value,Assign Qty,Assign Value,Net Qty,LTP,Net Value,Booked,Notional,MTM</WebColumns><Headings>'
