...
BEGIN
	If(@masterName = 'State')
		Begin
			Select st_State Code, st_State Name from State_Master With (NoLock) Order by st_State
		End
	Else If(@masterName = 'Relation')
		Begin
			...
			IF @CrossDB <> ''
				Begin
					SET @string = ' Select rtrim(ltrim(cs_code)) Code, rtrim(ltrim(cs_desc)) Name  From  '+@CrossDB+'.'+@CrossOwner+'.'+'Clientsub_master (NoLock) where cs_module = ''CS19'''
					Exec (@string)
				End
		End	
END
...
//////////////////////////////////////////////////////////////////////////////////////////            
  */      
  ...
  ---- For Segment details
  /*SET @string = 'SELECT ( '
		+' CASE WHEN tag = ''Equity'' AND CES_Segment = ''Cash'' THEN  '
					+'''CapitalMarket'' WHEN tag = ''Equity'' AND CES_Segment '
				+' = ''F&O'' THEN ''FO'' WHEN tag = ''Equity'' AND CES_Segment = '
				+' ''Fx'' THEN ''CurrencyDerivatives'' WHEN tag = ''Equity'' '
				+' AND CES_Segment = ''MF'' THEN ''MF'' WHEN tag = ''Commodity''  '
				+' THEN ''Commodities'' ELSE ''Slb'' END '
		+' ) AS Segment, ''"'' + x1.ce_companycode + ''"'' AS '
	+' ce_companycode  , x1.ce_companycode as ExchVal '
  +' FROM ( '
	+' SELECT ''Equity'' AS tag, ce_clientcd, ce_companycode, '
		+' ce_regDt, SEG.CES_Name, SEG.CES_Segment '
	+' FROM Client_details(NOLOCK) C, '
		+' CompanyExchangeSegments(NOLOCK) SEG '
	+' WHERE C.ce_companycode = SEG.CES_Cd  And  ce_clientcd = '''+ @i_vcClientCode +''' '
	+'	UNION ALL ' 
	
	+' SELECT ''Commodity'' AS tag, ce_clientcd, ce_companycode '
		+' , ce_regDt, SEG.CES_Name, SEG.CES_Segment '
	+' FROM '+@strCommexConn+'.DBO.Client_details C, '
	+' '+@strCommexConn+'.DBO.CompanyExchangeSegments SEG '
	+' WHERE C.ce_companycode = SEG.CES_Cd  And  ce_clientcd = '''+ @i_vcClientCode +''' '
	+' ) X1 '
  +' WHERE ce_clientcd = '''+ @i_vcClientCode +''' ' + @WhereCondition 
  */

  SET @string = ' SELECT * FROM (SELECT ''Equity'' AS tag, ce_companycode = ISNULL(LTRIM(RTRIM(ce_companycode)),''''), CES_Segment = LTRIM(RTRIM(SEG.CES_Segment)) '
  ...
  +' UNION ALL '
  +' SELECT ''Commodity'' AS tag, ce_companycode = ISNULL(LTRIM(RTRIM(ce_companycode)),''''), CES_Segment = LTRIM(RTRIM(SEG.CES_Segment)) '
  +' ,CES_Exchange = LTRIM(RTRIM(CES_Exchange)), MainExchCode =  ISNULL(LTRIM(RTRIM(CES_Cd)),'''') '
  ...
  ----SET @i_oJsonValue = (Select * from @tbl_SegmentTemp FOR JSON PATH, root('Response'))
...
CREATE PROCEDURE [dbo].[stpr_APISendEmail] @i_vcInputJson VARCHAR(MAX), @CallingAPIURLMain VARCHAR(MAX),
@o_vcOutputJson NVARCHAR(MAX) OUTPUT 
WITH ENCRYPTION
AS
BEGIN
  SET @o_vcOutputJson = ''
...
--@@VAIBHAV/18-DEC-2023/
 ...
 ------------ ******  Reject Logic Start ***********
  ...
  BEGIN
	...
	UPDATE A SET A.ca_Tplus = iif(@i_vcApprovalFlag='R','R','Y')
	...
	BEGIN
      ...
      BEGIN 
	    ...
	    SET @o_vcOutputJsonEMAIL = ''
	    EXEC stpr_APISendEmail @i_vcInputJsonEMAIL, @strThirdPartyURL, @o_vcOutputJsonEMAIL OUTPUT
  ...
  BEGIN
    BEGIN TRY
	 ...
	 @strAuditStringDetail VARCHAR(MAX)='', @strPermanentAddressFlag VARCHAR(1)='N'
	 ...
	 DECLARE @ca_KeyFieldDescp VARCHAR(200)=''
     ...
     BEGIN 
		...
		BEGIN
		  SET @strClientMasterString = @strClientMasterString +' , '+@ca_field+' = '''+@ca_newValue+'''' 
        END
    ...
    DECLARE @strClosuredpString VARCHAR(MAX)=''	
	  ...
	  BEGIN
		...
		BEGIN
		  SET @string = ' INSERT INTO  '+@CrossDB+'.'+@CrossOwner+'.'+'Client_Modification (ca_cmcd, ca_field, ca_oldvalue, ca_newvalue, '
            ...
            +' ''N'','''','''',Null,0,''Close Account'','''+CAST(@i_vcRefNo AS VARCHAR)+''', CONVERT(VARCHAR,GETDATE(), 112),'''',''C'',''S'',''1'',''99'',''N'','''','
            +' '''+@StrClosureReason+''',CONVERT(VARCHAR,GETDATE(), 112),'''','''',CONVERT(VARCHAR,GETDATE(), 108)) '
		...
		BEGIN
          ...
          +' ca_newValue = (CASE WHEN TableColumn = ''cm_divbranchno'' THEN (CASE WHEN X11.ca_newValue = ''CA'' THEN ''11'' '
		...
		BEGIN
		  ...
		  BEGIN
		    SET @string = ' INSERT INTO  '+@CrossDB+'.'+@CrossOwner+'.'+'Client_Modification(ca_cmcd, ca_field, '
		     ...
		     +' when ca_field = ''cm_state'' then ''cm_pstate'' else ca_field end, '
	...
	BEGIN
	  ...
	  SELECT @IserialNo = MAX(CK_SRNO)+ 1 
	  ...
	  SELECT @IserialNo, CK_Panno = cm_panno, CK_MaidenPrefix = 'Mrs', CK_MaidenFname = '', CK_MaidenMName = '', 
      ...
      Ck_Filler1 = '', Ck_Filler2 = '', Ck_Filler3 = '', Ck_NFiller1 = 0, Ck_Nfiller2 = 0, Ck_NFiller3  = 0
      ...
      AND ma_field IN('IDAttachment','PhotoAttachment','SignAttachment','AadhaarAttachment')) X1, CLIENT_MASTER(NOLOCK) CM
   ...
   SET @o_JsonOutput = (SELECT RefNo AS ReqRefNo, ca_cmcd ClientCode, Rtrim(LTrim(cm_name)) ClientName, Replace(convert(VARCHAR(12    
    ...
    FROM (    
     ...
     GROUP BY ca_cmcd, cm_name, ca_date, SUBSTRING(ltrim(ca_field), 1, 2), SUBSTRING(ltrim(    
        ca_field), 1, 7), SUBSTRING(ltrim(ca_field), 1, 3), Left(ca_desc, 7)  , C.mkrdt   
     ) A    
    GROUP BY ca_cmcd, cm_name, ca_date, RefNo   ,CheckerDate
...
BEGIN
  ...
  BEGIN
	...
	BEGIN
      BEGIN TRY
        SET @strString = 'SELECT SerialNo, ColumnName, ColumnValue, ValueTypeColumn, UpdateFlag, MasterTag, JsonLevel, MasterLevel  FROM '+@strtradeplustempdb+'.dbo.FN_JSONCUTTER('''+@i_vcOldJsonString+''')'
        INSERT INTO #TBL_OldJson(SerialNo, ColumnName, ColumnValue, ValueTypeColumn, UpdateFlag, MasterTag, JsonLevel, MasterLevel)
	    EXEC(@strString)
      END TRY
      BEGIN CATCH
        SET @o_vcErrorFlag = 'E'
	...
	BEGIN
	  SET @StrQuery = 'SELECT SerialNo,ColumnName,ColumnValue,ValueTypeColumn,UpdateFlag,MasterTag,JsonLevel,MasterLevel '
      +' FROM '+@strtradeplustempdb+'.DBO.fn_JsonCutter('''+@i_vcJsonString+''') '
  ...
  BEGIN
     ...
     BEGIN
       ...
       SET @SQueryValidationQuery = REPLACE(@SQueryValidationQuery,TRIM('<<'+TRIM(@SQuerydtlFieldName)+'>>'),''+@SQuerydtlcolumnvalue+'')
       FETCH NEXT FROM CurqueryValidationdtl INTO @SQuerydtlFieldName, @SQuerydtlcolumnvalue
     END
  ...
  BEGIN
    ...
    SELECT @WrongID = @WrongID+'|'+ca_newValue from client_ModifyAPI(NOLOCK) X  WHERE ca_cmcd  = @i_vcClientCode 
	...
	SELECT @WrongID = @WrongID+'|'+ba_micr+','+ba_ifsccode FROM @tbl_Bankmicr  X
...
@o_vcJsonOutput VARCHAR(MAX) OUTPUT, @i_vcTemplateCode VARCHAR(20) = 'Template1', @i_vcProcessTag VARCHAR(1)='G' WITH ENCRYPTION  AS
BEGIN
  set @o_vcErrorFlag = 'S'
  SET @o_vcErrorMessage = 'Process Executed'
  SET @o_vcJsonOutput = ''
  ...
  BEGIN
	...
	AND CK_SRNO IN(SELECT MAX(CK_SRNO) FROM Client_CKYC WHERE CK_Panno = CXXX.CK_Panno))
		...
		And rm_refno >= (Select Isnull(Max(rm_refno),0) From Client_ReKycMain(NOLOCK) where rm_cmcd = @i_vcClientCode)) FOR JSON PATH),
	    ---- FOR ReKyc HEADER
		...
		PersonalDetails = (
				SELECT ClientCode = ltrim(rtrim(Cm_cd)), 
					...
					FirstName = ltrim(rtrim((
								CASE WHEN CHARINDEX(' ', cm_name) > 0 THEN 
											SUBSTRING(cm_name, 1, CHARINDEX(' ', 
													cm_name) - 1) ELSE cm_name END
								))), MiddleName = ltrim(rtrim((
								CASE WHEN CHARINDEX(' ', cm_name) = 0 THEN '' ELSE 
										RTRIM(LTRIM(REPLACE(REPLACE(cm_name, 
														SUBSTRING(cm_name, 1, CHARINDEX(' ', 
																cm_name) - 1), ''), REVERSE(LEFT(
															REVERSE(cm_name), CHARINDEX(' ', 
																REVERSE(cm_name)) - 1)), ''))) END
								))), LastName = ltrim(rtrim((
								CASE WHEN CHARINDEX(' ', cm_name) > 0 THEN 
											REVERSE(LEFT(REVERSE(cm_name), CHARINDEX
													(' ', REVERSE(cm_name)) - 1)) ELSE '' END
					...
					cm_grossincome, [IncomeDate] =TRIM(INFO.cm_grossincomedt), [PermanentAddressFlag] = 'N'
				...
				SELECT Top 3 [BankAccNo] = ltrim(rtrim(ba_actno)), 
					...
					= ltrim(rtrim(ba_acttype)), [BankID] = ROW_NUMBER() OVER(ORDER BY ba_actno ASC),  
					[IsBankDeleted] = '', BankAttachment = ''
				FROM Bankact x, Bank_master mas
				WHERE ba_clientcd = @vcClientCode AND x.ba_micr = mas
					.bk_micr AND x.ba_ifsccode = mas.bk_IFCCode
				...
				---- For Demat details
				[DematDetails] = (
				SELECT TOP 3 [DematId] = ROW_NUMBER() OVER(ORDER BY da_clientcd ASC), [DPAcNo] = ltrim(rtrim(
					...
					[IsDematDeleted] = 'false', DematAttachment = ''
				...
				DormantStatus = (CASE WHEN cm_freezeyn in('','N') THEN 'Active' ELSE '' END)  
                FROM client_master(NOLOCK) WHERE CM_CD = @vcClientCode  FOR JSON PATH),
				[Attachments] = (select AddressAttachment='', AddressProofType = '',
				IncomeAttachment = '', IncomeProofType = '', 
				SegmentAttachment = '',SegmentProofType = '', SignatureAttachment= ''
        FOR JSON PATH),
  ...
  BEGIN
    ...
    SELECT @strSegment = @strSegment+','+ IIF(@i_vcProcessTag = 'G',trim(CES_Exchange)+'-'+trim(CES_Segment)+' ['+ce_companycode+']',ce_companycode) 
    ...
    SET @o_vcJsonOutput = (SELECT ClientCode = trim(CM_cd), ClientName = ltrim(rtrim(cm_name)), 
	...
	Mobile = trim(cm_mobile), Email = trim(cm_email), DateofBirth = 
	...
	MaritalStatus = IIF(@i_vcProcessTag = 'G', CASE WHEN cm_maritalstatus = 'M' THEN 'Married ['+trim(cm_maritalstatus)+']'
	WHEN cm_maritalstatus = 'S' THEN 'Single ['+trim(cm_maritalstatus)+']'
	WHEN cm_maritalstatus = 'W' THEN 'Widow ['+trim(cm_maritalstatus)+']'
	WHEN cm_maritalstatus = 'D' THEN 'Divorce ['+trim(cm_maritalstatus)+']'
	WHEN cm_maritalstatus = 'NA' THEN 'Not Applicable ['+trim(cm_maritalstatus)+']' ELSE 'Not Applicable ['+trim(cm_maritalstatus)+']' END,cm_maritalstatus) , 
	...
	CKYCNumber = ISNULL(Ck_Nfiller1, 0), CKYCDate = ISNULL(ckyc.mkrdt, ''), CKYCReffNo = ISNULL(
		...
		SUBSTRING(cn_filler9, CHARINDEX('~', cn_filler9) + 1, CHARINDEX('~', cn_filler9, CHARINDEX('~', cn_filler9
			...
			CHARINDEX('~', cn_filler9, CHARINDEX('~', cn_filler9) + 1) + 1, LEN(cn_filler9) - CHARINDEX('~', REVERSE(
					...
					BankPincode = ISNULL(BankPincode,''), BankIFSC = ISNULL(trim(BankIFSC),''), 
					BankAccNo = ISNULL(trim(BankAccNo),''), BankMICR = ISNULL(trim(BankMICR),''), 
...
BEGIN
  ...
  BEGIN TRY
    SET @strString = 'SELECT SerialNo, ColumnName, ColumnValue, ValueTypeColumn, UpdateFlag, MasterTag, JsonLevel, MasterLevel  FROM '+@strtradeplustempdb+'.dbo.FN_JSONCUTTER('''+@i_vcNewJsonString+''')'
	INSERT INTO #TBL_NewJson(SerialNo, ColumnName, ColumnValue, ValueTypeColumn, UpdateFlag, MasterTag, JsonLevel, MasterLevel)
	exec(@strString)
  END TRY
  BEGIN CATCH
    SET @o_vcErrorFlag = 'E'
  ...
  BEGIN
	...
	THEN CASE WHEN n.ColumnValue  = 'SAVINGS' THEN 'SB' ELSE n.ColumnValue 
  ...
  NewValue = ISNULL((CASE WHEN N.ColumnName ='AccountType' 
  THEN CASE WHEN n.ColumnValue  = 'SAVINGS' THEN 'SB' ELSE n.ColumnValue END ELSE n.ColumnValue END),''), 
  ...
  */
  ...
  SELECT @CommexDB = TRIM(OP_DataBase) FROM Other_Products(NOLOCK) WHERE OP_Product = 'Commex'
 ...
 IF @strOldValue <> '' OR @strNewValue <> ''
  BEGIN
    ...
    +' SELECT VALUE AS Segment FROM DBO.Returntable('''+@strOldValue+''','','')) OLD1  , CompanyExchangeSegments SEG (NOLOCK) '
    +' WHERE NOT EXISTS(SELECT 1 FROM( '
    ...
    +' SELECT VALUE AS Segment FROM DBO.Returntable('''+@strNewValue+''','','')) OLD1  , CompanyExchangeSegments SEG (NOLOCK) '
    +' WHERE NOT EXISTS(SELECT 1 FROM( '
...
--      SET @o_vcOutPutJSON = '[' + @o_vcOutPutJSON + ']'
    ...
    BEGIN
      ...
      BEGIN
        ...
        SET @o_vcOutPutJSON ='[{"ClientCode":"##ClientCode##","Status":"##Status##","Remark":"##ErrorMessage##"}]'
        SET @o_vcOutPutJSON = REPLACE(@o_vcOutPutJSON,'##ClientCode##', @strClientCode)
        SET @o_vcOutPutJSON = REPLACE(@o_vcOutPutJSON,'##ErrorMessage##', @o_vcErrorMessage)
...
--      SET @o_vcOutPutJSON = '[' + @o_vcOutPutJSON + ']'
    ...
    BEGIN
	  IF EXISTS(SELECT 1 FROM Client_ReKycMain(NOLOCK) WHERE rm_cmcd = @strClientCode AND RM_DESC = 'Only Closure' AND rm_status = 'Pending'
	  AND rm_rekyc = 'N')
	  BEGIN
	    SET @o_vcOutPutJSON = '{}'
		SET @o_vcErrorMessage = 'Closure request already available'
	    SET @o_vcOutPutJSON ='[{"Status":"N","Remark":"' + @o_vcErrorMessage + '","Data":' + @o_vcOutPutJSON + '}]'
		RETURN 1
	  END
	 ...
	 INSERT INTO Client_ReKycMain(rm_cmcd, rm_cdate, rm_ctime, rm_rekyc, rm_status, rm_desc, mkrdt, mkrtm, rm_step, rm_refno)
     VALUES(@strClientCode, CONVERT(VARCHAR,GETDATE(),112),CONVERT(varchar, getdate(), 108),'N','Pending','Only Closure',CONVERT(VARCHAR,GETDATE(),112),'',1,@vc_RefNo)
   ...
   BEGIN
	 ...
	 SET @o_vcOutPutJSON = ''
	 SET @o_vcErrorMessage = ''
     EXEC stpr_GetMarkerDataOnlyClosure @o_vcOutPutJSON OUTPUT
     SET @o_vcOutPutJSON='[{"Status":"Y","RefNo":"##RefNo##","Remark":"' + @o_vcErrorMessage + '","Data":' + @o_vcOutPutJSON + '}]'
	 SET @o_vcOutPutJSON=REPLACE(@o_vcOutPutJSON,'##RefNo##',CAST(@vc_RefNo AS VARCHAR))
     RETURN 1
   END
 ...
 --- Variable Declaration Start
  ...
  BEGIN
	...
	BEGIN
	  ...
	  BEGIN TRY
        SET @strString = 'SELECT SerialNo, ColumnName, ColumnValue, MasterTag,JSONLEVEL, MASTERLEVEL FROM '+@strtradeplustempdb+'.DBO.FN_JSONCUTTER('''+@o_vcOutputJsonapi+''')'
	  ...
	  BEGIN 
		...
		BEGIN
          ...
          SET @strString = 'SELECT SerialNo, ColumnName, ColumnValue, MasterTag,JSONLEVEL, MASTERLEVEL FROM '+@strtradeplustempdb+'.DBO.FN_JSONCUTTER('''+@stroutpuxmlFile+''')'
		  INSERT INTO @tbl_jsonoutput(SerialNo, ColumnName, ColumnValue,  MasterTag, JSONLEVEL, MASTERLEVEL)
		  EXEC(@strString)
  ...
  IF @i_vcFunctionName = 'EsignRequest'              
  BEGIN
    SELECT @strClientCode = Columnvalue FROM @tbl_InputJSONTable WHERE ColumnName='ClientCode'
    SET @strRefNo = @i_vcRefNo
	...
	BEGIN
	  ...
	  AND ma_filename = 'UnSignedPdf')
	  BEGIN
	    INSERT INTO Client_ModifyAttach(ma_cmcd, ma_date, ma_filename, ma_field, mkrdt, mkrtm, ma_proof, 
		ma_refno, ma_Nfiller1, ma_status)
		VALUES(@strClientCode, CONVERT(VARCHAR,GETDATE(),112), 'UnSignedPdf', 'ReKyc UnSigned Pdf',
		CONVERT(VARCHAR,GETDATE(),112), CONVERT(VARCHAR,GETDATE(), 108),@binaryRepresentation, @strRefNo, 1, 'N')
	  END
      ELSE
      BEGIN
  ...
  END	
  ELSE
  IF @i_vcFunctionName = 'CreateSign'              
  BEGIN
  ...
  END	
  ELSE
  IF @i_vcFunctionName = 'GetEsignDocument'              
  BEGIN
    SELECT @strClientCode = Columnvalue FROM @tbl_InputJSONTable WHERE ColumnName='ClientCode'
    SET @strRefNo = @i_vcRefNo
	SELECT @strRequestid = Columnvalue FROM @tbl_InputJSONTable WHERE ColumnName='Requestid'
	  ...
	  BEGIN
		...
		BEGIN
		  ...
		  BEGIN
		    INSERT INTO Client_ModifyAttach(ma_cmcd, ma_date, ma_filename, ma_field, mkrdt, mkrtm, ma_proof, 
		    ma_refno, ma_Nfiller1, ma_status)
		    VALUES(@strClientCode, CONVERT(VARCHAR,GETDATE(),112), 'SignedPdf', 'Esign Sign PDF Document Output for Request Id',
		    CONVERT(VARCHAR,GETDATE(),112), CONVERT(VARCHAR,GETDATE(), 108),@binaryRepresentation, @strRefNo, 1, 'N')
...
@o_vcOutputJson NVARCHAR(MAX) OUTPUT, @strHeaderVal VARCHAR(MAX)='', @strTokenVal VARCHAR(MAX)='' AS
WITH ENCRYPTION
BEGIN
  set @o_vcOutputJson = ''
  ...
  SET @RequestJSON = REPLACE(@RequestJSON,'##FILE##',@strFile)
  SET @RequestJSON = REPLACE(@RequestJSON,'##FILENAME##',@strFileName)  
  SET @RequestJSON = REPLACE(@RequestJSON,'##FILEPASSWORD##',@strFilePassword) 
  SET @RequestJSON = REPLACE(@RequestJSON,'##FileFormat##',@StrFileFormat) 
...
CREATE   PROCEDURE [stpr_GetMarkerDataOnlyClosure](@o_vcJsonOutput VARCHAR(MAX) OUTPUT) WITH ENCRYPTION AS
BEGIN
  DECLARE  @o_vcErrorFlag VARCHAR(1)='', @o_vcErrorMessage VARCHAR(MAX)=''
  
  SELECT ClientCode = rm_cmcd,   ClientName = cm_name, LastTradingDate = CAST('' AS VARCHAR(50)), 
  TradingLedgerBalance = CAST(0 AS money),
  DPLedgerBalance = CAST(0 AS money), DPHoldingValue = CAST(0 AS money), RequestDate = rm_cdate, RequestTime = rm_ctime,
  RequestType = CAST('' AS VARCHAR(50)), RequestReason = CAST('' AS VARCHAR(500)), rm_refno
  INTO #TBL_ClosureData
  FROM Client_ReKycMain(NOLOCK) cm, Client_master(NOLOCK) where rm_rekyc = 'N'
  AND rm_status = 'Pending' AND rm_desc = 'Only Closure'
  ...
  BEGIN 
	 ...
	 BEGIN
	    ...
	    SET @strString = 'SELECT SerialNo, ColumnName, ColumnValue, ValueTypeColumn, UpdateFlag, MasterTag, JsonLevel, MasterLevel  FROM '+@strtradeplustempdb+'.dbo.FN_JSONCUTTER('''+@o_vcJsonOutput+''')'
	   	INSERT INTO #TBL_JsonCutter(SerialNo, ColumnName, ColumnValue, ValueTypeColumn, UpdateFlag, MasterTag, JsonLevel, MasterLevel)
	   	exec(@strString)
  ...
  BEGIN
  SET @o_vcJsonOutput = (SELECT ClientCode, ClientName, LastTradingDate, TradingLedgerBalance, DPLedgerBalance, DPHoldingValue, RequestDate, RequestTime, 
  RequestType = CASE WHEN RequestType = 'T' THEN 'Trading' WHEN RequestType = 'D' THEN 'Demat' 
  WHEN RequestType = 'B' THEN 'Trading+Demat' else '' end, RequestReason, rm_refno FROM #TBL_ClosureData 
  ...
  BEGIN
	...
	RequestType = '', RequestReason = '', rm_refno = '' FOR JSON PATH)
  End
...
BEGIN
  ...
  @StrCompanyCode VARCHAR(1)='A', @vcXML nVARCHAR(MAX), @strString Nvarchar(max)='', @xmldata XML , 
  ...
  IF @strExists > 0
  BEGIN
    SET @strLedgerJson = (SELECT [TradingLedgerBalance] = IIF(Segment='Trading',LedgerBalance,0),
    [DPLedgerBalance] = IIF(Segment='DP',LedgerBalance,0) FROM @tbl_LedgerBalance  FOR JSON PATH )
  END
  ELSE
  BEGIN
...
BEGIN
  ...
  SELECT X.ca_cmcd,  X.ca_Nfiller3, ca_filler1 as JsonTag, X1.JsonKey, ca_oldValue, ca_newValue, ca_Nfiller1, ca_filler3, x1.FieldDescp
  ...
  SET @o_JsonOutput = (select distinct JSONTAG ,
  ...
  STUFF(replace(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(OldValue,'ABC','BSE-CASH'),'ANC','NSE-CASH'),
  'ANF','NSE-F&O'),'ANK','NSE-FX'),'ABM','BSE-MF'),'ANM','NSE-MF'),'AMF','MCX-Comm'),',,',','),',,',','), 1, 1, ''),
  OldValue),
  NewValue = IIF(IIF(FieldDescp='',JsonKey, FieldDescp) = 'Segment Details',
  STUFF(replace(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(NewValue,'ABC','BSE-CASH'),'ANC','NSE-CASH'),
  'ANF','NSE-F&O'),'ANK','NSE-FX'),'ABM','BSE-MF'),'ANM','NSE-MF'),'AMF','MCX-Comm'),',,',','),',,',','), 1, 1, ''),
  NewValue)
  FROM @tbl_JsonMatch WHERE JsonTag = xdetail.JsonTag and  ca_Nfiller1 = xdetail.ca_Nfiller1 ) XM123
  FOR JSON PATH),
  [Attachment] = (select ma_field = (CASE WHEN ma_field = 'AadhaarAttachment' THEN 'Aadhaar Attachment'
  ...
  WHEN ma_field = 'SegmentAttachment' THEN 'Segment Attachment' ELSE ma_field END),  ma_filename,  ma_proof = dbo.fnBinaryToBase64(DBO.FN_base64toBinary(CAST(CONVERT(varchar(MAX), REPLACE(ma_proof,'data:image/jpeg;base64,','')) AS NVARCHAR(MAX))))
  FROM  Client_ModifyAttach(NOLOCK) WHERE  ma_cmcd = @i_vcClientCode AND ma_refno = @i_vcRefNo
  and ma_field like '%Attachment' 
  AND ((ma_Nfiller1 = xdetail.ca_Nfiller1  and XMAIN.JsonTag not in('PersonalDetails','SegmentDetails','DisplaySegment')) or  
  XMAIN.JsonTag in('PersonalDetails','SegmentDetails','DisplaySegment'))
  ...
  or (XMAIN.JsonTag = 'DematDetails' and ma_field in('DematAttachment'))) FOR JSON PATH)
  FROM @tbl_JsonMatch  xdetail where JsonTag =  XMAIN.JsonTag FOR JSON PATH)
  FROM @tbl_JsonMatch XMAIN  FOR JSON PATH)
  
  SET @o_JsonOutput = substring(@o_JsonOutput, 2, len(@o_JsonOutput) - 2)
 
  DECLARE @tbl_fn_json_merge TABLE(JsonValue VARCHAR(MAX))
  
  DELETE FROM @tbl_fn_json_merge
  
  DECLARE @mainjson VARCHAR(MAX)= ''
  ...
  --SET @o_JsonOutput = JSON_MODIFY(@o_JsonOutput, '$[0].MainInfo', JSON_QUERY(@mainjson))
  
  DECLARE @Attch VARCHAR(MAX)=''
  SET @Attch = (SELECT FileDescp = case when ma_filename = 'RPD_GetResponse' then 'BankDetailFile'
  when ma_filename = 'SignedPdf' then 'Final Signed Document'
  when ma_filename = 'UnSignedPdf' then 'Customer UnSign PDF' 
  when ma_filename = 'Sign' then 'Signature Proof' else ma_filename end,  
  ...
  FROM Client_ModifyAttach(NOLOCK) WHERE ma_cmcd = @i_vcClientCode AND ma_refno = @i_vcRefNo 
  and ma_filename in('Sign','RPD_GetResponse','SignedPdf') 
  FOR JSON PATH, root('Attachment2'))
  
  --SET @mainjson = substring(@mainjson, 2, len(@mainjson) - 2)
  
  SET @strString = 'SELECT '+@strtradeplustempdb+'.DBO.fn_json_merge('''+@mainjson+''','''+@Attch+''') '
  INSERT INTO @tbl_fn_json_merge(JsonValue)
  EXEC(@strString)
  
  SELECT @mainjson = JsonValue FROM @tbl_fn_json_merge
  DELETE FROM @tbl_fn_json_merge
  
  SET @strString = 'SELECT '+@strtradeplustempdb+'.DBO.fn_json_merge('''+@o_JsonOutput+''','''+@mainjson+''') '
  INSERT INTO @tbl_fn_json_merge(JsonValue)
  EXEC(@strString)
  
  SELECT @o_JsonOutput = JsonValue FROM @tbl_fn_json_merge
  
  DELETE FROM @tbl_fn_json_merge
  
  --SET @o_JsonOutput = DBO.fn_json_merge(@o_JsonOutput,@mainjson)  
  RETURN 1
END
GO
