...
BEGIN
  ...
  SELECT X.ca_cmcd,  X.ca_Nfiller3, ca_filler1 as JsonTag, X1.JsonKey, CASE WHEN x1.FieldDescp like '%Date%' THEN CONVERT(VARCHAR,CAST(ca_oldValue AS DATE),103)
  ELSE ca_oldValue END, CASE WHEN x1.FieldDescp like '%Date%' THEN CONVERT(VARCHAR,CAST(ca_newValue AS DATE),103)
...
BEGIN
  ...
  BEGIN CATCH                                  
    ...
    RETURN 1                                   
  END CATCH
  DECLARE @jsonExport1 VARCHAR(MAX)='', @strClient VARCHAR(500)=''
  ...
  BEGIN
	...
	BEGIN
	   SET @jsonExport1 = (SELECT CASE WHEN APIVendorName = 'CVLKRA' AND APIName = 'InsertUpdateKYCRecord' THEN 'CVL-KRA (Modification)' ELSE APIVendorName+' - '+APIName END
	   as [DisplayValue], APIVendorName+' - '+APIName as [FieldValue] 
	   FROM tbl_VendorAPISetting(NOLOCK)
       WHERE APIVendorName+' - '+APIName IN(SELECT SP_SYSVALUE FROM WebParameter WHERE sp_parmcd ='APIEXPORT')
	...
	BEGIN
	  ...
	  BEGIN
        ...
        BEGIN
		  ...
		  UPDATE A SET A.Ck_NFiller2 = 0, A.Ck_Status = 'Y'
          FROM Client_CKYC A
	...
	BEGIN
	   ...
	   BEGIN
		 ...
		 BEGIN
           ...
           AND SerialNo in(Select max(SerialNo) from tbl_GenericAPIDebugLog(NOLOCK) xg WHERE RequestSource = 'CVLKRA'  and RequestUniqueID = xg.RequestUniqueID)),'Pending')
           ...
           AND Ck_NFiller2 = 0 AND Ck_Status = 'Y'))
           AND rm_rekyc ='Y'
           AND RM_CMCD = cm_cd
		 ...
		 IF @strOptionType IN('Approved','Re-Generate')
		 BEGIN
           ...
           AND SerialNo in(Select max(SerialNo) from tbl_GenericAPIDebugLog(NOLOCK) xg WHERE RequestSource = 'CVLKRA'  and RequestUniqueID = xg.RequestUniqueID)) X11 ON
		   (RequestUniqueID = CM_CD)
           WHERE rm_RequestType ='ReKYC'
           AND RM_CMCD IN(SELECT CM_CD FROM CLIENT_MASTER WHERE cm_panno IN(
           SELECT CK_Panno  FROM Client_CKYC(NOLOCK) CM  
           WHERE CK_SRNO IN(SELECT MAX(CK_SRNO) FROM Client_CKYC(NOLOCK) WHERE CK_Panno = CM.CK_Panno)
           AND Ck_NFiller2 > 0 AND Ck_Status = 'S'))
           AND rm_rekyc ='Y'
           AND RM_CMCD = cm_cd
           AND x.rm_refno in(select max(rm_refno) from Client_ReKycMain(NOLOCK) where rm_RequestType ='ReKYC'
           AND rm_cmcd = x.rm_cmcd)
