 ...
 BEGIN    
   SET @o_JsonOutput = (SELECT RefNo AS ReqRefNo, ca_cmcd ClientCode, Rtrim(LTrim(cm_name)) ClientName, Replace(convert(VARCHAR(12    
     ...
     PersonalIncome, CASE WHEN Sum(TotalEmail) > 0 THEN 'Yes' ELSE 'No' END PersonalEmail, CASE     
	  ...
	  [RejectionReason],
	  [CurrentStatus] =  CASE WHEN isnull(DormantStatus,'') = '' 
	  then ISNULL((SELECT CASE WHEN cm_freezeyn in('','N') THEN 'Active'  When cm_freezeyn='Y' Then 'Freeze for Trades' ELSE '' END
	  FROM CLIENT_MASTER(NOLOCK) WHERE CM_cD = ca_cmcd),'')  else isnull(DormantStatus,'')  end ,
	  [NewStatus] =  'Active'
    FROM (    
     SELECT Isnull(Max(ca_Nfiller3), 1000) AS RefNo, ca_cmcd, cm_name, ca_date, C.Mkrdt as CheckerDate, (    
	   ...
	   [DormantStatus] = ISNULL((CASE WHEN LTRIM(RTRIM(ca_field)) = 'DormantStatus' THEN ca_oldvalue ELSE '' END),'')
    ...
    GROUP BY ca_cmcd, cm_name, ca_date, RefNo   ,CheckerDate, [RejectionReason], isnull(DormantStatus,'') 
