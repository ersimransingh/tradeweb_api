   ...
   BEGIN
	...
	SET @CommanXMLString = REPLACE(@CommanXMLString,'##ReportDisplay##',@strReportDisplay)
	 ...
	 BEGIN
	   ...
	   END
	   ELSE IF CHARINDEX('Trading',@strLReportType) > 0 
	   BEGIN
...
*/
 --- Parameter Declaration
  ...
  DECLARE @strString NVARCHAR(MAX)='',
	...
	@strCompanyCode VARCHAR(1)='A', @XMLData XML,  @strUserId VARCHAR(50)=''
  ...
  BEGIN
	...
	BEGIN
	  ...
	  AND OP_Status='A'
	END
	declare @strSFromDate VARCHAR(11)='', @strSToDate VARCHAR(11)='', @strClient VARCHAR(20)='',@strClient1 VARCHAR(20)=''
 ...
 --SELECT @i_vcProjectCode, @i_vcReportCode, @i_vcReportCategory,@vcProcedureName
  ...
  BEGIN TRY
	...
	@menuname1 VARCHAR(100)='', @Exchange VARCHAR(100)=''
...
BEGIN 
  ...
  DECLARE @StrRequestFrom VARCHAR(1)='M', @strOption VARCHAR(50)='', @strModuleName VARCHAR(50), @strEntryName VARCHAR(100)='', @strUserid VARCHAR(100)=''
  ...
  BEGIN
    ...
    BEGIN
	  ...
	  "tableData":'+CASE WHEN @strOption in('Master_Edit','ChildEntry_Edit') THEN '"~~'+GroupName+'~~"' ELSE '[]' END +',"Data":'+data1 from(
      ...
      where TEMPLATENAME = @strEntryName ) x111 FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, '') + ']' 
    END
...
BEGIN
  ...
  BEGIN
	...
	EXEC stpr_APISendEmail @i_vcInputJsonPDF, @strThirdPartyURL, @o_vcOutputJsonPDF OUTPUT
...
CREATE PROCEDURE stpr_APISendEmail @i_vcInputJson VARCHAR(MAX), @CallingAPIURLMain VARCHAR(MAX),
...
BEGIN 
  ...
  DECLARE @TBL_json TABLE(SerialNo int, Jsonvalue VARCHAR(MAX), ParentName VARCHAR(MAX), TargetForm VARCHAR(MAX))
  ...
  INSERT INTO @TBL_json(SerialNo, Jsonvalue, ParentName, TargetForm)
  ...
  [iconType] = CASE WHEN Menutag = 'Ledger' THEN 'fontawesome5'   
   ...
   [autoFetch] = CASE WHEN ISNULL(@strUserType,'Client') = 'Branch' THEN 'false' ELSE  'true' END,
   ...
   WHERE XMAIN.SrNo = XXX.SrNo AND ISNULL(Enable,'N') = 'Y' FOR JSON PATH), ParentMenu, ISNULL(TargetForm,'') 
  ...
  and charindex('##'+columnname+'##',CASE WHEN @strUserType <> 'CLIENT' AND ISNULL(XX.BranchFilters,'') <> '' THEN XX.BranchFilters ELSE  xx.filters
  END)>0 AND DBQueryType ='Q'
  AND dbquery <> ''
  ...
  SET DATEFORMAT MDY
   ...
   BEGIN
     ...
     --END), 
	   ...
	   T112AFlag, [A112A_Rate] = ISNULL(Tmp_112ARate,''), [STT] = (SaleQty+BuyQty)*td_stt, Tmp_RefNo, td_Filler1 
       FROM @tbl_HoldingRep A, Client_master(NOLOCK) M, Securities(NOLOCK) SC  
...
*/
  ...
  ELSE IF @strModuleName NOT IN('FundRequest','OffLineMarginPledge')
  BEGIN
...
BEGIN
	...
	@strFromDate VARCHAR(12)='', @strSettlementNo VARCHAR(40)=''
...
BEGIN
	...
	DECLARE @o_ParameterList VARCHAR(max) = '', @o_ParameterListxml XML;
	DECLARE @strModuleName VARCHAR(50) = '', @strOption VARCHAR(50) = '', @strUserid VARCHAR(50)='',
	@strUserType VARCHAR(10), @strSelection varchar(1)='C'
	...
	IF ISNULL(@strGROUPLOGIN,'') = 'Group' AND ISNULL(@strUserType,'user') IN('user','') AND ISNULL(@userla_Group,'') = ''
	BEGIN
	  INSERT INTO @tbl_UserList
	  SELECT cm_cd FROM CLIENT_MASTER(NOLOCK)
	  WHERE cm_groupcd IN(SELECT DISTINCT cm_groupcd 
	  FROM CLIENT_MASTER(NOLOCK) 
	  WHERE CM_cD IN(SELECT ClientCode FROM @tbl_UserList))
	END  
	
		
	DECLARE @strType VARCHAR(50) = @strSelection
	...
	BEGIN
	  IF ISNULL(@userla_Group,'') <> 'A'
	  BEGIN
		SELECT DISTINCT LTRIM(RTRIM(bm_branchcd)) AS [Value], bm_branchname + ' [' + bm_branchcd + ']' AS [DisplayName]
		FROM branch_master WITH (NOLOCK)
		WHERE bm_branchcd <> ''
		AND bm_branchcd IN(SELECT DISTINCT cm_brboffcode FROM CLIENT_MASTER(NOLOCK) XMAIN, @tbl_UserList
		WHERE ClientCode = XMAIN.CM_CD
		AND cm_schedule = 49843750 )
	  END
      BEGIN
	    SELECT DISTINCT LTRIM(RTRIM(bm_branchcd)) AS [Value], bm_branchname + ' [' + bm_branchcd + ']' AS [DisplayName]
		FROM branch_master WITH (NOLOCK)
		WHERE bm_branchcd <> ''
		AND bm_branchcd IN(SELECT DISTINCT cm_brboffcode FROM CLIENT_MASTER(NOLOCK) XMAIN
		WHERE cm_schedule = 49843750 )
      END	  
	END
	ELSE IF @strType = 'G'
	BEGIN
	  IF ISNULL(@userla_Group,'') <> 'A'
	  BEGIN
		SELECT DISTINCT gr_cd AS [Value], gr_desc + ' [' + gr_cd + ']' AS [DisplayName]
		FROM Group_master WITH (NOLOCK)
		WHERE gr_cd IN(SELECT DISTINCT cm_groupcd FROM CLIENT_MASTER(NOLOCK) XMAIN, @tbl_UserList
		WHERE ClientCode = XMAIN.CM_CD
		AND cm_schedule = 49843750 )
	  END	
	  ELSE
	  BEGIN
		SELECT DISTINCT gr_cd AS [Value], gr_desc + ' [' + gr_cd + ']' AS [DisplayName]
		FROM Group_master WITH (NOLOCK)
		WHERE gr_cd IN(SELECT DISTINCT cm_groupcd FROM CLIENT_MASTER(NOLOCK) XMAIN
		WHERE cm_schedule = 49843750 )
	  END	
	END
	ELSE IF @strType = 'F'
	BEGIN
	  IF ISNULL(@userla_Group,'') <> 'A'
	  BEGIN
		SELECT DISTINCT fm_cd AS [Value], fm_desc + ' [' + fm_cd + ']' AS [DisplayName]
		FROM Family_master WITH (NOLOCK)
		WHERE fm_cd <> ''
		AND fm_cd IN(SELECT DISTINCT cm_familycd FROM CLIENT_MASTER(NOLOCK) XMAIN, @tbl_UserList
		WHERE ClientCode = XMAIN.CM_CD
		AND cm_schedule = 49843750 )
	  END
	  ELSE
	  BEGIN
		...
		FROM Family_master WITH (NOLOCK)
		WHERE fm_cd <> ''
		AND fm_cd IN(SELECT DISTINCT cm_familycd FROM CLIENT_MASTER(NOLOCK) XMAIN
		WHERE cm_schedule = 49843750 )
	  END	
	END
END
...
BEGIN
  ...
  BEGIN
    ...
    SELECT CASE WHEN SUBSTRING(da_dpid,1,2)='IN' then da_dpid+da_actno else da_actno end
    AS [Value], CASE WHEN SUBSTRING(da_dpid,1,2)='IN' then da_dpid+da_actno else da_actno end
