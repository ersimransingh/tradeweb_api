 ...
 SET @o_vcErrorMessage = 'Process Completed'
  ...
  BEGIN  
	...
	BEGIN
	  ...
	  SET @str20String = @str20String+ISNULL(@um_designation,'')+'|'+LTRIM(RTRIM(ISNULL(@cx_VerifyBy,'')))+'|'+ISNULL(@um_empCode,'')+'|'+@strCKYCCOMPNAME+'|'+@strCKYCFINO+'|'+CAST(@strIDAttCount AS VARCHAR)+'|0|01||'
	  SET @str20String = @str20String+CAST(@strIDAttCount1 AS VARCHAR)+'|'+CAST(@COUNTNO40 AS VARCHAR)+'||||'
...
BEGIN
  ...
  SELECT @strIDProof1 = IDProof, @strIDProofNo = DocNo FROM(
  SELECT TOP 1 ma_cmcd = LTRIM(RTRIM(ma_cmcd)), 
  ...
  SELECT @strAddressProof1 = AddressProof, @strAddressProofNo1 = DocNo FROM(
  SELECT TOP 1 ma_cmcd = LTRIM(RTRIM(ma_cmcd)), 
  [FILENAME] = LTRIM(RTRIM(ma_cmcd))+'_'+REPLACE(replace(substring(CASE WHEN isnull(DocumentName,'') = '' THEN ma_filename ELSE DocumentName END  ,1,15),' ',''),'/','')+'_'+
  CAST(ma_srno AS VARCHAR),
  ISNULL(KRAIDCode,'16') as IDProof, ISNULL(KRAAddressCode,'32') as AddressProof,  DocNo = (CASE WHEN DocumentName LIKE '%PAN%' THEN 
  ...
  BEGIN
    ...
    +'<APP_FILLER1></APP_FILLER1><APP_FILLER2></APP_FILLER2><APP_FILLER3></APP_FILLER3><APP_UID_TOKEN></APP_UID_TOKEN>'
  ...
  IF ISNULL(@strurl,'') = ''
  BEGIN
    SELECT @strRequestString = @strRequestString+'<ADDITIONALDATA><APP_ADDLDATA_UPDTFLG>01</APP_ADDLDATA_UPDTFLG><APP_ENTITY_PAN>'+cc_cpanno+'</APP_ENTITY_PAN>'
	  +'<APP_ADDLDATA_PAN></APP_ADDLDATA_PAN><APP_ADDLDATA_NAME>'+cc_cname+'</APP_ADDLDATA_NAME>'
