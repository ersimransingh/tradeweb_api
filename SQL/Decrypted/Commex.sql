...
*/
 --- Parameter Declaration
  ...
  DECLARE @StrReportDisplay VARCHAR(1)='P'
  ...
  @StrReportDisplay = ISNULL(x.value('(Display)[1]', 'VARCHAR(1)'),'')
...
--SELECT @strLookUp
	...
	END
	ELSE IF SUBSTRING(@strLookUp,3,1)in('F','X')
	BEGIN
      SET @strProduct = 'DERV'	  
  ...
  BEGIN TRY
	...
	BEGIN
	  ...
	  BEGIN
        SELECT CompanyCode, Exchange, Segment, ClientCode, ClientName, BillNo, BillDate, Scrip = ISNULL(Scrip,''), Symbol = isnull(Symbol,''), 
	    seriesid = ISNULL(seriesid,''), [Expiry Date] = ISNULL(ExpiryDate,''), TrxnDate, 
	    [Buy Qty] = ISNULL(BuyQty,0), [Sell Qty] = ISNULL(SellQty,0), [Market Rate] = ISNULL(MarketRate,0), 
	    [Buy Value] = ISNULL(BuyValue,0), [Sell Value] = ISNULL(SellValue,0), [Net Value] = NetValue 
	    FROM #tbl_ClientMainPL 
	    ORDER BY SerialNo
	    --RETURN 1
	  END  
  ...
  DECLARE @strCompanyName VARCHAR(500)=''
  SELECT @strCompanyName = em_name  FROM Entity_master(NOLOCK)
  IF @strOutputType <> 'X' AND ISNULL(@StrReportDisplay,'P') = 'P'
  BEGIN
    SELECT [Settings] = '<XmlData><TotalList>,Booked,Notional,MTM</TotalList><RightList></RightList>'
    +'<HideList></HideList><DateFormat>DD-MMM-YYYY</DateFormat><DateFormatList></DateFormatList>'
    +'<Dec2List></Dec2List><Dec4List>'
    +'</Dec4List><DrCRColorList></DrCRColorList>'
    +'<PnLColorList></PnLColorList><PrimaryKey></PrimaryKey>'
    +'<CompanyName>'+@strCompanyName+'</CompanyName><CompanyAdd1></CompanyAdd1><CompanyAdd2></CompanyAdd2><CompanyAdd3></CompanyAdd3>'
    +'<ReportHeader>Bill</ReportHeader><PDFWidth>520</PDFWidth><PDFHeight>269</PDFHeight>'
    +'<MobileColumns>Exchange,ClientCode,ClientName,BillNo,BillDate,Scrip,Symbol,seriesid,Expiry Date,TrxnDate,Buy Qty,Sell Qty,Market Rate,Buy Value,Sell Value,Net Value</MobileColumns>'
    +'<TabletColumns>Exchange,ClientCode,ClientName,BillNo,BillDate,Scrip,Symbol,seriesid,Expiry Date,TrxnDate,Buy Qty,Sell Qty,Market Rate,Buy Value,Sell Value,Net Value</TabletColumns>'
    +'<WebColumns>Exchange,ClientCode,ClientName,BillNo,BillDate,Scrip,Symbol,seriesid,Expiry Date,TrxnDate,Buy Qty,Sell Qty,Market Rate,Buy Value,Sell Value,Net Value</WebColumns><Headings>'
    +'<Heading></Heading><Heading></Heading></Headings></XmlData>'
  END
...
*/
 --- Parameter Declaration
  ...
  @StrLookup VARCHAR(50)='', @StrSummary VARCHAR(1)='',@StrReportDisplay VARCHAR(1)='P'
 ...
 -- INSERT INTO @tbl_Exchange
  ...
  SELECT @ExchSeg = SUBSTRING(@ExchSeg,1,2)+','+ExchangeCode from @tbl_Exchange   
  ...
  BEGIN 
	...
	BEGIN
	  ...
	  +' BF_CloseRate = (CASE WHEN sum(td_bqty- td_sqty) <> 0 THEN SUM(CASE WHEN TD_BSFlag=''B'' THEN td_bqty*CAST(td_rate AS numeric(19,6))*ISNULL(sm_multiplier,1) '
	  +' ELSE td_sqty*CAST(td_rate AS numeric(19,6))*ISNULL(sm_multiplier,1)*-1 END)'
	  +' /sum(td_bqty- td_sqty) ELSE 0 END), '
      +' BF_VALUE = SUM(CASE WHEN TD_BSFlag=''B'' THEN td_bqty*CAST(td_rate AS numeric(19,6))*ISNULL(sm_multiplier,1)*-1 ELSE td_sqty*CAST(td_rate AS numeric(19,6))*ISNULL(sm_multiplier,1) END) '
	  +' FROM Trades (NOLOCK), Series_master(NOLOCK), @tbl_UserList X  '
	...
	BEGIN
      ...
      VALUES_NS = (ISNULL(td_bqty,0)+ISNULL(td_sqty,0))*td_rate*sm_multiplier, td_rate*sm_multiplier
      FROM Trades (NOLOCK), Series_master(NOLOCK), @tbl_UserList X
      ...
      AND td_exchange = sm_exchange --and td_segment = sm_segment 
      AND td_seriesid = sm_seriesid 
	  AND ((TD_Exchange+cast(td_seriesid as varchar) =  @StrLookup and @StrLookup <> '') or @StrLookup = '')
      AND ltrim(rtrim(td_groupid)) <> 'B'
	  ...
	  VALUES_NS = td_Rate * ABS(td_bqty - td_Sqty)*sm_multiplier, td_Rate*sm_multiplier
	  FROM Trades(NOLOCK), Series_master(NOLOCK), @tbl_UserList X
      WHERE  td_clientcd = x.Client_Code
      AND td_dt >= @dtFromDate
      AND td_dt <= @dtToDt
	...
	BEGIN
	  ...
	  BEGIN
		...
		BEGIN
          ...
          CAST(seriesid AS VARCHAR) as SeriesId,  Scrip As SeriesDetails, Multiplier,  BF_QTY = ISNULL(BF_QTY,0), BF_CloseRate = ISNULL(BF_CloseRate,0), 
          ...
          FROM #tbl_ClientPL
          UNION ALL
          SELECT TAG = 2, Exchange, Segment, ClientCode, ClientName = '', SeriesId = '',  SeriesDetails = ChargesDescp, 
	      ...
	      MTM = (bc_amount)*-1, --AccountType = '', 
		  RelPL = (ISNULL(bc_amount,0))*-1, UnRelPL = 0, Lookup = Exchange+Segment, Charges = ISNULL(bc_amount,0)*-1
		  ...
		  ORDER BY ClientCode, TAG, SeriesDetails FOR XML PATH('ProfitLoss'))
	      SET @o_vcErrorMessage = CAST(@XMLDATA1 AS VARCHAR(MAX))
	    END
	    ELSE
		...
		BEGIN
	      ...
	      SOLD_QTY = 0, SOLD_RATE = 0, SOLD_VALUE = 0, EX_QTY = 0, EX_VALUE = 0, AS_QTY = 0, AS_VALUE = 0, NET_QTY = 0, CMP = 0, NET_VALUE = 0, 
	      MTM = (bc_amount)*-1, --AccountType = '', 
		  RelPL = (ISNULL(bc_amount,0))*-1, UnRelPL = 0, Lookup = Exchange+Segment
	  ...
	  ELSE
	  BEGIN
		SELECT [ClientCode] = ClientCode, [Client Name] = ClientName, Exchange, [Series ID] = SeriesId, [Series Name] = SeriesDetails, Multiplier, 
		[B/F Qty] = BF_QTY, [B/F Rate] = BF_CloseRate, 
		[B/F Value] = BF_VALUE, [Buy Qty] = BOT_QTY, [Buy Value] = BOT_VALUE,
		[Sell Qty] = SOLD_QTY, [Sell Value] = SOLD_VALUE, 
		[Excr Qty] = EX_QTY, [Excr Value] = EX_VALUE, 
		[Assign Qty] = AS_QTY, [Assign Value] = AS_VALUE, [Net Qty] = NET_QTY,
		[LTP] = CMP, [Net Value] = NET_VALUE, Booked = RelPL, [Notional] = UnRelPL, MTM, Scrip = Lookup
		FROM (
	    SELECT TAG = '1', Exchange, ClientCode, ClientName, 
        CAST(seriesid AS VARCHAR) as SeriesId, Scrip as SeriesDetails, Multiplier,  BF_QTY = ISNULL(BF_QTY,0), 
		BF_CloseRate = ISNULL(BF_CloseRate,0), 
        BF_VALUE = ISNULL(BF_VALUE,0), BOT_QTY = ISNULL(BOT_QTY,0), 
	    BOT_RATE = CAST(round((CASE WHEN ISNULL(BOT_QTY,0) <> 0 THEN ISNULL(BOT_VALUE,0)/ISNULL(BOT_QTY,0) ELSE 0 END),4) AS MONEY),
	    BOT_VALUE = ISNULL(BOT_VALUE,0), 
        SOLD_QTY = ISNULL(SOLD_QTY,0), 
	    ...
	    THEN ISNULL(MTM ,0) ELSE 0 END,
	    UnRelPL = CASE WHEN ISNULL(AccountType,'') ='' AND ISNULL(NET_QTY,0) <> 0 THEN  ISNULL(MTM ,0) ELSE 0 END,
		Lookup = Exchange+CAST(seriesid AS VARCHAR)
        FROM #tbl_ClientPL
        UNION ALL
        SELECT TAG = 2, Exchange, ClientCode, ClientName = '', SeriesId = '', SeriesDetails = ChargesDescp, 
	    Multiplier = 0,BF_QTY = 0, BF_CloseRate = 0, BF_VALUE = 0, BOT_QTY = 0, BOT_RATE = 0, BOT_VALUE = 0,
	    SOLD_QTY = 0, SOLD_RATE = 0, SOLD_VALUE = 0, EX_QTY = 0, EX_VALUE = 0, AS_QTY = 0, AS_VALUE = 0, NET_QTY = 0, CMP = 0, NET_VALUE = 0, 
	    MTM = (bc_amount)*-1, --AccountType = '', 
		RelPL = (ISNULL(bc_amount,0))*-1, UnRelPL = 0, Lookup = Exchange+'C'
	    FROM @tbl_billCharges) X1 
		--WHERE ((seriesid <> '' and @StrLookup <> '') or isnull(@StrLookup,'') = '')
		ORDER BY ClientCode, TAG, SeriesDetails
	...
	BEGIN
      ...
      Symbol, CAST(seriesid AS VARCHAR) as SeriesId, Scrip as SeriesDetails, Multiplier,  BF_QTY = ISNULL(BF_QTY,0), BF_CloseRate = ISNULL(BF_CloseRate,0), 
	...
	BEGIN
	  ...
	  IF ISNULL(@strOutputType,'') <> 'X'
	  BEGIN
	    IF ISNULL(@StrLookup,'') <> ''
		BEGIN
		  /*
		  SELECT [ClientCode] = ClientCode, [Client Name] = ClientName, Exchange, [Series ID] = CAST(seriesid AS VARCHAR), 
		  [Series Name] = SeriesDetails, Multiplier, TrxnDate, 
		  [B/F Qty] = ISNULL(BF_QTY,0), [B/F Rate] = ISNULL(BF_CloseRate,0), 
		  [B/F Value] = ISNULL(BF_VALUE,0), [Buy Qty] = ISNULL(BOT_QTY,0), [Buy Value] = ISNULL(BOT_VALUE,0),
		  [Sell Qty] = ISNULL(SOLD_QTY,0), [Sell Value] = ISNULL(SOLD_VALUE,0), 
		  [Excr Qty] = ISNULL(EX_QTY,0), [Excr Value] = ISNULL(EX_VALUE,0), 
		  [Assign Qty] = ISNULL(AS_QTY,0), [Assign Value] = ISNULL(AS_VALUE,0), [Net Qty] = ISNULL(NET_QTY,0),
		  [LTP] = ISNULL(CMP,0), [Net Value] = NET_VALUE, Booked = CASE WHEN (ISNULL(AccountType,'N') ='Y' OR ISNULL(NET_QTY,0) = 0)
          THEN ISNULL(MTM ,0) ELSE 0 END, 
		  [Notional] = CASE WHEN ISNULL(AccountType,'') ='' AND ISNULL(NET_QTY,0) <> 0 THEN  ISNULL(MTM ,0) ELSE 0 END, MTM
		  FROM #tbl_ClientPL
		 */
		  SELECT TAG = '1', Exchange, ClientCode, [Client Name] = ClientName, 
          CAST(seriesid AS VARCHAR) as [Series ID], Scrip as [Series Name], Multiplier,  [Trade Date] = TrxnDate,  [B/F Qty] = ISNULL(BF_QTY,0), 
		  [B/F Rate] = ISNULL(BF_CloseRate,0), 
          [B/F Value] = ISNULL(BF_VALUE,0), [Buy Qty]  = ISNULL(BOT_QTY,0), 
	      --[Buy Rate]  = CAST(ROUND((CASE WHEN ISNULL(BOT_QTY,0) <> 0 THEN ISNULL(BOT_VALUE,0)/ISNULL(BOT_QTY,0) ELSE 0 END),4) AS MONEY),
	      [Buy Value] = ISNULL(BOT_VALUE,0), 
          [Sell Qty] = ISNULL(SOLD_QTY,0), 
	      --[Sell Rate] = CAST(ROUND((CASE WHEN ISNULL(SOLD_QTY,0) <> 0 THEN ISNULL(SOLD_VALUE,0)/ISNULL(SOLD_QTY,0) ELSE 0 END),4) AS MONEY),
	      [Sell Value] = ISNULL(SOLD_VALUE,0), [Excr Qty] = ISNULL(EX_QTY,0), [Excr Value] = ISNULL(EX_VALUE,0), [Assign Qty] = ISNULL(AS_QTY,0), 
          [Assign Value] = ISNULL(AS_VALUE,0), [Net Qty] = ISNULL(NET_QTY,0),
          [LTP] =  ISNULL(CMP,0), [Net Value] = ISNULL(NET_VALUE,0), MTM = ISNULL(MTM ,0), 
	      --AccountType = ISNULL(AccountType,'N'),
          RelPL = CASE WHEN (ISNULL(AccountType,'N') ='Y' OR ISNULL(NET_QTY,0) = 0) 
	      THEN ISNULL(MTM ,0) ELSE 0 END,
	      UnRelPL = CASE WHEN ISNULL(AccountType,'') ='' AND ISNULL(NET_QTY,0) <> 0 THEN  ISNULL(MTM ,0) ELSE 0 END
		...
		BEGIN
	      ...
	      SELECT * FROM (
	      SELECT TAG = '1', Exchange, ClientCode, ClientName, 
          CAST(seriesid AS VARCHAR) as SeriesId, Scrip as SeriesDetails, Multiplier,  TrxnDate, BF_QTY = ISNULL(BF_QTY,0), BF_CloseRate = ISNULL(BF_CloseRate,0), 
          BF_VALUE = ISNULL(BF_VALUE,0), BOT_QTY = ISNULL(BOT_QTY,0), 
	      BOT_RATE = CAST(ROUND((CASE WHEN ISNULL(BOT_QTY,0) <> 0 THEN ISNULL(BOT_VALUE,0)/ISNULL(BOT_QTY,0) ELSE 0 END),4) AS MONEY),
	      BOT_VALUE = ISNULL(BOT_VALUE,0), 
          SOLD_QTY = ISNULL(SOLD_QTY,0), 
          ...
          FROM #tbl_ClientPL
          UNION ALL
          SELECT TAG = 2, Exchange, ClientCode, ClientName = '', SeriesId = '', ExpiryDate= '', SeriesDetails = ChargesDescp, 
	      Multiplier = 0,TrxnDate = @dtToDt , BF_QTY = 0, BF_CloseRate = 0, BF_VALUE = 0, BOT_QTY = 0, BOT_RATE = 0, 
	      BOT_VALUE = 0,
	      SOLD_QTY = 0, SOLD_RATE = 0, SOLD_VALUE = 0, EX_QTY = 0, EX_VALUE = 0, AS_QTY = 0, AS_VALUE = 0, NET_QTY = 0, CMP = 0, NET_VALUE = 0, 
	      MTM = (bc_amount)*-1, --AccountType = '', 
		  RelPL = (ISNULL(bc_amount,0))*-1, UnRelPL = 0
	      FROM @tbl_billCharges) X1 ORDER BY ClientCode, TAG, SeriesDetails, TrxnDate
		  END TRY
		  BEGIN CATCH
	  ...
	  BEGIN
	    ...
	    FROM @tbl_billCharges) X1 
		WHERE ((seriesid <> '' and @StrLookup <> '') or isnull(@StrLookup,'') = '')
		ORDER BY ClientCode, TAG, SeriesDetails, TrxnDate 
		FOR XML PATH('ProfitLoss'))
	    SET @o_vcErrorMessage = CAST(@XMLDATA1 AS VARCHAR(MAX))
	  END
	END
  ...
  IF @strOutputType <> 'X' AND ISNULL(@StrReportDisplay,'P') = 'P'
  BEGIN
    SELECT [Settings] = '<XmlData><TotalList>,Booked,Notional,MTM</TotalList><RightList>,Booked,Notional,Excr Qty,Excr Value,Assign Qty,'
    +'Assign Value,Net Qty,LTP,Net Value,MTM,B/F Qty,B/F Rate,B/F Value,Buy Qty,Buy Value,Sell Qty,Sell Value</RightList>'
    +'<HideList>,Expiry Date,Multiplier</HideList><DateFormat>DD-MMM-YYYY</DateFormat><DateFormatList>,Expiry Date</DateFormatList>'
    +'<Dec2List>,Booked,Notional,Excr Value,Assign Value,LTP,Net Value,MTM,B/F Rate,B/F Value,Buy Value,Sell Value</Dec2List><Dec4List>'
    +'</Dec4List><DrCRColorList>,MTM,Client Code,Client Name,Expiry Date,Series Name</DrCRColorList>'
    +'<PnLColorList></PnLColorList><PrimaryKey></PrimaryKey>'
    +'<CompanyName>'+@strCompanyName+'</CompanyName><CompanyAdd1></CompanyAdd1><CompanyAdd2></CompanyAdd2><CompanyAdd3></CompanyAdd3>'
    +'<ReportHeader>Profit and Loss - Client From Date - '+ISNULL(CONVERT(VARCHAR(10), CAST(@dtFromDate AS DATE), 103),'')+' to '
    +ISNULL(CONVERT(VARCHAR(10), CAST(@dtToDt AS DATE), 103),'')+'</ReportHeader><PDFWidth>520</PDFWidth><PDFHeight>269</PDFHeight>'
    +'<MobileColumns>Client Name,Series Name,Booked,Notional</MobileColumns>'
    +'<TabletColumns>Client Name,Series Name,B/F Qty,Buy Qty,Sell Qty,Excr Qty,Assign Qty,Net Qty,LTP,Booked,Notional</TabletColumns>'
    +'<Headings>'
    +'<Heading>Period '+ISNULL(CONVERT(VARCHAR(10), CAST(@dtFromDate AS DATE), 103),'')
	+' to '+ISNULL(CONVERT(VARCHAR(10), CAST(@dtToDt AS DATE), 103),'')
    +'</Heading><Heading>Segment :- Commodity </Heading></Headings></XmlData>'
  END
