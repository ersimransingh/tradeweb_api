...
BEGIN
  ...
  @strRequestFrom VARCHAR(1) = 'W', @XMLDATA1 XML =''
  ...
  IF ISNULL(@dtToDt,'') = ''
  BEGIN
    SELECT @dtToDt = ISNULL(x.value('(ToDt)[1]', 'VARCHAR(8)'), '')
    FROM @XMLData.nodes('/root') AS XTbl(x)
  END
  ...
  SELECT ld_clientcd AS ClientCode, ld_dt, Voucher AS voucherNo, ld_particular AS Narration, 
	...
	FROM (
		...
		SELECT 2 AS tag, ld_clientcd, ld_dt = CONVERT(VARCHAR, ld_dt, 112), ld_documenttype + '/' + 
...
BEGIN
  ...
  @strJsonFormat VARCHAR(1)= 'N',@strJsonText VARCHAR(MAX)=''
  ...
  @strLevel = ISNULL(x.value('(Level)[1]', 'VARCHAR(1)'), ''), 
  @strCode = ISNULL(x.value('(Code)[1]', 'VARCHAR(500)'), ''), 
  @strSelection = ISNULL(x.value('(Selection)[1]', 'VARCHAR(1)'), ''), 
  @strOutputType = ISNULL(x.value('(OutputType)[1]', 'VARCHAR(1)'), ''), 
  @strRequestFrom = ISNULL(x.value('(RequestFrom)[1]', 'VARCHAR(1)'), ''),
  ...
  Runingbalace = ABS(SUM(DebitQty+CreditQty) OVER (PARTITION  BY td_ac_code, sc_Company_Name, bt_description,  td_ac_type 
  ORDER BY td_ac_code, sc_Company_Name, bt_description, td_curdate, td_trxno, SerialNo)) 
  FROM( 
  ...
  FROM DBO.Trxdetail(nolock) where CONVERT(VARCHAR,td_curdate,112) < @dtFromDate
  ...
  UNION ALL 
  SELECT td_ac_code, td_curdate = CONVERT(VARCHAR,td_curdate,112), TAG = 2, td_isin_code, td_trxno = CAST(td_reference AS VARCHAR), td_description , td_narration, td_beneficiery, 
  td_settlement, td_ac_type , '' td_PledgeDesc, 
  ...
  FROM DBO.Trxdetail(nolock) where CONVERT(VARCHAR,td_curdate,112)  >= @dtFromDate
  and CONVERT(VARCHAR,td_curdate,112) <= @dtToDt
  ...
  IF ISNULL(@strOutputType,'G') = 'G'
  BEGIN
    SELECT ClientCode, ClientName, TrxnDate, ISIN, ScripName, TrxnNo, Description, Narration, DebitCredit, DebitQty,
...
BEGIN
  ...
  @strISIN VARCHAR(50)='', @strBenefType VARCHAR(50)='', @strSql1 NVARCHAR(MAX)='',
  ...
  BEGIN
    ...
    FROM(  
    SELECT td_ac_code = hld_ac_code, ScripName = sc_company_name, td_isin_code = hld_isin_code, td_ac_type = hld_ac_type ,  
    SUM(hld_ac_pos) as BalanceQty  
    FROM DBO.Holding(nolock), DBO.Security(nolock) , @tbl_UserList x
    where hld_ac_code = x.clientcode 
    AND hld_isin_code = sc_isincode 
    GROUP BY hld_ac_code, hld_isin_code, hld_ac_type, sc_company_name  
  ...
  BEGIN
     DECLARE @dtMax DATE
     SELECT @dtMax = max(rm_trx_date) from  DBO.Rate_master where rm_trx_date  <= CONVERT(VARCHAR,GETDATE(),112)
