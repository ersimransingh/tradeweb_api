...
*/
  ...
  @strReportDisplay VARCHAR(1)='P', @strISIN VARCHAR(50)='', @strBenefType VARCHAR(50)='', @strSql1 NVARCHAR(MAX)='',
  @strCallingFrom VARCHAR(1)=''
  ...
  @strISIN = ISNULL(x.value('(ISIN)[1]', 'VARCHAR(30)'), ''),
  @strBenefType = ISNULL(x.value('(BenefType)[1]', 'VARCHAR(30)'), '') ,
  @strCallingFrom = ISNULL(x.value('(CallingFrom)[1]', 'VARCHAR(30)'), '') 
  ...
  BEGIN
	...
	SELECT * FROM (SELECT VALUE FROM DBO.RETURNTABLE(@strUserId,'|')) X1 WHERE ISNULL(X1.VALUE,'') <> ''
  END
  ELSE
  IF ISNULL(@strCallingFrom,'') = 'T'
  BEGIN
	DECLARE @strConn1 VARCHAR(MAX)='', @tplusUser VARCHAR(50)=''
	SELECT @strConn1 = sp_sysvalue FROM Sysparameter WHERE sp_parmcd='TplusConn'

	SELECT @tplusUser = SUBSTRING(@strConn1,CHARINDEX('/', @strConn1) + 1,
	CHARINDEX('/', @strConn1, CHARINDEX('/', @strConn1) + 1) - CHARINDEX('/', @strConn1) - 1)
	...
	BEGIN
	  ...
	  SELECT CLIENT_CODE FROM ['+@tplusUser+'].DBO.fn_GetClients('''+@strUserid+''','''','''')
	  UPDATE A SET clientcode = b.da_actno
	  FROM @tbl_AccessRights A, ['+@tplusUser+'].DBO.DEMATACT B
  ...
  CREATE TABLE #tbl_HoldingRepDP (ClientCode VARCHAR(500), ClientName VARCHAR(200), BranchCode VARCHAR(50),
  ScripCode VARCHAR(50), ScripName VARCHAR(200), ISIN VARCHAR(20), Qty MONEY, ClosingPrice MONEY,
  MarketValue MONEY, AccountType VARCHAR(500), BackOfficeCode VARCHAR(509))
  
  BEGIN TRY  
  IF @strReportType = 'UPTODATE'
  BEGIN  
	INSERT INTO #tbl_HoldingRepDP(ClientCode, ClientName, BranchCode, ScripName,
	ISIN, AccountType, Qty, ClosingPrice, MarketValue, BackOfficeCode)
	SELECT cm_cd As ClientCode, cm_name As ClientName, cm_brboffcode As BranchCode, 
	ScripName , td_isin_code As ISIN, bt_description, BalanceQty As Qty, 0, 0, cm_blsavingcd 
	FROM (Select td_ac_code, td_isin_code, ScripName , bt_description , td_ac_type,  BalanceQty = sum(BalanceQty)  
	FROM(  
	...
	GROUP BY td_ac_code, td_isin_code, bt_description, td_ac_type, ScripName  ) A, [dbo].client_master(NOLOCK) 
	...
	AND ((td_ac_type = @strBenefType and @strBenefType <> '') or @strBenefType = '')
  ...
  BEGIN
	 ...
	 BEGIN
	   SELECT ClientCode, ClientName, BackOfficeCode, [ISINName] = ScripName, ISIN, [BalanceType] = AccountType, 
	 ...
	 BEGIN
	   ...
	   BEGIN
		 SET @XMLDATA1 = 
		 (SELECT ClientCode = '', ClientName = '', BackOfficeCode = '', ISINName = '', ISIN ='', BalanceType = '', 
   ...
   BEGIN
	 ...
	 BEGIN
	   ...
	   BEGIN
		 SET @XMLDATA1 = 
		 (SELECT ClientCode ='', BackOfficeCode = '', ClientName ='', ISINName = '', 
