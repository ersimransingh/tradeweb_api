 ...
 */
  ...
  BEGIN
    ...
    DECLARE @StrBinaryFile NVARCHAR(MAX)='', @strString VARCHAR(MAX)='', @strEsignDB VARCHAR(50)=''
   
    SELECT @strEsignDB = LTRIM(RTRIM(OP_DataBase)) 
	...
	BEGIN
      ...
      +' FROM  '+@strEsignDB+'.DBO.Digital_details(NOLOCK) WHERE dd_filetype = ''CMRG'' AND dd_clientcd = '''+@i_vcUserCode+'''' 
      +' AND DD_DT IN(SELECT MAX(DD_DT) FROM  '+@strEsignDB+'.DBO.Digital_details(NOLOCK) WHERE dd_filetype = ''CMRG'''
