...
*/
  ...
  BEGIN
	...
	IF @strScripCode <> ''
	BEGIN
	  SET @StrString =   @StrString+' AND SSCD = '''+@strScripCode+''' '
	END
    ...
    BEGIN
	  ...
	  BEGIN
        ...
        +' FROM (SELECT Client_code cmcd,  im_scripcd sscd, SS_Name As ScripName, ISNULL(cm_poaforpayin,''N'')+''DP'' Dtype, hld_ac_pos As hld_ac_pos, im_isin '
  ...
  BEGIN
	...
	BEGIN
      ...
      ORDER BY ClientCode, ScripCode FOR XML PATH('DPHolding'))
	  SET @o_vcErrorMessage = CAST(@XMLDATA1 AS VARCHAR(MAX))
  ...
  BEGIN
   ...
   BEGIN
	 ...
	 */
	 ...
	 [EXP], UNDEL, MTFBENF, MTFCOLL, [POOL], Ben, TotalQty, AvgRate = ISNULL(AvgRate,0), 
   ...
   BEGIN
     ...
     ORDER BY ScripName, ClientCode, AccountType, ScripName
	 DROP TABLE #tbl_HoldingRepdp
   END
   SET @o_vcErrorMessage = 'Process Completed'
  END
