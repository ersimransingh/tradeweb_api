...
BEGIN
  ...
  @ISINReqId VARCHAR(24)='', @ISINResId VARCHAR(24)='', @strRefNo VARCHAR(16)='',
...
BEGIN
  ...
  DECLARE @strAPIRequest VARCHAR(100)='', 
  ...
  BEGIN  
    ...
    SET @strRequest = REPLACE(@strRequest,'##reqtime##',CAST(FORMAT(GETDATE(), 'ddMMyyyyHHmmss') AS VARCHAR))
	SET @strRequest = REPLACE(@strRequest,'##dpid##',SUBSTRING(@strdpid,3,6))
	...
	SET @strjsondata = (SELECT  pledgeeboreqid, [pledgeeboid] = @strdpid, [pledgeseqno] = PSNNo, [prfno] = PledgeFormNo, [pledgetype] = 'MR',
	                    [pledgequantity] = CAST(QTY AS INT), [pledgeexpdate] = CAST(FORMAT(GETDATE(), 'ddMMyyyy') AS VARCHAR),
	                    [pledgevalue] = CAST(StockValue AS INT), agrmntno, pldgrdpintrefno, reasoncode = @strReasonCode, cmid = @strCMID
	                    FROM TBL_ClientMarginPledgeDtl(NOLOCK) 
	                    WHERE RequestDate = CONVERT(VARCHAR,GETDATE(),112)
	                    AND ClientCode = @curClientCode
	                    AND CDSLReqNo = @strRefNo
	...
	SET @strRequest = REPLACE(@strRequest,'##DATA##',@strjsondata)  
    ...
    --- ADD DECRIPTION
	  ...
	  BEGIN
	    ...
	    SELECT @strbrokerOrderNo = ColumnValue FROM @tbl_jsonoutput 
	    WHERE MasterTag = '' and ColumnName = 'reqid'
	
	
	    IF @strbrokerOrderNo <> @strRefNo
	    BEGIN
	      SET @o_vcOutPutJSON ='{"ResponseFlag":"E","ResponseMessage":"##ErrorMessage##","DATA":""}'                             
	    ...
	    AND MasterTag='' AND ColumnValue = '0')
	    BEGIN
	      DECLARE CURSOR_CDSLINQ CURSOR FOR      
          SELECT DISTINCT JSONLEVEL  
          ...
          BEGIN  
		    ...
		    SELECT @strStatus, @strReqid, @strReqid1, @strRefNo, @curClientCode
	        IF ISNULL(@strStatus,'') <> '0'
	        BEGIN
...
BEGIN
  ...
  BEGIN
    ...
    WHERE RequestDate = CONVERT(VARCHAR,GETDATE(),112)
    AND (ISNULL(PSNNo,'') = '' OR ISNULL(ResponseCode,'') = 'X')
...
BEGIN
  ...
  DECLARE @strRandomNoFinal VARCHAR(30)='', @strCRandomNoFinal VARCHAR(16)=''
  ...
  BEGIN
    ...
    [value] = CAST(CAST(X.StockValue AS INT) AS VARCHAR), [segmentid] = @strCSegmentid , [cmid] = @strCCmId, [ReasonCode] = @strReasonCode
	...
	SET @strRequestString  = REPLACE(@strRequestString, '##DATA##',@strJsonData)
	SET @o_vcJsonOutput = @strRequestString
	SET @o_vcParam = (SELECT [APIUrl] = @strAPIUrl, DPId  = SUBSTRING(@strpledgeeboid,3,8),
