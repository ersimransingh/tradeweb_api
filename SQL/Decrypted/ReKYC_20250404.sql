...
BEGIN
  ...
  SELECT @svar = COUNT(*)
   ...
   WHERE X1.ColumnName = XMAIN.JsonKey
   and mastertag = xmain.MasterJsonTag  AND DefaultValueTag IN('B','U')
   AND XMAIN.RecordType ='M' and xmain.MasterJsonTag = 'NomineeDetails' AND XMAIN.JsonKey <> 'NomineeResidualSecurities'
...
BEGIN
  ...
  and ma_filename in('Sign','RPD_GetResponse','SignedPdf','IPV_FINAL') )
  BEGIN
	...
	when ma_filename = 'IPV_FINAL' then @i_vcClientCode+'_'+'IPV Image' ELSE @i_vcClientCode+'_'+ma_filename END,
...
BEGIN
  ...
  BEGIN
	...
	Set @CompanyAddress = ''
    Select @DPID = Left(da_dpid,8), @ClientId = RIGHT(da_actno,8)   From Dematact where da_clientcd = @strClientCode 
...
CREATE OR ALTER PROCEDURE [stpr_APIReKycExport]
  @i_vcProjectName VARCHAR(50), @i_vcModuleName VARCHAR(100),                                   
  @i_vcFunctionName VARCHAR(100), @i_vcSource VARCHAR(1),                                 
  @i_vcUniqueID VARCHAR(50), @i_vcUserID VARCHAR(50),                                   
  @i_vcInputJSON NVARCHAR(MAX) , @o_vcOutPutJSON NVARCHAR(MAX) OUTPUT                                 
	...
	BEGIN
	   ...
	   BEGIN
		 ...
		 BEGIN
		   ...
		   and RequestUniqueID = xg.RequestUniqueID)),'Pending')
		 ...
		 BEGIN
		   ...
		   AND ISNULL(cn_KRAStatus,'') <> '' AND cn_filler5 IN('Modification Validated','KRA Verified','KRA Validated','Existing KYC Verified'))
           AND x.rm_refno in(select max(rm_refno) from Client_ReKycMain(NOLOCK) where rm_RequestType ='ReKYC'
           AND rm_cmcd = x.rm_cmcd)
		   AND X.mkrdt >= @strFromDate
...
CREATE OR ALTER PROCEDURE stpr_APICVLKRAStatusUpdate @vcXML NVARCHAR(MAX), @o_vcErrorFlag VARCHAR(1) OUTPUT, @o_vcErrorMessage VARCHAR(MAX) OUTPUT AS
BEGIN
  ...
  BEGIN
    ...
    VALUES(0,'CVLKRA',@strClient,@strRequestString,'','','SA',GETDATE())
	...
	BEGIN
	  UPDATE A SET A.ResponseString = @StrKRAPANStatus+' WITH '+@StrKRAPANCode, A.ExceptionMessage = @StrKRAPANStatus+' WITH '+@StrKRAPANCode
      FROM tbl_GenericAPIDebugLog A
      WHERE A.SerialNo =  @strLogSerialNo	
	  SET @o_vcErrorFlag = 'E'
	...
	ELSE 
	  ...
	  UPDATE A SET A.ResponseString = @o_DecyText, A.ExceptionMessage = @StrKRAPANStatus+' WITH '+@StrKRAPANCode
      FROM tbl_GenericAPIDebugLog A
      WHERE A.SerialNo =  @strLogSerialNo	
...
CREATE OR ALTER PROCEDURE [dbo].[stpr_GetKRAEncryDecry] @i_vcInputJson NVARCHAR(MAX), @i_vcType VARCHAR(50), 
...
CREATE OR ALTER PROCEDURE stpr_APICKYCFILEGEN @vcXML NVARCHAR(MAX), @o_vcErrorFlag VARCHAR(1) OUTPUT, @o_vcErrorMessage NVARCHAR(MAX) OUTPUT AS
BEGIN
  --SET @vcXML = '<root>'+@vcXML+'</root>' 
...
CREATE OR ALTER PROCEDURE stpr_APICVLKRA @vcXML NVARCHAR(MAX), @o_vcErrorFlag VARCHAR(1) OUTPUT, @o_vcErrorMessage VARCHAR(MAX) OUTPUT AS
BEGIN
  --SET @vcXML = '<root>'+@vcXML+'</root>' 
  DECLARE @XMLData XML = CAST(@vcXML AS XML), @strClient VARCHAR(20)='', @strThirdPartyURL VARCHAR(200)='',
  @o_vcOutputJsonapi VARCHAR(MAX)='', @strKRAAPIPassword NVARCHAR(MAX)='', @strurl VARCHAR(MAX)='', @StrHeaderString VARCHAR(MAX)=''
  ...
  BEGIN
	...
	IF LTRIM(RTRIM(@StrKRAPANStatus)) NOT IN('Existing KYC Verified','KRA Validated','KRA Verified','KYC Registered with CVLMF')
	BEGIN
	  UPDATE A SET A.ResponseString = @StrKRAPANStatus+' WITH '+@StrKRAPANCode, A.ExceptionMessage = @StrKRAPANStatus+' WITH '+@StrKRAPANCode
      FROM tbl_GenericAPIDebugLog A
      WHERE A.SerialNo =  @strLogSerialNo	
	...
	*/
	...
	IF @strAckStatus NOT LIKE 'ERR-%' AND @strAckNo <> '0'
	BEGIN
	  
 	  UPDATE A SET A.ResponseString = @o_DecyText, A.ExceptionMessage = 'Process Executed'
      FROM tbl_GenericAPIDebugLog A
      WHERE A.SerialNo =  @strLogSerialNo	
